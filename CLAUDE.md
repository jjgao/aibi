...
**`SPEC.md` is the source of truth.** Read §3 (principles) before changing anything in
`schema/`, `engine/` or `analyses/`. If a change conflicts with the spec, update the spec in
the same PR or don't make the change.

## Status

Pre-code: the repository holds the spec only. Milestone M0 (§13) creates the skeleton below.
...
- **Missing is not negative.** Keep query logic three-valued (§6.3). Never drop, impute or
  reclassify a missing observation without counting it in the result.
...
```
server/src/aibi/{schema,store,importers,catalog,engine,analyses,api,mcp,assistant}
server/tests/
web/
fixtures/
```
...
- Pydantic models in `schema/` are the single definition of every descriptor, document and
