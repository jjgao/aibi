...
exactly ``memory allocation of N bytes failed``). Running out of time, or of CPU time, is
``LIMIT_EXCEEDED`` naming ``reader_seconds``. Any other ``BaseException`` that is no
``Exception``, a panic or an abort otherwise, and a child that ends in any other way before it
answers are ``UNPARSEABLE_SOURCE``. A ``MemoryError`` in the server is ``LIMIT_EXCEEDED`` too:
naming ``import_bytes`` while it pickles and sends a request, which holds a file's bytes, and
``decoded_bytes`` while it receives or unpickles an answer. Any other exception, and an answer
that breaks the framing or holds another class, is a fault of the reader, raised as
``ReaderError``. After a failure the child is gone, and the import is refused.
"""
...
def _answer(
    ...
    try:
        ...
        return ("panic", type(error).__name__), decoded
...
def _received(sock: socket.socket, size: int, deadline: float) -> bytes:
...
def _exactly(sock: socket.socket, size: int, deadline: float) -> bytes:
    parts: list[bytes] = []
    while size:
        parts.append(_received(sock, size, deadline))
        size -= len(parts[-1])
    return b"".join(parts)


def _receive(sock: socket.socket, deadline: float, maximum: int) -> deque[bytes]:
    """One frame, as ``serve`` writes it, read under ``deadline``: at most
    ``maximum`` bytes, in chunks."""
    (size,) = struct.unpack("!i", _exactly(sock, 4, deadline))
    if size == -1:
        (size,) = struct.unpack("!Q", _exactly(sock, 8, deadline))
    if size < 0 or size > maximum:
        raise ReaderError(f"a reader's answer claims {size} bytes; at most {maximum} are read")
    chunks: deque[bytes] = deque()
    while size:
        chunks.append(_received(sock, min(size, _CHUNK), deadline))
        size -= len(chunks[-1])
    return chunks
...
class _Ended:
    ...
    peak: int
    """Its peak memory in bytes: its peak address space if it was read before it ended (Linux),
    its peak resident size otherwise."""
    cpu: float
    stderr: bytes


def _peak(pid: int) -> int:
    """A living process's peak address space, in bytes, from ``/proc``; 0 where there is none."""
    try:
...
class _Child:
    ...
    def stop(self, wait: float = 0.0) -> _Ended:
        ...
        resident = usage.ru_maxrss * (1 if sys.platform == "darwin" else 1024)
        return _Ended(
...
class Reader:
    ...
    def run[**P, T](self, function: Callable[P, T], *args: P.args, **kwargs: P.kwargs) -> T:
        ...
        try:
            ...
            answer = _Unpickler(
                _Chunks(_receive(child.sock, self._deadline, self.limits.reader_memory))
            ).load()
        except _Late:
            self._kill()
            raise refused(
        ...
        if kind == "panic":
            raise self._ended(ended, str(found[1]))
