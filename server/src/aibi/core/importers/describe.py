...
- ``imported``: what the source gave. Labels from original names, a column's original name,
  a sheet or Parquet table's ``source``, a datatype a Parquet file declares, and the dataset's
  ``name``, ``source`` and ``packs``.
...
- ``proposed``: everything else the importer inferred. Grain, role, primary key, relationships,
  coverage, list syntax, identifiers, and the datatypes it did not read from the source.
...
class TableOrigin:
    ...
    original_name: str
    """The file's name, or the sheet's."""
    label: str
...
class _Curation:
    ...
    def descriptor(
        ...
        """A descriptor whose ``label`` and ``fields`` each have (value, status, evidence)."""
    ...
    *,
    ...
    found: list[Descriptor] = [
        curation.descriptor(
            ...
            (dataset.name, "imported", None),
            {
                ...
                "source": (
                    {"kind": "files", "location": dataset.location},
                    "imported",
                    None,
                ),
    ...
    for table in inferred.tables:
        ...
        if table.key is not None:
            fields["primary_key"] = (list(table.key), "proposed", table.key_evidence)
            grain = "One row per " + " and ".join(table.key)
            fields["grain"] = (grain, "proposed", "From the key (D229)")
        found.append(
            curation.descriptor("table", table.id, (origin.label, "imported", None), fields)
        )
        for name, (column, guess) in zip(origin.columns, table.columns.items(), strict=True):
            ...
            found.append(curation.descriptor("column", f"{table.id}.{column}", label, members))
    for relationship in inferred.relationships:
        ...
        found.append(
            curation.descriptor(
                ...
                relationship.id,
                (
                    f"{relationship.child}.{relationship.column} to {relationship.parent}",
                    "imported_default",
                    None,
                ),
                {
                    "child_table": (relationship.child, "proposed", why),
                    "child_columns": ([relationship.column], "proposed", why),
                    "parent_table": (relationship.parent, "proposed", why),
                    "parent_columns": ([relationship.parent_column], "proposed", why),
                    "cardinality": (
