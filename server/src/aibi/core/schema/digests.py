"""Hashes and digests of canonical objects and outputs (SPEC §7.6, §9.3; D282, D283, D298).

Every hash is the lowercase hexadecimal SHA-256 of the RFC 8785 serialisation of the object it
names, in UTF-8; sorting by a canonical serialisation compares it as UTF-16 code units, as RFC
8785 orders keys.
...
The digests live with the schemas so that an output checks its own: a cohort count or a result
envelope whose ``digest`` is not the digest of what it carries cannot be built (D298).
"""
...
def output_digest(
    dumped: Mapping[str, JsonValue], caveats: Sequence[Caveat], members: Sequence[str]
) -> str:
    """The digest of an output from its JSON form: the ``members`` of §7.6 as ``dumped`` holds
    them, and its caveats reduced."""
    found = {member: dumped[member] for member in members if member != "caveats"}
    return digest({**found, "caveats": reduced(caveats)})
