...
- the rules of the disclosure pass that need no data (§8.4): nothing is suppressed, and no
  ...
  *k* − 1, nor any count of a shown breakdown; no linked set shows one suppressed member, or a
  suppressed total, through a non-zero other; ``lift_differs`` is suppressed whenever a
  ``min_cell_count`` is set; with ``n_true`` shown, ``n`` and
...
- ``analysed.n`` is at least each variable's ``n`` and at most their sum, and so each
  variable excludes at least as many units; ``lift_differs`` is at most the size;
...
def _check_disclosure(
    ...
    if population.lift_differs is not None and k is not None:
        raise problem(
            "disclosure",
            "lift_differs counts units of one side of n_true, n_false and n_unknown, which "
            "another cohort of the same document counts beside the rest, and is suppressed "
            "whenever a min_cell_count is set (§8.4)",
        )
    for entry in analysed:
