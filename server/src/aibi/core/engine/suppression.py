...
- **Linked counts**, over a larger table, until nothing changes (``suppressed``): a count from 1
  ...
  ``size.numerator``).
- ``lift_differs`` is always suppressed, a 0 included: it counts units of one side of the
  cohort's accounting (a strict cohort's ``some`` steps turn only unknown units definite when
  the lift flips, so it is at most ``n_unknown``; a cohort whose lift turns definite units
  unknown has it at most ``n_true`` or ``n_false``), and the counts of ``not X``, ``known X``
  and ``unknown X`` move that side into another place, so ``known X``'s ``lift_differs``,
  shown beside its own counts, would bound one of ``X``'s suppressed counts by the size less
  it, whether or not any of ``known X``'s counts is small (D297).
...
``not X``, ``known X`` and ``unknown X`` counted together, each with its ``lift_differs``:
``test_suppression``'s exhaustive checks, the family's for *k* of 3 or more (under *k* = 2,
where 1 to *k* − 1 is one value, a ``known X`` beside two counts of 1 shows them, and so the
third). A suppressed member is ``null``, listed in the population's ``suppressed`` or in the
size's ``not_estimable`` as ``suppressed``; the count then carries ``SUPPRESSED``, as every
count under *k* does. ``UNKNOWN_EXCLUDED`` and ``LIFT_DIFFERS`` are carried whenever
``n_unknown`` and ``lift_differs`` are suppressed, as the model requires, with messages that
hold whether or not a unit is unknown or changes; their messages quote only the counts shown
(§8.4).
...
def disclosed(parts: CountParts, k: int | None) -> CountParts:
    ...
    hidden = {*suppressed(counts.n_true, counts.n_false, counts.n_unknown, k), "lift_differs"}
