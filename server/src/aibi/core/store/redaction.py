...
How a constant is matched follows one principle (D290): a constant goes when it could name one
of the person's rows (theirs or one below them), or when it quotes one of the person's own
identifying texts; anything else stays. Where it sits says which rows it could name
(``Naming.column``, ``Terms.part_places``, ``Terms.is_key``):

- on a key column of one column, or a foreign key of one column (a ``covered`` clause's scope
  column being one of its table's): the keys of the person's rows in the table its values name,
  its own or the one it points into, by every reading, JSON numbers too, and the values that
  identify the person (``Naming.identifying``: their key's and their rows' identifier values
  that are not numbers by datatype) by their text alone, as on another column; no other term,
  so a number on a key of a table that holds none of their rows matches nothing, while the
  person's text key ``b-2`` goes there too;
- a unit key: as a whole, part by part by value, against the keys of the person's rows in its
  unit's table (every part erased when they match), and each part as a constant on its column; a
  part of a key, or of a foreign key, of more columns names rows only through the whole key, so
  it matches by itself only the values that identify the person, by their text, but a part that
  is a foreign key of one column still names the table it points into;
- on an identifier column (§5.4) of any table: the values that identify the person (their key's
  and their rows' identifier values that are not numbers by datatype) and the person's values of
  that column, numbers too, by every reading, and every other term by its text alone;
- on a column or unit the core cannot name (a parameter, a value concept or a concept unit, a
  scope of a table given as either), in a pack leaf as written, and anywhere with
  ``redact_only``: every term by every reading (``Place.NAMING``);
- on any other known column, and anywhere in a tree, or a cohort or a view as written, over
  another dataset: a string by its text alone (``Place.OTHER``), the whole of it or a whole
  token of it that is no part of a longer number, date, time or range, so that ``07``, ``7.0``,
  ``+7``, ``6-7`` and ``7-8`` do not hold ``7``, nor ``2024-01-01T00:00:00Z`` or
  ``2024-01-01 10:00`` the date ``2024-01-01``; JSON numbers and booleans, and the numbers below
  the person, never (``Where.quoted`` holds some terms so where ``Place.OTHER`` holds none);
- in free text, structure, a parameter no clause refers to, a result's strings, the audit trail
  and the proposal queue, where nothing says what it is (``Place.UNKNOWN``): every term by every
  reading, but for JSON numbers and booleans, and for the terms below the person that are numbers
  by their columns' datatypes (a surrogate key ``2``), which are matched in neither.

A number in other ``units`` than its column's is read only as evaluation reads it, converted by
one multiplication of doubles by the factor rounded to a double (§6.4, ``Where.scales``). Text
is compared ``normal``: in Unicode normalisation form C, without format characters; text that
holds no term is kept as it is written.
...
- the term as it is written, next to no letter, digit or mark (Unicode category ``M``), in any
  ...
  ``José``, nor ``क`` from ``कि``; text terms match exactly once ``normal``, case included:
  keys are exact;
...
- a number (``0017``, ``+17``, ``17.0``, ``1.7e1``: a sign, digits, a fraction and an exponent)
  ...
  whose value is one of the person's terms' (so ``017`` in ``member 017: Grace``, but neither
  ``10:017`` nor ``3/017``: no number there is ``17``). Digits grouped by separators
  (``1,017``) are read as no number, neither the whole nor a group. A number is parsed only if
  its significant digits are one of the person's terms', or more than a double's
  (``DOUBLE_DIGITS``).
...
class Place(IntEnum):
    """How a constant is matched beyond the terms that may name its rows (D290; the module's
    docstring): the kinds of place a ``Where`` is made of."""

    OTHER = 0
    """A string by its text alone: the whole of it, or a whole token that is no part of a
    longer number, date, time or range."""
    UNKNOWN = 1
    """By every reading, but never a JSON number or boolean, nor a number below the person."""
    NAMING = 2
    """By every reading, every term: where the core cannot say which rows a value names."""


@dataclass(frozen=True)
class Where:
    """Where a constant is (D290). ``naming`` is the terms it holds by every reading, JSON
    numbers and booleans too, since its values may name the rows those terms are keys or
    identifying values of (``None`` for every term, where the core cannot say which rows they
    name). ``rest`` says how it holds every other term: as ``Place.UNKNOWN`` or ``Place.OTHER``
    does, or not at all (``None``). ``quoted`` is the terms it holds besides by their text
    alone, as ``Place.OTHER`` holds every term: on a key, the texts that identify the person.
    ``scales`` are the factors by which a JSON number there is read, as evaluation reads a
    constant in other units than its column's (§6.4): ``1`` as it is written, and the factor to
    its column's units where it is in others. A value in more than one place (a parameter
    referred to from several) is matched as in each: ``a | b`` holds what either holds. Its
    ``repr`` counts the terms and names none of them (D269)."""

    naming: frozenset[str] | None = frozenset()
    rest: Place | None = None
    scales: frozenset[float] = frozenset({1.0})
    quoted: frozenset[str] = frozenset()

    def __post_init__(self) -> None:
        if self.rest is Place.NAMING:
            raise ValueError("a naming place is given by its terms")

    def __or__(self, other: "Where") -> "Where":
        terms = None if self.naming is None or other.naming is None else self.naming | other.naming
        rests = [rest for rest in (self.rest, other.rest) if rest is not None]
        return Where(
            terms,
            max(rests) if rests else None,
            self.scales | other.scales,
            self.quoted | other.quoted,
        )

    def __repr__(self) -> str:
        naming = "every term" if self.naming is None else f"{len(self.naming)} terms"
        return (
            f"Where(naming={naming}, rest={self.rest!r}, scales={sorted(self.scales)!r}, "
            f"quoted={len(self.quoted)} terms)"
        )

    @classmethod
    def of(cls, place: "Place | Where") -> "Where":
        """A place as a ``Where``; ``Place.NAMING`` alone holds every term."""
        if isinstance(place, Where):
            return place
        return cls(None) if place is Place.NAMING else cls(frozenset(), place)


NOWHERE = Where(scales=frozenset())
"""A place that holds no term and reads no number: where nothing has referred to a parameter
yet, so that the places that refer to it alone say how its numbers are read (a number of hours
is read converted to days, never also as it is written)."""
...
@cache
def _formats() -> dict[int, None]:
    """The format characters (Unicode category ``Cf``, as this Python's ``unicodedata`` has
    them): zero-width spaces and joiners, the soft hyphen, bidirectional marks."""
    return {c: None for c in range(sys.maxunicode + 1) if unicodedata.category(chr(c)) == "Cf"}


def normal(text: str) -> str:
    """Text as erasure compares it (D290): without format characters, which are invisible, and
    in Unicode normalisation form C, so that ``José`` is one name however its accent is
    encoded, and ``m-1\u200b7`` is ``m-17``."""
    if text.isascii():
        return text
    return unicodedata.normalize("NFC", text.translate(_formats()))
...
def _scaled(value: JsonValue, scales: frozenset[float]) -> frozenset[Reading]:
    """A number constant's readings in its column's units, at each factor but ``1``, by one
    multiplication of doubles as evaluation converts it (§6.4): a JSON number, or a string the
    engine reads as one (an integer beyond the safe range)."""
    number = typed_constant(value, "number") if isinstance(value, str) else value
    if not isinstance(number, int | float) or isinstance(number, bool):
        return frozenset()
    found: set[Reading] = set()
    for ratio in scales - {1.0}:
        with suppress(OverflowError):
            found |= _json_readings(float(number) * ratio)
    return frozenset(found)
...
DOUBLE_DIGITS = 15
"""A double's decimal precision (``DBL_DIG``): two numbers written with at most this many
significant digits are one double only if they are one number, so a number in free text whose
significant digits are no term's is not parsed."""


def _significant(number: str) -> str:
    """A number's significant digits as written: its mantissa's digits, without the point, the
    sign, and zeros at either end (``""`` for zero), which every spelling of its value shares
    (``0017``, ``17.0`` and ``1.7e1`` are ``17``, and so is ``1700``)."""
    mantissa = number.lstrip("+-").partition("e")[0].partition("E")[0]
    return mantissa.replace(".", "").strip("0")


def _frozen(given: Mapping[str, Collection[Any]]) -> dict[str, frozenset[Any]]:
    return {key: frozenset(values) for key, values in given.items()}


def _listed(given: Mapping[str, frozenset[Any]]) -> dict[str, list[Any]]:
    return {key: sorted(values) for key, values in sorted(given.items())}


@dataclass(frozen=True)
class Naming:
    """What the releases that held the person say of the places whose constants may name their
    rows, for the rule table of D290 (``column``). Built by ``Naming.of``, which normalises the
    values (``normal``). Its ``repr`` counts the values and names none of them (D269)."""

    keys: Mapping[str, frozenset[tuple[str, ...]]]
    """By table, the keys of the person's rows there, in any release, each part its canonical
    string."""
    key_columns: Mapping[str, frozenset[tuple[str, ...]]]
    """By table, its key's columns, as each release has them."""
    names: Mapping[str, frozenset[str]]
    """By column id, the tables whose rows its values name by themselves: its own table's for
    the one column of its table's key, the table it points into for the one column of a foreign
    key; none for a part of a key or a foreign key of more columns, which name rows only as a
    whole."""
    identifiers: Mapping[str, frozenset[str]]
    """By identifier column (§5.4), the values the person's rows have there."""
    identifying: frozenset[str]
    """The values that identify the person in any identifier column: those of their key and of
    their rows' identifier columns that are not numbers by their columns' datatypes."""
    units: Mapping[str, str]
    """By column id, the column's units."""

    @classmethod
    def of(
        cls,
        *,
        keys: Mapping[str, Collection[Sequence[str]]],
        key_columns: Mapping[str, Collection[Sequence[str]]],
        names: Mapping[str, Collection[str]],
        identifiers: Mapping[str, Collection[str]],
        identifying: Collection[str],
        units: Mapping[str, str],
    ) -> "Naming":
        return cls(
            {t: frozenset(tuple(normal(p) for p in key) for key in k) for t, k in keys.items()},
            {table: frozenset(tuple(c) for c in found) for table, found in key_columns.items()},
            _frozen(names),
            {column: frozenset(normal(v) for v in found) for column, found in identifiers.items()},
            frozenset(normal(value) for value in identifying),
            dict(units),
        )

    def __repr__(self) -> str:
        keys = sum(len(found) for found in self.keys.values())
        values = sum(len(found) for found in self.identifiers.values())
        return (
            f"Naming(tables={sorted(self.key_columns)!r}, keys={keys}, "
            f"identifier values={values}, identifying={len(self.identifying)})"
        )

    def column(self, column: str, units: str | None) -> Where:
        """Where a constant on a column is, by the rule table of D290: on a key or foreign key
        column, the keys of the person's rows in the table its values name by every reading,
        and the values that identify the person by their text alone (``quoted``), and no other
        term; on an identifier column, the values that identify the person and the person's
        values of that column, by every reading, and any other term by its text; on any other
        known column, every term by its text alone. A number in ``units`` other than the
        column's is also read converted to them (``scales``)."""
        tables = self.names.get(column)
        values = self.identifiers.get(column)
        if tables is None and values is None:
            return _OTHER
        found = {
            key[0] for table in tables or () for key in self.keys.get(table, ()) if len(key) == 1
        }
        if values is not None:
            found |= self.identifying | values
        given = self.units.get(column)
        ratio = None if units is None or given is None else scale(units, given)
        scales = frozenset({1.0 if ratio is None else ratio})
        rest = None if values is None else Place.OTHER
        quoted = frozenset[str]() if tables is None else self.identifying
        return Where(frozenset(found), rest, scales, quoted)

    def dumps(self) -> dict[str, Any]:
        return {
            "keys": {t: sorted(list(key) for key in k) for t, k in sorted(self.keys.items())},
            "key_columns": {
                t: sorted(list(columns) for columns in found)
                for t, found in sorted(self.key_columns.items())
            },
            "names": _listed(self.names),
            "identifiers": _listed(self.identifiers),
            "identifying": sorted(self.identifying),
            "units": dict(sorted(self.units.items())),
        }

    @classmethod
    def loads(cls, stored: Mapping[str, Any]) -> "Naming":
        return cls.of(
            keys=stored["keys"],
            key_columns=stored["key_columns"],
            names=stored["names"],
            identifiers=stored["identifiers"],
            identifying=stored["identifying"],
            units=stored["units"],
        )


class Terms:
    """The values to erase: the person's and those below them, each matched as its place says
    (``Where``, D290), and what the releases say of the places that may name the person's rows
    (``Naming``; ``None`` when no release says, as with ``redact_only``, so that every column
    and unit names every term's rows). Terms are compared as ``normal`` makes text. A term is
    found in text by splitting the text into tokens (``_runs``) and looking up the runs of them
    that may be one in sets, so that no pattern is compiled of the terms, however many there
    are."""
    ...
    def __init__(
        ...
        *,
        ...
        self.person = frozenset(normal(term) for term in person if term)
        self.below = frozenset(normal(term) for term in below if term) - self.person
        self.terms = self.person | self.below
        self.numeric = frozenset(normal(term) for term in numbers) & self.terms
        ...
        gives (``0017``, ``1.7e1``)."""
        self.digits = frozenset(_significant(reading[1]) for reading in self.values)
        """Their significant digits, which a number in free text must have to be parsed."""
        ...
        return json.dumps(
            {
                ...
                "naming": None if self.naming is None else self.naming.dumps(),
            }
        )
    ...
    def loads(cls, text: str) -> "Terms":
        ...
        naming = stored.get("naming")
        return cls(
            stored["person"] or (),
            stored["below"] or (),
            numbers=stored.get("numbers") or (),
            naming=None if naming is None else Naming.loads(naming),
        )

    def place(self, column: JsonValue, units: JsonValue = None) -> Where:
        """Where a column puts its constants (``Naming.column``), in ``units`` if given; every
        term's naming place when no release says or the column is not a descriptor id as
        written (a ``"$name"`` parameter, or a value concept, from M6, whose column each dataset
        maps, D290)."""
        if (
            self.naming is None
            ...
            return _NAMING
        return self.naming.column(column, units if isinstance(units, str) else None)

    def part_places(self, unit: str | None, count: int) -> list[Where]:
        """Where each part of a unit key of ``count`` parts is: on its key column (``place``),
        a part of a key of more columns naming rows only as the whole key does (``is_key``);
        every term's naming place when no release says or the unit is not known to be a table
        (a concept unit, from M6, whose table each dataset maps, D290)."""
        naming = self.naming
        if naming is None or unit is None or unit not in naming.key_columns:
            return [_NAMING] * count
        found: list[Where] | None = None
        for columns in naming.key_columns[unit]:
            if len(columns) == count:
                places = [self.place(f"{unit}.{column}") for column in columns]
                found = (
                    places if found is None else [a | b for a, b in zip(found, places, strict=True)]
                )
        return [_NAMING] * count if found is None else found

    def is_key(self, unit: str | None, parts: Sequence[JsonValue]) -> bool:
        """Whether a unit key, as a whole, is the key of one of the person's rows in its unit's
        table: each part sharing a reading with that key's part (D290)."""
        if self.naming is None or unit is None:
            return False
        read = [self._value_readings(part) for part in parts]
        return any(
            len(key) == len(read)
            and all(
                not found.isdisjoint(self.readings(part))
                for found, part in zip(read, key, strict=True)
            )
            for key in self.naming.keys.get(unit, ())
        )

    def _value_readings(self, value: JsonValue) -> frozenset[Reading]:
        return self.readings(normal(value)) if isinstance(value, str) else _json_readings(value)

    def _every(self, term: str, where: Where) -> bool:
        """Whether a term is matched by every reading where it is: one of the naming place's
        terms, or any term on ``Place.UNKNOWN``."""
        return where.rest is Place.UNKNOWN or where.naming is None or term in where.naming

    @staticmethod
    def _textual(term: str, where: Where) -> bool:
        """Whether a term is matched by its text alone where it is: any term on
        ``Place.OTHER``, or one the place quotes."""
        return where.rest is Place.OTHER or term in where.quoted
    ...
    def text(self, value: str, place: Place | Where = Place.UNKNOWN) -> str:
        ...
        number whose value is one of the person's terms'; on ``Place.OTHER``, and for the terms a
        place quotes, a term as it is written only, and no part of a longer number, date or
        time; on a naming place, every reading of its terms only. Text that holds no term is
        returned as it is; text that holds one, ``normal``."""
        where = Where.of(place)
        if where.naming == frozenset() and where.rest is None and not where.quoted:
            return value
        found = normal(value)
        redacted = self._tokens(found, where)
        if where.naming != frozenset() or where.rest is Place.UNKNOWN:
            if self.instants:
                redacted = _instants().sub(lambda at: self._instant(at, where), redacted)
            if self.values:
                redacted = _numbers().sub(lambda at: self._number(at, where), redacted)
        return value if redacted == found else redacted
            ...
            if counts is not None and (run is not None or at == 0 or tokens[at - 1][2] is None):
                for count in counts:
                    ...
                    if term in self.exact and (
                        ...
                        or (self._textual(term, where) and _bounded(value, term, start, stop))
                    ):
    ...
    def _number(self, found: re.Match[str], where: Where) -> str:
        """A number in free text, ``[erased]`` if its value is one of the person's terms' that
        its place holds; parsed only if its significant digits could be a term's."""
        number = found.group()
        digits = _significant(number)
        if digits not in self.digits and len(digits) <= DOUBLE_DIGITS:
            return number
        for reading in self.readings(number) & self.values:
            if any(
    ...
    def whole(self, value: JsonValue, place: Place | Where) -> bool:
        ...
        boolean is one only on a naming place, where it shares a reading with one of its terms,
        a number read at each of the place's ``scales`` (a count bound, an age or a surrogate
        key of another table that happens to equal a term names no one, nor does a number of
        hours that happens to equal a number of days). A string on ``Place.OTHER`` is one when
        it is the text of a term but the numbers below the person, and so is one a place quotes
        (``Where.quoted``) when it is the text of one of those; elsewhere, when it shares a
        reading with a term: on a naming place one of its terms, and on ``Place.UNKNOWN`` any of
        the person's and one below them read as anything but a number (``"2"`` names no loan
        there, but ``"104233"`` is a member)."""
        where = Where.of(place)
        if isinstance(value, str):
            value = normal(value)
            if value in self.exact and self._textual(value, where):
                return True
        found = self._value_readings(value)
        if where.naming != frozenset():
            number = isinstance(value, int | float) and not isinstance(value, bool)
            raw = isinstance(value, str) or not number or 1.0 in where.scales
            read = found if raw else frozenset[Reading]()
            for reading in read | _scaled(value, where.scales):
                for term in self.index.get(reading, ()):
                    if where.naming is None or term in where.naming:
                        return True
        if isinstance(value, str) and where.rest is Place.UNKNOWN:
            return any(
    ...
    def string(self, value: str) -> str:
        ...
        free text."""
        return MARK if not self.readings(normal(value)).isdisjoint(self.index) else self.text(value)
...
def _naming_holds(value: JsonValue, terms: Terms) -> bool:
    """Whether what the core cannot read holds a term, as a constant that holds every term by
    every reading does (JSON numbers and booleans too), or as an object key."""
    if isinstance(value, list):
...
def _scope_place(terms: Terms, table: JsonValue, column: str) -> Where:
    ...
    the clause's ``table`` by its name alone, so it is that table's column (``Terms.place``); a
    table the core cannot name (a ``"$name"`` parameter, or a concept, from M6) puts them in
    every term's naming place."""
    known = (
        isinstance(table, str) and _reference(table) is None and not CONCEPT_ID_RE.fullmatch(table)
    )
    return terms.place(f"{table}.{column}" if known else None)


def _key_holds(terms: Terms, unit: str | None, parts: list[JsonValue], ours: bool) -> bool:
    """Whether a unit key holds a term: as a whole, the key of one of the person's rows in the
    unit's table, or a part of it on its key column (``Terms.part_places``); in a tree over
    another dataset's release, a part by its text alone."""
    if not ours:
        return any(terms.in_constant(part, _OTHER) for part in parts)
    places = terms.part_places(unit, len(parts))
    return terms.is_key(unit, parts) or any(
        terms.in_constant(part, place) for part, place in zip(parts, places, strict=True)
    )
...
def _clause_holds(clause: JsonValue, terms: Terms, unit: str | None, ours: bool) -> bool:
    ...
    ``scope`` values and unit keys (§7.6), each where its column or unit puts it (``Where``).
    ...
    if kind == "value":
        place = terms.place(clause.get("column"), clause.get("units")) if ours else _OTHER
    ...
    if kind == "ids":
        ...
        for key in keys if isinstance(keys, list) else []:
            ...
            if isinstance(parts, list) and _key_holds(terms, unit, parts, ours):
                return True
        return False
...
class _Written:
    ...
    def __init__(self, terms: Terms, unit: str | None, dataset: str) -> None:
        ...
        whose strings are unit keys as written (``"lib:2"``), and whether one of the cohorts
        that refer to them may be over ``dataset``."""
        self.clauses: dict[str, bool] = {}
        """The parameters referred to as a clause, and whether one of the cohorts or views that
        refer to them may be over ``dataset``."""
        self.given: Mapping[str, JsonValue] = {}
        """The document's own parameters, as written, which give the units a ``"$name"``
        stands for."""
        self.verbatim = False
    ...
    def place(self, column: JsonValue, units: JsonValue = None) -> Where:
        name = self.reference(units)
        if name is not None:
            units = self.given.get(name)
        return self.terms.place(column, units) if self.ours else _OTHER
    ...
    def parameter(self, name: str, value: JsonValue) -> JsonValue:
        """A parameter, redacted as the places that refer to it are: its unit keys and its
        clauses as those of the cohorts that refer to it (over ``dataset`` if one of them may
        be, or if none refers to it), and its constants in each place that refers to them."""
        if name in self.keys:
            self.ours = self.keys[name]
            try:
                value = (
                    [self.key(item) for item in value]
                    if isinstance(value, list)
                    else self.key(value)
                )
            finally:
        ...
        if _clause_shaped(value) or (
            ...
            self.ours = self.clauses.get(name, True)
            try:
    ...
    def clause(self, clause: JsonValue) -> JsonValue:
        ...
        if not isinstance(clause, dict):
            ...
            if name is not None:
                self.clauses[name] = self.clauses.get(name, False) or self.ours
        ...
        kind = clause.get("kind")
        if kind == "value":
            place = self.place(clause.get("column"), clause.get("units"))
            return {
    ...
    def ids(self, ids: JsonValue) -> JsonValue:
        ...
        if isinstance(ids, list):
            return [self.key(member) for member in ids]
        name = self.reference(ids)
        if name is not None:
            return self.keyed_by(name)
        return self.pack(ids)

    def keyed_by(self, name: str) -> str:
        """Record a reference to a parameter from an ``ids`` clause; the reference, redacted."""
        self.keys[name] = self.keys.get(name, False) or self.ours
        return self.refer(name, NOWHERE)

    def key(self, member: JsonValue) -> JsonValue:
        """A unit key as written (``"<dataset>:<key>"``, split at the first ``:``, or
        ``{dataset, key}``): every part ``[erased]`` if, as a whole, it is the key of one of the
        person's rows (``Terms.is_key``), and each part otherwise by ``Terms.constant`` where
        its key column puts it (``Terms.part_places``); a parameter that stands for it is
        redacted as it is."""
        ...
        if isinstance(member, str) and ":" in member:
            ...
            return f"{self.plain(dataset)}:{self.parts([key])[0]}"
        if isinstance(member, dict):
            return {
                key: self.parts(part)
                if key == "key" and isinstance(part, list)
                else self.plain(part)
                for key, part in member.items()
            }
        return self.parts([member])[0]

    def parts(self, parts: list[JsonValue]) -> list[JsonValue]:
        """A unit key's parts, redacted (``key``)."""
        if not self.ours:
            return [self.constants(part, _OTHER) for part in parts]
        literal = all(self.reference(part) is None for part in parts)
        if literal and self.terms.is_key(self.unit, parts):
            return [MARK] * len(parts)
        places = self.terms.part_places(self.unit, len(parts))
        return [self.constants(part, place) for part, place in zip(parts, places, strict=True)]
    ...
    def pack(self, value: JsonValue) -> JsonValue:
        """What the core cannot read (a pack leaf as written): every constant as one that holds
        every term by every reading, JSON numbers and booleans too, and every parameter it
        refers to redacted as one; in a cohort over another dataset, in ``Place.OTHER``."""
