...
issuances included, and read in outline: descriptors, never rows (§12.2). The pins are the
background kind (``Store.pin``), so that releasing them never has the call wait on the store's
lock, a sweep, a redaction or the vacuum after it (D300). A draft's effective
