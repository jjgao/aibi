...
def test_a_count_of_zero_is_shown_and_takes_nothing_with_it(
    ...
    assert parts == count_parts(cohort, tally(0, 30, 0))
...
def test_lift_differs_is_a_linked_set_alone(canon: Canon, city: City, doc: Doc) -> None:
    cohort = one_clause(canon, city, doc)
...
def test_lift_differs_is_suppressed_with_any_count_zeros_included(
    canon: Canon, city: City, doc: Doc
) -> None:
    """A strict cohort's ``lift_differs`` is at most its ``n_unknown``, and may equal it, so a
    shown one beside a suppressed count could pin it; it is suppressed, and ``LIFT_DIFFERS``
    carried without a number, whatever it was (D297)."""
    cohort = one_clause(canon, city, doc)
    for lift in (0, 9):
        parts = disclosed(count_parts(cohort, tally(3, 20, 9, lift_differs=lift)), 5)
        population = parts.population
        assert (population.n_true, population.n_unknown, population.lift_differs) == (
            None,
            9,
            None,
        )
        ...
        assert _digits(parts, CaveatCode.LIFT_DIFFERS) == ""
        served(cohort, parts, 5)
    shown = disclosed(count_parts(cohort, tally(20, 20, 9, lift_differs=9)), 5)
    assert shown.population.lift_differs == 9
...
def test_a_breakdown_with_a_small_count_is_null_as_a_whole(
    ...
    assert population.suppressed == ["/unknown_by_reason"]
    served(cohort, parts, 5)
...
def test_a_leaf_breakdown_with_a_small_count_is_null_beside_a_shown_n_unknown(
    ...
    assert population.suppressed == ["/unknown_by_leaf"]
    served(cohort, parts, 5)
...
def test_the_digest_is_taken_after_the_pass(canon: Canon, city: City, doc: Doc) -> None:
    ...
    assert disclosed(count_parts(cohort, tally(20, 30, 9)), 5).digest == (
...
def _pinned(
    ...
    for shown, found in seen.items():
        ...
        for name in names:
            ...
            aside = name == "lift_differs" and (k == 2 or values == {0})
...
def test_lift_differs_and_the_flags_caveats_pin_no_suppressed_count_either(
    ...
    *k* − 1, *k* and *k* + 1 (at most the size) and without or with the units' flags, whose
    caveats say that some unit carries them: a suppressed ``lift_differs`` is pinned only
    under *k* = 2, as a 1."""
    cohort = one_clause(canon, city, doc)
    for k in range(2, 6):
        counted = [
...
def test_a_lift_differs_bounded_by_the_counts_pins_no_suppressed_count(
    ...
    count two values or more; a suppressed ``lift_differs`` is pinned only where the counts
    shown bound it to 0, and under *k* = 2 as a 1 (D297)."""
    cohort = one_clause(canon, city, doc)
