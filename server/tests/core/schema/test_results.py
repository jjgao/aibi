...
def _lift_hidden(value: dict[str, Any]) -> dict[str, Any]:
    """An envelope whose populations suppress ``lift_differs`` under a ``min_cell_count``, as
    the pass does (§8.4, D297)."""
    if value["derivation"]["disclosure"]["min_cell_count"] is None:
        return value
    if all(given["code"] != "SUPPRESSED" for given in value["caveats"]):
        value["caveats"] = [*value["caveats"], caveat("SUPPRESSED", ["/population"])]
    for position, population in enumerate(value["population"]):
        if population["lift_differs"] is not None:
...
def _count_with(n_true: int | None, n_false: int | None, n_unknown: int | None, size: int) -> Any:
    """A cohort count under k = 5; members given as None are suppressed, and so is
    ``lift_differs``, as under every k (D297)."""
...
def test_no_lift_differs_count_is_shown_under_a_min_cell_count_whatever_its_size() -> None:
    """``known X`` moves the side of ``X`` its ``lift_differs`` counts into another place, so
    one shown beside counts of any size bounds a count of ``X`` the pass suppressed (D297)."""
    for lift in (0, 2, 5, 12):
        value = envelope(
            population=[known_population(lift_differs=lift), known_population()],
            derivation=derivation(disclosure=K5),
            caveats=[
                caveat("SUPPRESSED", ["/population"]),
                caveat("LIFT_DIFFERS", ["/population"]),
            ],
        )
        assert "disclosure" in error_types(ResultEnvelope, value)
        value["derivation"]["disclosure"] = {"min_cell_count": None}
        value["caveats"] = value["caveats"][1:] if lift else []
        assert _valid(ResultEnvelope, value)
    assert _valid(
        ResultEnvelope,
        _lift_hidden(
            envelope(
                population=[known_population(lift_differs=5), known_population()],
                derivation=derivation(disclosure=K5),
            )
        ),
    )
...
def _accounted(
    ...
    if not size:
        ...
        count["caveats"] = [*count["caveats"], caveat("NOT_ESTIMABLE", ["/population"])]
...
@pytest.mark.parametrize(
    ...
    [
        ((12, 0, 0), 12, None, None),
        ((12, 0, 0), 12, 12, "disclosure"),  # shown under k, beside counts of any size (D297)
        ((None, None, 0), 12, None, None),
        ((None, None, 0), 12, 5, "disclosure"),
        ((0, 0, 0), 0, None, None),  # an empty table's, 0, is suppressed as any other
    ],
)
def test_lift_differs_is_suppressed_under_a_min_cell_count_whatever_the_counts(
    parts: tuple[int | None, int | None, int | None], size: int, lift: int | None, error: str | None
) -> None:
    count = _accounted(parts, size, lift)
    if error is None:
