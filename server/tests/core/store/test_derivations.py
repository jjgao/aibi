...
@example(key="integer", tag="datetime", at="integer", seed=20)
@example(key="integer", tag="datetime", at="string", seed=0)
@example(key="date", tag="datetime", at="integer", seed=0)
@example(key="string", tag="datetime", at="integer", seed=0)
...
def _distinct(
    store: Store, manifest: str, count: int, start: int, notes: int = 0, others: int = 0
) -> list[Issue]:
    """Issuances of ``count`` derivations no other has, from ``start``: distinct cohorts, each
    written with ``notes`` bytes of notes and over ``others`` releases of other datasets besides
    ``manifest``'s, each listed with its dataset and manifest alone, as short as a listing
    goes."""
    ...
    for number in range(start, start + count):
        ...
        listed: list[JsonValue] = [release]
        over: dict[str, JsonValue] = {manifest: tree}
        for other in range(others):
            elsewhere = "sha256:" + hashlib.sha256(f"{number}/{other}".encode()).hexdigest()
            listed.append({"dataset": f"d{other}", "manifest": elsewhere})
            over[elsewhere] = tree
        hashed: JsonValue = {
            "cohort": over,
            "unit": "members",
...
@pytest.mark.parametrize(
    ("notes", "again", "others"),
    [
        ((0,), True, 0),  # a count made again: its issuance alone
        ((0,), False, 0),  # small rows
        ((1950,), False, 0),  # a text just over half a page, one to a page
        ((4100,), False, 0),  # just over a page: a leaf's local part and an overflow page
        ((13000,), False, 0),  # several pages
        ((3700, 0), False, 0),  # a text near a page, then a small one, and again
        ((0,), False, 63),  # 64 releases each, their rows charged by LOG_RELEASE_FACTOR
    ],
)
def test_the_log_counts_at_least_the_pages_its_rows_take(
    store: Store, imported: str, notes: tuple[int, ...], again: bool, others: int
) -> None:
    """``usage`` grows by at least the bytes of the pages the log adds to the app DB, for rows
    of every size and rows of two sizes in turn, and derivations over many releases, whose
    rows and index entries ``LOG_RELEASE_FACTOR`` charges, and by at most a little over twice
    that: each row's cell is charged twice, for the page the next one may leave part empty
    (D300)."""
    log = store.derivations
    log.issue_all(_distinct(store, imported, 1, 0, others=others))
    pages, used = _pages(store), log.usage()
    for number in range(1, 201):
        size = notes[number % len(notes)]
        log.issue_all(_distinct(store, imported, 1, 0 if again else number, size, others))
