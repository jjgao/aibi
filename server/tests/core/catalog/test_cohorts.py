...
def test_validate_document_reads_back_and_marks_ids_not_yet_issued(
    ...
    written = document(
        ...
        params={"h": 5, "unused": 1},
        views=[{"analysis": "compare.columns", "cohorts": ["tall"]}],
    )
    ...
    assert "M3.2c" in json.dumps(deferred.model_dump(mode="json")["message"])
...
def test_a_due_vacuum_runs_in_the_store_s_housekeeping_never_in_the_call_s_thread(
    world: World, orchard: Orchard, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A redaction left waiting runs when the call's pins are released, and the vacuum after it
    may take long: the call answers within its deadline, its issuances recorded, while the
    store's housekeeping thread vacuums (D300)."""
    world.publish("orchard", orchard())
    store = world.store
    vacuumed: list[str] = []
    ...
    monkeypatch.setattr(store.db, "vacuum", slow)
    with store.db.transaction() as db:
        request = store.request_redaction(db, "orchard", Terms(["nobody"]), [])
    try:
        deadline, found = _within(RECORD_SECONDS + 10, catalog_of(world), {"document": document()})
        assert isinstance(found, CohortCounts), found
        assert time.monotonic() < deadline.at
    ...
    [named] = found.counts
    assert store.derivations.issuances(named.count.id) == [named.count.issuance.id]
...
def test_an_erasure_while_the_call_canonicalises_refuses_it_and_it_records_nothing(
    world: World, orchard: Orchard, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The erasure is recorded after the call began, before it pinned the releases and
    canonicalised the document, so it may have run over the log before the call records: the
