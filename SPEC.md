...
**Status:** Draft v0.8.16 · 2026-09-26
...
| **Derivation** | The canonical, hashed description of how a result or count was produced (§7.6), kept in the derivation log while an issuance names it (§12.2). |
...
credentials, §14; a database's location is its host, database and schema, or its file's location,
D305), `license`, `data_use` (a list of `OntologyRef`, e.g. GA4GH DUO codes),
...
- **Translation.** Documents in other formats (cbio-lab's first) are translated by pack document
  translators (§10.1) through `validate_document`, which flags every negation (`not`, `negate`,
  `!=`, `every`) whose meaning may change under three-valued logic (D303).
...
- The document as written, including pack leaves, is kept in the result envelope outside every
  hash; `validate_document` returns both forms, `explain` the canonical one, and the operator
  router the document as written (D302).
...
Phase 2 checks each view against its registry entry: the analysis must be one the registry holds
(`UNKNOWN_ANALYSIS`, listing them), its parameters are the analysis's, refused where they are
written, one that declares `uses_reference` has its cohorts listed, and each predicate of its
parameters holds no `ids` and no `cohort` leaf (`LEAF_NOT_ALLOWED`) and is resolved with the
cohorts, on the unit table, as a cohort of one clause; each variable of its parameters (§9.2) is
resolved with them too, its `where` holding no `ids`, `cohort` or pack leaf (D325). A view whose
cohorts, predicates and variables all canonicalised gets its canonical form (every default
written, each predicate its canonical clause tree, each variable its canonical form) and its ids; a view that names no cohorts orders them by their computation ids, so
that raising a floor never reorders them (D284, D317).
...
A readback is a function of the canonical form and the release's descriptors alone, rendered in
the canonical order: two documents with one canonical form have one readback, but for the words of
pack summaries, which packs render from their leaves as written. Every label, id, constant, unit,
key (each part of a composite one) and the release's label is a data token; the template text
holds nothing from data or the document. For each down step it states the coverage the question
relies on (every row's children recorded, recorded only for the listed rows and by which scope
columns, or undeclared, and whether it is only proposed), its record filter and its parent scope;
it ends by saying that a unit whose answer cannot be decided is unknown and not counted, and pack
summaries follow, each labelled as the pack's summary of its leaf in the conditions whose leaf keys
it names, and ordered by them, never by the leaf's pointer, which holds the cohort's name. Lists
are cut: at most 64 constants of a `values` list, keys of an `ids` leaf or values of a scope, then
how many more there are (D296).
...
- **Cohort counts** (`count_cohort`, §11.1) are `{id, digest, population, size, disclosure,
  ...
  counts: "unit_table"}` and no `excluded`, since nothing is excluded from the unit table; its
  `denominator`, the unit table's size, is suppressed only when that is from 1 to *k* − 1 (D297).
  ...
  `CONFOUNDED_WITH_DATASET`, `INVALID_EXCLUDED`). A count over an empty unit table carries
  `NOT_ESTIMABLE` for its size (D297). `count_cohort` returns one per cohort, by cohort name,
  each beside its name (as data) and the map from its leaves as written to its leaf keys, with
  the views checked as `validate_document` checks them and the parameters used (D300, D317).
- **Digests are checked.** A result envelope or a cohort count whose `digest` is not the digest of
  the digested members it carries cannot be built (D298).
...
| `LIFT_DIFFERS` | info | The other lift rule would change some units' results, of a cohort or of a predicate a view's parameters ask (D323); the message gives the count (subject to §8.4), in the pack's wording where a pack provides one |
...
- With *k* set, a disclosure pass runs on every output after it is computed. A suppressed value
  ...
  - **Linked counts.** In each linked set, a count from 1 to *k* − 1 is suppressed, and if exactly
    ...
    `n_unknown`, whose total, the size of the unit table, is a member (cohort counts, below);
    each predicate's split of a cohort's units, with `analysed` (predicates of a result, below);
    the cells of each row and each column of a cohort ×
    ...
    and `lift_differs` alone, which is always suppressed, a 0 included: the units it counts lie
    on one side of `n_true`, `n_false` and `n_unknown`, which `not`, `known` or `unknown` of the
    cohort counts beside the rest, so shown beside any counts it could bound a suppressed one. A
    count of 0 is otherwise shown. The pass runs after categories are pooled and
    ...
    suppresses only values that were computed; but a value whose counts are suppressed is
    `suppressed` whatever else would have made it not estimable, since `no_units` or
    `zero_denominator` beside it would give a suppressed count away as 0 (D320).
  - **Breakdowns.** A per-reason or per-leaf map with any count from 1 to *k* − 1 is replaced by
    `null` as a whole. In catalogue statistics, categories with any cell from 1 to *k* − 1 are
    pooled into one *suppressed categories* row; if that row still has such a cell, the
    distribution is suppressed (D271). In a result, a variable's categories are merged in their
    listed order as histogram bins are (below), so that a merged row's count is never read off
    as a pooled row's can be, and a category column's values that it does not declare are one
    row, after the declared ones, that names none of them, since a value held by few units is
    itself what they hold (D329).
  ...
  - **Distributions.** Histograms take their bin edges from the analysis parameters, or divide
    ...
    *below* and *above* bins. Repeatedly, the leftmost bin with 1 to *k* − 1 units is merged,
    together with any empty bins between them, with the nearest non-empty bin on its right (on
    its left when none is), until no bin has 1 to *k* − 1 units or one non-empty bin remains;
    empty bins are otherwise kept (D329: merging the smallest first, toward the neighbour with
    fewer units, gave the order it merged in away, and with it the counts merged). Minima and
    maxima are not reported; medians and quartiles are reported as the merged bin that contains
    them, read off the merged bins' counts, and not at all where the two values a quantile lies
    between are in two bins. No other statistic computed from the values themselves (a mean, a
    standard deviation) is reported: values are not counts, so the rule that hides counts from 1
    to *k* − 1 cannot protect them, and a mean over coarse bins gives the counts finer bins hide,
    a standard deviation of zero every unit's value (D329).
    Catalogue statistics follow the same rules, and pool categories with fewer
  ...
  - **Predicates of a result** (D320): at each position, a predicate's split of the cohort's
    units, those for which it is TRUE, FALSE and UNKNOWN, is disclosed exactly as a cohort count
    is (below), the cohort's `n_true` its size. A proportion shows its numerator where the TRUE
    units are shown, and its denominator, and the predicate's `excluded_units`, where the UNKNOWN
    units are (the size less them); the pass never leaves one of the three suppressed alone. A
    position whose `n_true` is suppressed shows none of its predicates' counts. With one predicate,
    `analysed` is its split's. With more, no count that combines predicates is shown: `analysed`'s
    `n`, `excluded_units` and `excluded` are `null`, since the predicates' own counts bound a
    joint count from both sides (the units for which none is known are at least the sum of their
    UNKNOWN units less the size once for each predicate after the first, and at most the fewest)
    and a map of their reasons bounds each predicate's own; each predicate's `variables` entry is
    disclosed as its split is. A figure that combines predicates may be shown only where the
    counts shown give it alone, and an `excluded` map is `null` wherever a map of the same units
    is. Every estimate, interval, effect, test and chart value is computed only from counts
    shown: an effect with both proportions' counts, a test with every position's, and a
    proportion whose numerator of 0 is shown beside a suppressed denominator withholds its
    estimate and interval with it, though both would be 0. TRUE and FALSE are treated alike, so a
    predicate and its `not` are disclosed alike, and a count a view shows as FALSE of one and TRUE
    of the other is suppressed in both. At one position, nothing shown pins a suppressed count of
    a predicate's split to one value, for a predicate alone or beside its `not`, `known` and
    `unknown` in one view, or beside predicates independent of it, but under *k* = 2, as for
    cohort counts (where a reason map of two UNKNOWN units split one and one is pinned too). The
    positions are disclosed apart, as the cohorts of one call are: positions that are unions of
    others give each other's counts by difference, and a two-valued collapse of a predicate (`P or
    unknown P`, `P and known P`) beside it is another predicate; §8.4's Limits disclaim both, as
    they do for cohorts.
  - **Cohort counts** (D297): the unit table's size is the total of the linked set `n_true`,
    `n_false` and `n_unknown`, and a member of it: from 1 to *k* − 1 it is suppressed
    (`size.denominator`), as the catalogue suppresses that table's `n_rows`, and a table of fewer
    than max(*k*, 4) units shows none of the three counts, zeros included. Over a larger table the
    size is shown, and every count's complement is a member too (it is a count of `not`, `known`
    or `unknown` of the cohort): a complement from 1 to *k* − 1 suppresses a non-zero count. One
    suppressed alone takes another with it: `n_true` and `n_false` each other (`n_unknown` when
    the other is 0), and `n_unknown` the smaller non-zero of the two, both on a tie; two
    suppressed beside a non-zero shown count take it too when the rules split their sum only one
    way. `n_true` and `n_false` are treated alike, so `not` of a cohort is disclosed as the cohort
    with the two swapped. `lift_differs` is a set alone, and is always suppressed, a 0 included:
    the units it counts lie on one side of the cohort (a strict cohort of `some` steps turns only
    unknown units definite when its lifts flip, so its `lift_differs` is at most, and may be, its
    `n_unknown`; one whose lifts turn definite units unknown bounds it by `n_true` or
    `n_false`), and `known` of the cohort counts that side beside the rest, so its
    `lift_differs`, shown beside counts all large, would give back by the size less it a count
    of the cohort the pass suppressed. The breakdowns are `null` with
    `n_unknown` or when any of their counts is from 1 to *k* − 1; the size's estimate is
    suppressed with its numerator or its denominator; `UNKNOWN_EXCLUDED` and `LIFT_DIFFERS` are
    carried whenever `n_unknown` and `lift_differs` are suppressed, worded to say nothing of
    whether a unit is unknown or would change; and the
    messages of `UNKNOWN_EXCLUDED` and `LIFT_DIFFERS` quote only the counts shown. Nothing shown
    pins a suppressed count to one value, for a cohort alone or counted with its `not`, `known`
    and `unknown` in one call, each with its `lift_differs`, but under *k* = 2, where 1 to *k* − 1 is itself one value. The
    cohorts of one output are otherwise disclosed each alone, so a count does not depend on what
    else its call counts (Limits). The digest is taken after the pass.
...
- `allow_row_ids: false` refuses the `ids` leaf, the `summary.members` analysis and predicates on
  identifier columns, and requires `min_cell_count`. `summary.members` is also refused under any
  `min_cell_count`, the effective *k* of §7.6 (a deployment floor included), whatever its cohort's
  size: a unit key is held by one unit, so no count rule protects a list of them, and lists of
  cohorts whose sizes are shown give by difference the keys of a set of any size (every unit,
  `not X` and `X and known X` give the units whose `X` is unknown), which would make every count
  the pass hides a list of named units; under *k* keys may be written into a document (`ids` leaves, predicates on
  identifier columns), never read out of an output (D332).
- **Limits.** These settings reduce casual disclosure. They do not prevent inference across
  repeated queries (for example by differencing two counts, or two cohorts of one `count_cohort`
  call, which are disclosed apart, unless one is the other's `not`, `known` or `unknown`), and
  they are not a privacy guarantee. A two-valued collapse of a cohort is such another cohort:
  `X and known X` counts `X`'s `n_true` beside its `n_false` + `n_unknown`, and `X or unknown X`
  its `n_true` + `n_unknown` beside its `n_false`, so that, shown whole when they are large, they
  give back counts `X`'s own pass suppressed beside a shown one (over 24 units under *k* = 4, `X`
  of 8, 13 and 3 shows only its 13, and `X and known X` shows 8, 16 and 0). No pass over one
  cohort's count can prevent that: the collapse's counts are its own, and suppressing them for
  `X`'s sake would need the pass to recognise every cohort that differs from another by few
  units, which is differencing again; so is a predicate's collapse asked beside it in one view
  (D320). Whether cohorts share units is reported too: a view of an analysis that assumes
  independent groups is refused with the count of what they share, and a result not refused
  says they share none, which beside the sizes of the other cohorts and of the unit table can
  bound a suppressed size (D318). A result's positions are disclosed apart, as the cohorts of one
  call are, so a view whose cohorts are unions of one another, allowed with `overlap: "allow"`
  (`A`, `not A` and `known A`), gives a predicate's suppressed count at one position by
  difference of those shown at the others (a numerator of 12 over `known A` beside 10 over `not
  A` gives the 2 over `A`), as the same cohorts asked in separate calls do (D320). Restricted
  data needs access control, which is outside v1 (§1.2). Without it, whether a cohort was counted
  on the server within the retention period of a count's issuances, and whether a view's result
  was issued within that of a result's (§12.2), is visible to anyone who can write the cohort or
  the view (its id's status in the derivation log, and `validate_document`'s `status`), though
  not when; issuance ids are not guessable, and `explain`
  gives no issuance's document as written (D302). Distributions (D329): a call reads a
  column's values with one set of bins only, across its views and whichever variable reads them
  (the column, or `max`, `min` or `mean` of it), but two calls with other bins give by difference
  the units of a bin that merging hides (bins `[0, 45, 100]` beside `[0, 44, 100]` give those in
  [44, 45)), as the same cohorts asked in separate calls do, and two variables of one column that
  are not histograms of its values (`some` of two value sets, a column and a `count` over its
  rows) give by difference what one of them hides, as a predicate beside its collapse does, and
  so do two histograms with the same bins of variables whose units or rows differ (another
  `empty`, `where`, `lift` or aggregate: `max` of a column beside the same with `empty` 0 gives
  the units without rows in its lowest bin), as two cohorts that differ by few units do; an
  `excluded` map, whose reasons overlap, gives by inclusion and
  exclusion the units excluded for two reasons, as a cohort's `unknown_by_reason` does; and
  catalogue statistics still pool categories (D271), so declared values missing from a list are
  those pooled, each 1 when the pooled count is their number (#49).
...
transforms that compute numbers, no expressions built from data, and no URLs, `$schema`
included; a value that is `null` has no row (D322). Clients render
...
A tool that takes a document (`validate_document`, `count_cohort`) points the refusals of the
document into the document as written, not into the request that carries it; problems of the
request itself, and of its JSON text, point into the request (D299).
...
A column is **multi-valued** for a unit when it is below the unit or list-valued. A view reads a
column as a **variable**, `{column, via?, aggregate?, values?, where?, lift?, empty?, bins?}`:
`values` goes with `some` and `every` alone, `where` (the conditions a row reached at the last
down step must meet, on its table as an `exists` leaf's `where` is) with `count`, `max`, `min`
and `mean`, `empty` with `max`, `min` and `mean`, and `bins` with numbers; a variable resolves
on the unit table in the release of the view's cohorts, one value per unit or the unit excluded
with its reasons (D325, D326).
...
- Numeric aggregates are computed over the rows reached at the last down step of the column's
  ...
  proposed. A view that leaves such scope columns open is refused (`OPEN_SCOPE`); one that
  gives a numeric aggregate of what it cannot take (`mean` of what is not a number, `max` or
  `min` of what is neither a number nor an ordered category, or any aggregate of a column with
  one value per unit) is refused (`AGGREGATE_NOT_ALLOWED`), and one that reads a multi-valued
  number without an aggregate (`AGGREGATE_REQUIRED`); `max` and `min` of an ordered category
  compare its listed order, a value outside it counting as UNKNOWN (`NO_INFORMATION`).
...
- Descriptive analyses count units per category, with a denominator per category (the units for
  ...
  need an `aggregate` or `count: "rows"`. Per-category denominators and `count: "rows"` come
  with M3.2c; until then a multi-valued categorical column without an `aggregate`, and a
  variable's `count`, are `NOT_SUPPORTED` (D324).
...
- **Arithmetic.** DuckDB computes only `SUM`, `COUNT`, `MIN`, `MAX` and `AVG` over integer and
  DECIMAL columns, `MIN` and `MAX` over floating-point columns too (they pick one of the values
  and compute nothing, `-0.0` being read as `0.0`; D326), and `BIT_OR` and `BOOL_OR` over integers
  and booleans, for values that enter a digest (D294). Every other aggregate (any aggregate over
...
`imported_default`). The dataset page and `describe_dataset` both show this, and `list_analyses`
given a dataset; without a unit, each keyed table is tried and the best status kept (D316).
...
| Id | Returns |
|---|---|
| `summary.distribution` | Per column, per cohort: for categories, units per category (proportions over units whose value is known; per-category denominators for multi-valued columns, §9.2); for numbers, n, mean, standard deviation, median, quartiles, minimum and maximum, and a histogram (bins as in §8.4, from the parameters, the column's declared range or, without disclosure settings, *B* equal-width bins between the minimum and maximum; *B* = 10); observation-state counts (each column's `analysed` units excluded by reason: NOT_APPLICABLE, NOT_ASSESSED, NO_INFORMATION and the rest). Parameters: `columns`, 1 to 8 variables (§9.2), each a category (category and boolean columns, `max` and `min` of ordered categories, and `some` and `every`, whose categories are `false` and `true`) or a number (number, integer and time-offset columns, `count`, `mean`, and `max` and `min` of numbers), with `bins` for a number. Descriptive only (D328) |
| `summary.members` | The unit keys of exactly one cohort's members, each its values in key order as they are stored, sorted as step 8 of §7.6 sorts an `ids` leaf's members (RFC 8785 serialisation, UTF-16 code units), paginated with `offset` (0 by default) and `limit` (1 to 1,000, 100 by default), with the key columns and whether more follow. Refused where the dataset allows no row ids and under any disclosure setting (§8.4). Descriptive only; no chart (D331, D332) |
...
| `compare.existence` | Per existence predicate (e.g. *some grade ≥3 adverse event*, *a TP53 mutation*): the proportion per cohort over units for which it is known, with a Wilson score interval without continuity correction; risk difference (Newcombe hybrid score interval) and risk ratio (Katz log interval; see the estimability table for zero numerators) versus the reference; Fisher's exact test for two cohorts, chi-squared for more. Parameters: `predicates`, 1 to 16 clauses asked of every unit of the unit table, without `ids` or `cohort` leaves, and `level`, 0.95 by default; per position and predicate, `lift_differs` (D319, D323) |
...
| **Validator** | `validate_source(source, result) -> [Refusal]`, `source` a `ConfinedPath` or a database snapshot's `DatabaseSource` (D305); `validate_descriptors(release) -> [Refusal]` | at import; on every draft change | the dataset's `packs` |
...
| **Document translator** | `translate(document) -> (aibi document, [{pointer, message}])`, by format `<pack id>.<name>`, on a copy of the document; the core flags every negation of the translation (`not`, `negate`, `!=`, `every`), and what raises or is not a JSON document and notes it can read is `PACK_FAILED` (D303) | `validate_document` with a `format` | the pack named by the format |
...
- The extension points are delivered in M1–M3 alongside the core features that call them, each
  tested with a test-only, non-biomedical pack defined in the core test suite (§15). A pack's
  analyses and requirement predicates come in two steps: registered, listed and matched for
  applicability from M3.1, and run from M3.2d, which materialises the inputs their `requires`
  name; until then a view of a pack's analysis is `NOT_SUPPORTED` (D316, D324).
...
- Unit keys are listed only by the `summary.members` analysis (§9.5), and only without disclosure
  settings (§8.4, D332). The disclosure settings apply to every tool.
...
- The MCP transport is stateless streamable HTTP at `/mcp`, behind request protection (§14); the
  ...
  `propose_descriptor`; M2 adds `validate_document`, `count_cohort` and `explain`; M3 adds
  `list_analyses` and `run_analysis` (D316, D318).
- `validate_document` and `count_cohort` take `{"document": <the document as written>}`, and
  `validate_document` also a `format`; their input schema holds the schema of a document as
  written, and a document loads from its JSON text under a document's rules (D299, D304).
  `validate_document` returns every refusal, the cohorts that canonicalised by name, each with its
  canonical form, its ids and what the derivation log says of them (*not issued* until a count
  issues them), its readback and the caveats of its count that need no data, and each view that
  checked with its canonical form, ids, readback and the caveats that need no data (D299, D317). `count_cohort` counts every cohort of the document in one query worker, which
  ends in time for the call to record and answer within `tool_seconds`, runs the disclosure pass,
  and records, in one transaction and only while the call can still be answered, one issuance per
  cohort naming the call's request (the document as written and the parameters used, stored once)
  and the cohort's SQL as run (D300); it is not read-only. `explain` takes `{"id"}`, a derivation
  or an issuance id, and gives what it resolves to and what the log holds, but when a derivation
  was recorded and an issuance's request, which the operator router alone serves (D302).
  `run_analysis` takes `{"document"}`, checks it as `count_cohort` does, runs every view's
  queries in one query worker, and returns one result envelope per view, recording one issuance
  per result and per cohort its views name; it is not read-only. `list_analyses` takes
  `{"dataset"?, "release"?, "unit"?}` and gives every registry entry and, for a dataset, their
  applicability (D316, D318).
...
- The read-only catalogue page of M1 (`GET /` and `GET /datasets/<id>`) renders what
  `search_catalog` and `describe_dataset` answer for the latest published releases, through the
  same tool calls, and nothing else (D311).
...
- **Erasure.** Honouring an erasure request means re-importing from a source without the
  ...
  names none of their rows, and, for the texts that identify them, on a key; a number only where
  it names their rows, D290) loses
  ...
  **Erasure matching (D290).** A constant goes when it could name one of the person's rows
  (theirs or one below them), or when it quotes one of the person's own identifying texts;
  anything else stays. Where it sits says which rows it could name, and whether its text alone
  counts. A value is read under every datatype it may be of (text, a number, a
  date, an instant in UTC), by the store's cell reader and the engine's constant reader alike;
  a *token* is a term's text as a whole token, or, in free text and wherever every reading
  counts, a date and time or a number in the spellings D290 lists whose value is a term's.
  Text is compared in normalisation form C without format characters, and a number in other
  `units` than its column's as evaluation converts it (§6.4).

  | Where the value is | Place | Matched by | Terms |
  |---|---|---|---|
  | On a key column of one column, or a foreign key of one column, a `covered` scope column included (a column of its `table`) | naming, and quoted | every reading, whole or as a token; the quoted texts by their text alone, as *other* matches | the keys of the person's rows in the table its values name (its own, or the one it points into), JSON numbers and booleans too; quoted, the values that identify the person (their key's and their rows' identifier values that are not numbers by datatype); no other term, so a number on a key of a table that holds none of their rows matches nothing, while their text key does |
  | A unit key; on a part of a key, or of a foreign key, of more columns | naming, and quoted | the key as a whole, part by part by value; each part also as a constant on its own column | the keys of the person's rows in the unit's table; a part alone, only the values that identify the person, quoted (but a part that is a foreign key of one column names the table it points into) |
  | On an identifier column of any table | naming, and other | every reading, whole or as a token; any other term by its text alone | the values that identify the person (their key's and their rows' identifier values that are not numbers by datatype) and the person's values of that column |
  | On a column or unit the core cannot name (a parameter, a value concept, a concept unit, a scope of a table given as a parameter or a concept); a pack leaf as written; anywhere, with `redact_only` | naming | every reading, whole or as a token | every term, JSON numbers and booleans too |
  | On any other known column; a tree over another dataset's release, or a cohort as written over another dataset | other | its text alone: the whole string, or a whole token of it that is no part of a longer number, date, time or range | the person's, and those below them but the numbers by their columns' datatypes; no JSON number or boolean |
  ...
  A parameter referred to from several places is matched as in each of them, and a clause or a
  unit key given as one as in the cohorts that refer to it. A view's constants are in the places
  of the cohorts it names, or, naming none, of the document's.
- **Derivation log.** Two tables. *Derivations*, keyed by derivation id, hold the canonical
  document, the releases and the versions hashed into the id; they are kept while an issuance
  names them, and an erased one for good.
  *Issuances*, keyed by issuance id, record each time `run_analysis` or `count_cohort` produced
  ...
  An issuance's request (the document as written and the parameters used) and its SQL are texts
  stored once each, by digest, so that the cohorts of one call share its request (D300).
  Issuances of counts (whichever tool issued them: `count_cohort`'s, and `run_analysis`'s of a
  view's cohorts and of the units two cohorts share) are pruned after a configured period (`[log]
  keep_count_issuances_days`, 30 days by default), and issuances of results after another
  (`keep_result_issuances_days`, 365 days by default), when the server starts and hourly after,
  in a thread of their own, a batch at a time; an operator prunes every issuance recorded before
  a time, or each kind before its period (`aibi prune`). A kept issuance that names one as the
  source of its values keeps it, and a result keeps the counts of its cohorts its call recorded; a derivation no issuance names then goes with them (D300, D318). What pruning can free (the texts, and the derivations and issuances with their rows),
  counted by the pages its rows and index entries can take, is at most `log_bytes`, past which
  counts are refused (D300). A derivation id is recorded
  only with the object it is the hash of, over the releases its object is over, each under its
  manifest's dataset, and none of them withdrawn when it is issued. The ids of a derivation
  resolve, in this order, to *erased* when erasure took
...
- **App DB (SQLite).** The catalogue index (one entry per dataset, for its latest published
  ...
  content only, §8.1; from M3.5, D318) and the derivation log.
...
| Data | DuckDB (CSV, Parquet, and Postgres, MySQL and DuckDB sources, Postgres and MySQL through DuckDB's own signed scanner extensions, pinned and bundled as wheels, D306; 1.5, and the query engine in worker processes, D293), Python's `sqlite3` (SQLite sources, D306), SQLGlot (30.x, building the query engine's SQL, D292), python-calamine (XLSX, XLS, ODS; `.xls` is refused in v1, D225), Parquet, SQLite |
...
| Frontend | React + TypeScript + Vite, types generated from OpenAPI; renders Vega-Lite with a CSP-safe interpreter, never charts from model text (M5). M1's read-only catalogue page is HTML the server renders, without a script (D311) |
  ...
  server/
    ...
    src/aibi/
      core/
        ...
        api/         # the application: request protection, configuration, refusals over HTTP, the catalogue page, aibi-server
        mcp/
...
- **Databases:** Postgres, MySQL, SQLite and DuckDB sources, through named connections from
  server configuration (§14, D305). All tables are read in one consistent snapshot (one
  transaction) so that keys stay consistent, in the import's worker process (D306). Declared
  primary and foreign keys and table and column comments are imported with status `imported`
  (comments as `definition`s), and so are the column types DuckDB reads (SQLite's cells keep
  types of their own); a declared key the rows break stops the import, as §13.2 has it for
  declared keys (D307). Only base tables are read: views, materialized views, virtual and foreign
  tables are skipped and noted.
...
- **Request protection.** One middleware protects every router (the HTTP API, the MCP transport
  ...
  site (`Sec-Fetch-Site`) needing an allowed Origin (D257), but a top-level navigation to the
  catalogue page that the person started (D314); CORS disabled unless configured, and
...
- **Untrusted text (A6).**
  ...
  - The catalogue page (M1) has no script: every text of an output is an escaped HTML text node,
    its control and format characters written as marked, visible escapes, text from data
    isolated in a `<bdi>`, and its policy allows its one stylesheet by hash, and images and
    connections to the server only; every answer at its paths is a page, none is stored by the
    browser, and a page that opened its window loses its handle (D311–D314).
...
- **Imports.**
  ...
  - From SQLite and DuckDB files only base tables are read, never views or triggers. SQLite files
    are read with SQLite's own reader, opened read-only and defensive, with views, triggers and
    extensions disabled by SQLite itself (D306).
  - DuckDB runs with external access disabled except for allowed directories or files (for
    ...
    (D293), and so does a DuckDB file's snapshot, allowed that file alone (D306). A Postgres or
    MySQL snapshot runs instead in its own importer worker, whose DuckDB connection loads only the
    scanner of its kind, attaches that one named connection read-only, then disables external
    access and locks its configuration, and runs only the snapshot's generated statements (DuckDB
    refuses such attachments once external access is disabled, and enabling it lifts all file
    confinement); the query engine's configuration never changes.
  - Database sources are named connections in server configuration; a request can name a
    ...
    releases, logs, refusals or results; provenance records host, database and schema only
    (D305).
...
- **Resource limits.** Request bodies, strings, lists (at most 10,000 members in a `values` or
  ...
  of rows (`query_answer_bytes`, D293). DuckDB's memory grows with its threads, so
  `query_memory` should grow with `query_threads`: the 2 GiB default runs `run_analysis` over a
  million units, 16 predicates and 6 cohorts at up to 4 threads (D318). A query tool resolves and canonicalises its document in
  its tool call's thread, under the call's `tool_seconds`, bounded by the document's limits, and
  runs only its queries in a worker, which ends early enough for the call to record its
  issuances and answer in time (D301). The derivation log, which unauthenticated clients add to,
  grows with each `count_cohort` call's document, not with its cohorts, is pruned by age, a batch
  at a time and never in a tool call, and is bounded by `log_bytes`, which counts all that
  pruning can free (D300). The catalogue's tools (M1) read no rows, so a release built before its
  catalogue statistics were kept is refused rather than counted (D270); they run in worker threads
  ...
  upload's deadlines with a shorter idle time (`tool_body_idle_seconds`, D266, D278). Categorical levels per analysis (at most 150), unit keys per page of
  `summary.members` (at most 1,000, the whole cohort's keys read and ordered on the server within
  the answer cap and the call's deadline, D333) and resampling replicates are capped. A request body has at most 8 MiB by default (`request_bytes`), and an upload at most
...
| **M3** | Registry and the core analyses of §9.5, with aggregates (added to the reference evaluator), estimability checks, determinism and the remaining disclosure rules; `list_analyses`; `applicable_analyses`; `run_analysis`; charts; extension points: analyses and requirement predicates. In slices (D315): M3.1 the registry, applicability, phase 2, `run_analysis`, `compare.existence` and its chart; M3.2 in four parts (D324): M3.2a variables, aggregates and `summary.distribution`, M3.2b `summary.members`, M3.2c `compare.columns` and per-category denominators, M3.2d running packs' analyses; M3.3 survival; M3.4 the thread-count determinism tests; M3.5 the result cache | Golden, R-reference, determinism and disclosure tests pass, including the reference-swap test |
...
| D289 | The derivation log (M2) | The app DB's `derivations` table holds, by id, the kind (`cohort` or `result`), the object the id hashes in RFC 8785 form and the releases; `derivation_releases` lists a derivation's datasets; `issuances` holds, by `iss:` and a ULID (48 bits of milliseconds and 80 random bits), the derivation, the tool, the document as written, the parameters, the SQL as run (none for a cache hit, which names an issuance of the same derivation whose SQL ran), the engine and pack versions and the time. A derivation is recorded only with the object its id is the hash of, and of the kind its object is (a cohort's names a `cohort`, a result's a `view`), once, with at least one release, each dataset once and each under the dataset the store records its manifest for (erasure finds a derivation by its releases' datasets), and a cohort's exactly the manifests its object's `cohort` is over; a list holding anything but releases is refused whole. A derivation any of whose releases is withdrawn is neither recorded nor issued (`WithdrawnReleaseError`), as the issuing transaction sees the releases, so an output computed before an erasure and issued after it records nothing of the person. Triggers hold these rules against every write, `INSERT OR REPLACE` included: a derivation is inserted once and with its object, is removed only by pruning once no issuance names it and never once erased (D300), and changes only once, when erasure takes its object; its `derivation_releases` are the releases it lists, each dataset once, of its manifest's dataset, none withdrawn, never change, and go only with it; an issuance is inserted once, only for a derivation whose object is kept and none of whose releases is withdrawn, and a cache hit only naming an issuance of its derivation whose SQL ran; it changes only by redaction (its document, parameters and SQL, each to JSON that holds `[erased]`; the table's check keeps the SQL from going to or from none), and is removed only once its derivation is erased, or, for `count_cohort`'s, by pruning, never one a kept issuance names. Taking an object, changing an issuance and removing one or a derivation each need a permit in `log_permits`: redaction writes `redaction`'s, and pruning `pruning`'s with the time it prunes before, first within its transaction and removes it last, so no permit is ever committed (redaction refuses to run outside a transaction, where its permit would be); a permit is one of each kind and never changes. The triggers guard the log against the store's own code and its mistakes, not against a writer with SQL access to the app DB, who could write a permit or drop a trigger; that limit is accepted, the app DB being the server's own file. `count_cohort`'s issuances are pruned by time, but those a kept issuance names; `prune` takes an RFC 3339 time with its offset (`T` or `t`, `Z`, `z` or `±HH:MM`, a four-digit year, a fraction to the microsecond), refusing anything else (ISO 8601's basic and week forms, an offset of hours alone, an instant beyond year 9999 in UTC) with a `ValueError`, and compares the instant, written as the store's clock writes times, the year always in four digits. Issuance ids only ever increase, by their time, in the order they are recorded, across restarts too, each with 80 fresh random bits (D302), and a derivation's issuances are listed by id; an issuance's document as written with its parameters, and its SQL, are texts stored once by digest (D300). An id resolves, in this order, to *erased*, *withdrawn* (a release withdrawn), *discarded* (a release a draft state no longer live), *unknown release* (a release the store has no record of) or *issued*; an unknown derivation id is *not issued*, an unknown issuance id *unknown* | §12.2 names the tables and what they hold; recording only verified objects makes `explain` exact, and the order of the statuses puts first what the id can no longer show; the releases are what erasure finds a derivation by, so they must be its object's, and an issuance after a withdrawal would bring back what an erasure took; permits make a stray `UPDATE` or `DELETE` fail, which checking the shape of a write alone does not; times compared as text in two spellings order wrongly |
| D290 | Erasure of the derivation log (M2) | Erasure takes the object of every derivation over one of the dataset's releases whose object holds a term as data, and deletes its issuances. A cohort's data are the constants of its canonical clause trees: `values`, range bounds, `scope` values and unit keys' parts; a result's are its canonical parameters: a clause among them (a predicate or a covariate, canonicalised as in phase 1, §7.6) by a cohort's rules alone, its unit not known, never again as strings, and any other string as a string constant. Versions, the disclosure setting, descriptor ids, paths, counts such as `min_count` and a result's numbers outside clauses (an analysis's settings) are never matched. Values are read under every datatype they may be of (as text; as a number, written as RFC 8785 writes it; as a date; as an instant, in UTC), by the store's cell reader and the engine's constant reader alike (§6.4), so that every spelling either accepts has its value: a datetime is its instant in every spelling (the canonical form's `Z`, a cell's `+00:00`, a client's `+01:00`, `t` and `z`, a fraction padded with zeros beyond the microsecond), and `2.0` or `2e0` is the number 2. How a constant is matched follows one principle: a constant goes when it could name one of the person's rows (theirs or one below them), or when it quotes one of the person's own identifying texts; anything else stays; where it sits says which rows it could name, and whether its text alone counts, as the table in §12.2 (Erasure matching) sets out: (1) on a key column of one column, or a foreign key of one column (a `covered` clause's scope names a column of its `table` by its name alone, so it is that table's column): the keys of the person's rows, in any release, in the table its values name (its own, or the one it points into), by every reading, whole or as a token, JSON numbers and booleans too, and, quoted, the values that identify the person (as in (3): their key's and their rows' identifier values that are not numbers by datatype) by their text alone, as in (5), and no other term, so that a number on a key of a table that holds none of their rows matches nothing, and a number on any key only a key of theirs in the table it names (the member numbered 3 is not the person's loan 3, nor the loan numbered 17 the member 17), while the person's key and name go wherever they are quoted (`loans.book_id = "m-17"`, `loans.member_id = "Grace"`), and so does another table's key written as the person's text key (the book `b-2`, where the member is `b-2`); (2) a unit key: as a whole, part by part by value, against the keys of the person's rows in its unit's table, every part being erased when they match, and each part also as a constant on its own column; a column that is only a part of a key, or of a foreign key, of more columns names rows only through the whole key, and by itself holds only the values that identify the person, quoted as in (1), but a part that is a foreign key of one column still names the table it points into (so the visit `["m-17", 9]` goes, while `["m-1", 1]` and a visit's `seq = 1` stay); (3) on an identifier column (§5.4) of any table: the values that identify the person, those of their key and of their rows' identifier columns that are not numbers by their columns' datatypes, wherever they appear, and the person's values of that same column, numbers too, by every reading, whole or as a token, and every other term by its text alone as in (5) (a room named `Grace` for the person goes, a number of days only on its own column); (4) on a column or unit the core cannot name (one given as a parameter, a value concept or a concept unit, from M6, whose column or table each dataset maps, a scope of a `covered` table given as a parameter or a concept, a unit not known to be a table), in a pack leaf as written, and anywhere with `redact_only`, which knows no release: every term by every reading, JSON numbers and booleans too; these hold only in the tree over the dataset's release, or in a cohort or a view as written that may be over the dataset; (5) *other*: on any other known column, and anything in a tree over another dataset's release or in a cohort as written over another dataset (its `dataset`, every one of its `datasets`, or else the document's, names another; one given as a parameter, `datasets` included, may be over the dataset), or in a view over such cohorts only (those its `cohorts` and `reference` name, or, naming none, every cohort of the document; one it names by a parameter, or that no cohort has, may be over the dataset): a string by its text alone, the whole of it or a whole token of it that is no part of a longer number, date, time or range (a term starting or ending with a digit has no sign before it, no `.`, `,`, `:`, `/` or `-` joining it to another digit on either side, and no time of day, ` HH:MM`, after it), so `07`, `7.0`, `+7`, `6-7` and `7-8` do not hold the text key `7` there, nor `2024-01-01T00:00:00Z` or `2024-01-01 10:00` the date `2024-01-01`, while `2024-01-01` does and so does `for m-17 only`; never a JSON number or boolean, nor a number below the person; (6) *unknown*: free text (`notes`, `note`, `drafted_by`), structure, a parameter no clause refers to, a result's strings: by every reading and as tokens, but never a JSON number or boolean, nor a term below the person that is a number by its column's datatype (`"2"` names no loan there, `"104233"` is a member whatever it reads as). A number with `units` other than its column's is read only as evaluation reads it, converted to the column's units by one multiplication of doubles by the factor rounded to a double (§6.4), so `72` hours is the person's loan of 3 days and `3` hours is not. Text is compared, terms and constants alike, in Unicode normalisation form C and without format characters (category Cf: zero-width spaces and joiners, the soft hyphen, bidirectional marks), so no invisible character or other encoding of an accent keeps a term; text that holds no term is kept as written, and text that holds one is redacted in that form. A parameter referred to from several places is matched as in each of them, a term any of them holds being erased; an object key anywhere is matched whole by every reading against every term, as the audit trail's values are (D223), two keys redacted alike being kept apart as `[erased] (2)`, `[erased] (3)` and so on; a name a document gives (a cohort's or a parameter's, which starts with a letter or `_`, so only a text term can be one) is such a key, and every reference to it (a view's `cohorts` and `reference`, a `cohort` leaf, `"$name"`) is redacted as the name is, so each still names what it named: §7.6 keeps names out of ids, but a name, like a note, may quote the person. A token, in free text and in a string matched by every reading, is: the term as written, next to no letter, digit or mark of any script; a date and time (`T`, `t` or a space between them; to the minute or the second; a fraction after `.` or `,` of any length, truncated to the microsecond as a cell's is; `Z`, `z`, `UTC`, `GMT`, `±HH:MM`, `±HHMM`, `±HH` or none, after a space or not) whose instant, read at its offset or, the offset aside, as UTC, is a term's; and a number (a sign, digits, a fraction, an exponent: `0017`, `+17`, `17.0`, `1.7e1`) that no `.`, sign or `-` joins to more text, nor a `,`, `:` or `/` to another digit, whose value is one of the person's terms' (so, for the integer key 17, `017` in `member 017: Grace` is one, while `017` in `10:017` or `3/017` is none; the key's own text `17` is still a token of text there, so free text loses it from `10:17` and `3/17`, and *other* keeps it, since by text alone a term is no part of a range or a time); digits grouped by separators (`1,017`) are read as no number, neither the whole nor a group, and a number below the person by its datatype is never a token; a number in text is parsed only when its significant digits are those of one of the person's terms, or more than a double's 15, so a note of many numbers costs a lookup for each. Every other issuance that names the dataset, through its derivation or in its document as written (its `dataset`, a cohort's `dataset` or `datasets`) or its parameters (those given with the issuance or the document's own `params`; one, or one in a list), has its document, parameters and SQL redacted by the same rules: each constant in its place, everything else (`notes`, structure, the SQL's text and strings) as *unknown*; a number only as a naming constant, a parameter as the places that refer to it (`"$name"`) are, a clause among them as a clause and a parameter that stands for an `ids` list or a member of it as inline unit keys are (`"<dataset>:<key>"` split at the first `:`), each as in the cohorts and views that refer to it (over the dataset when one of them may be, or when none refers to it), and a pack leaf as written, whose columns the core cannot know, losing every constant that is a term as a naming place's, JSON numbers and booleans too (in a cohort over another dataset, as an other place's); never `aibi`, a count, a quantifier, a bound on another column or an analysis's setting; the SQL's numbers, which are its derivation's constants (which held no term) or structure, are kept. The log is read a batch at a time: the dataset's derivations in id order, and the issuances, those of its derivations and those whose text holds its id chosen in SQL. Terms are found in text by its tokens (runs of letters, digits and marks, and each other character) looked up in sets, so that no pattern of them is compiled and their number costs a lookup, not a step of every match. Erasure is one pass over what the log holds when it runs: an issuance canonicalised before it and recorded after it is checked only as every issuance is (a derivation over a withdrawn release is refused, D289), so the query tools must record an issuance only if no erasure of a dataset it names was recorded since they canonicalised it; checking later issuances against the terms would mean keeping the person's data | §12.2: a partial redaction next to a hash can be reversed by enumerating keys, so the whole object goes, and it cannot be restored, so it must go only when it holds the person: a loan numbered 1 below them names no one as `semantics_version` 1, `min_count` 2 or an age of 3 does, while a string is never a count, an age or a version, so a key that reads as a number stays a key; an issuance of a kept derivation is still a document, whose structure names no one, and redaction is as irreversible there; issuances of other derivations may still quote the person in notes or parameters; a value is identifying whatever its spelling, and each spelling matched as text left one behind (a text key that read as a number, an instant the canonical form writes with `Z` and a cell with `+00:00`, a key split inline but not through a parameter), so values are compared, not their text; but a value's readings erase too much where it names no one, since a date or a number is a period's or a range's bound as often as a person's value, and an erased object cannot be restored, so on a column known to name none of the person's rows only a quote of a term's text counts, and every reading only where the value may name them (review round 4 found both directions); and a value on a naming place is a key of the rows of its tables, so a term of another table there names another row, while every value that names a row of the person's there is a term of that row's table, which gave it (review round 6); and each such rule, patched in one direction, opened the other (a room named for the person kept, a visit's shared number erased, a number of hours kept, another table's key spelled like the person's erased), so the rules are derived from one principle, what a constant could name, and each place from what its values are keys or identifying values of (review round 7); and the principle counts quotes as well as names: a number on a key is a value, which names rows of the table the key names and no other, but a text that identifies the person quotes them wherever it is written, so on a key too their key and name go by their text (`loans.book_id = "m-17"` and `loans.member_id = "Grace"` kept both in the app DB), and another table's text key written as the person's goes with them, the collateral D223 already accepts of equal values; a number by datatype quotes no one, so another table's number stays; and a number given through a parameter is read only as the places that refer to it read it, never also unconverted (review round 8) |
...
| D296 | Readbacks (M2) | `engine/readback.readback` renders a canonical cohort from templates as segments, in the canonical order of step 8, from the canonical form and the release's descriptors alone: *Rows of* the unit table's label *in* the dataset's label *@*label, *for which* its clauses joined by *and* (an `any` by *or*, parenthesised when nested), or *every row* for an empty cohort; a value leaf as *the* column label, each lookup (*of the* parent table's label *row reached through* the relationship's label, the last lookup first), *has some item that* / *has only items that each* for lists, then *is*, *is not*, *is one of*, *is none of*, or a range's bounds (*greater than*, *at least*, *less than*, *at most*, negated as *is not (…)*), and *(in* units*)* for every numeric constant; an existence question as *some*, *at least k* or *every*, the child table's label, *linked through* the relationship, its lookups, *other than the row itself*, the coverage in brackets (*every row's … rows are recorded*; *recorded only for the rows the coverage lists, by* its scope columns; *undeclared, so no row is known to have no further … rows*; *proposed, not confirmed*; its record filter; its parent scope rendered on the parent table), the lift rule of an intermediate question, and *such that* its `where`; `covered`, `ids`, `not`, `known` and `unknown` likewise; then one sentence on what is unknown and not counted, and the pack summaries, each labelled with the leaf keys of the conditions its leaf became part of (*in the condition* …, or *which adds no condition*), ordered by those keys and then by the summary's text, each once, never by the leaf's pointer as written. Every label, id, constant, unit and key is a data token, each part of a composite key its own (in parentheses) and the release's label too; at most 64 members of a list are shown, then how many more | §7.7 lists what a readback states; rendering from the canonical form in its canonical order gives equivalent documents one readback, as they have one id, and a readback that no model writes can be confirmed against the document it came from (A2); data tokens keep labels, which curators and importers write, out of the template text (A6); a pointer as written holds the cohort's name and the document's layout, so labelling a summary by it would give one canonical form two readbacks, and parts of a key joined into one token would read `["a, b"]` and `["a", "b"]` alike; a pack summary is rendered from the leaf as written (§7.3), so its words are the one part two equivalent documents may give differently |
| D297 | Count suppression (M2; refines D288) | `engine/suppression.disclosed(parts, k)` runs §8.4's pass on a cohort count's digested parts. The unit table's size is the total of the linked set `n_true`, `n_false` and `n_unknown`, and a member of it: from 1 to *k* − 1 it is suppressed (`size.denominator`), with all three counts, as the catalogue suppresses that table's `n_rows` (D271); a table of fewer than max(*k*, 4) units shows none of the three, zeros included. Over a larger table, until nothing changes (`suppressed`): a count from 1 to *k* − 1 is suppressed, and so is a non-zero count whose complement (the table's size less it) is; one suppressed alone takes another with it, `n_true` and `n_false` each other (`n_unknown` when the other is 0) and `n_unknown` the smaller non-zero of `n_true` and `n_false`, both on a tie; and two suppressed beside a non-zero shown count take it too when those rules suppress them, and show it, for only one split of their sum into two non-zero counts (which need be tried only where one part is under *k* or over the size less *k*; two that would each be 1 are one such case, so no rule of their own is needed). The rules treat `n_true` and `n_false` alike, so `not X` is disclosed as `X` swapped, and every complement is a count of `not X`, `known X` (`n_true` + `n_false`, `n_unknown`, 0) or `unknown X` over the same table, which a writer of `X` can ask for in the same call. `lift_differs` is a set alone, and is suppressed under every *k*, a 0 included: the units it counts lie on one side of the cohort (a strict cohort of `some` steps turns only unknown units definite when its lifts flip, so its `lift_differs` is at most its `n_unknown` and may equal it; one whose lifts turn definite units unknown bounds it by `n_true` or `n_false`), and `not X`, `known X` and `unknown X` count the same units, that side among them, beside counts that may all be large, so a `lift_differs` of theirs shown would bound a suppressed count of `X` by the size less it (22 establishments, one with a pest violation on its one inspection and 21 with an assessed and an unassessed inspection each, under `lift` `assessed`: `known X`, 22, 0 and 0, showed a `lift_differs` of 21, which gave back `X`'s suppressed `n_true` of 1); `unknown_by_reason` and `unknown_by_leaf` are `null` with `n_unknown` or when one of their counts is from 1 to *k* − 1; the size's estimate is suppressed with its numerator or its denominator; the count then carries `SUPPRESSED`, as every count under *k* does, `UNKNOWN_EXCLUDED` whenever `n_unknown` is suppressed and `LIFT_DIFFERS` whenever `lift_differs` is, so under every *k* (a 0 included, with messages that say nothing of whether a unit is unknown or would change); `UNKNOWN_EXCLUDED` and `LIFT_DIFFERS` are worded again without the counts suppressed. The model of a cohort count accepts a suppressed denominator only under *k* and with all three counts, and requires a table under max(*k*, 4) to show none of them; any output's population suppresses `lift_differs` under a *k* (amending D200, whose check of a suppressed one against a size of at least 1 goes); of two suppressed beside a shown count it bounds their sum only when `n_unknown` is one of them, by the larger of 2(*k* − 1) and *k* − 2 plus the shown count (amending D200's check, which assumed the smallest other and a listed-order tie). Exhaustive tests check that what the pass shows (counts, size, breakdowns, caveats, messages included) leaves every suppressed count two values or more: for every table of up to 12 units and *k* from 2 to 6, the size under *k* = 2 aside, which a suppression pins to 1 as it pins any count there; for every table of up to 6 units and *k* from 2 to 5 with `lift_differs` at 0, 1, *k* − 1, *k* and *k* + 1 and without or with the flags' caveats, a `lift_differs` pinned only to a 0 the counts bound it to; for every table of up to 6 units and *k* from 2 to 5 with `lift_differs` at every value up to `n_unknown`, and again up to `n_true` + `n_false`, for a writer who knows which bound holds, a `lift_differs` pinned only to a 0 the counts shown bound it to; for `X`, `not X`, `known X` and `unknown X` counted together over every table of up to 40 units, for *k* from 3 to 6, while under *k* = 2 they pin only an `n_unknown` of 1, or an `n_true` and an `n_false` of 1 each, and what those leave of the size; and for the four counted together over every table of up to 12 units, for *k* from 3 to 5, each with a `lift_differs` of the same units, drawn from one side of `X` the writer knows, and so bounded by the side of each that can change, a `lift_differs` pinned only to a 0 that side bounds it to. One leaf and one reason stand for any: the breakdowns count units of `n_unknown` alone and are shown only beside it. Complements within a breakdown are not linked: `n_unknown` less one leaf's or one reason's count, which may be small, is a difference of two counts shown, which is differencing, as §8.4's Limits disclaim. The cohorts of one output are otherwise disclosed each alone, so a count, and its digest, do not depend on what else its call counts: two cohorts of one call that differ by a few units other than by `not`, `known` and `unknown` difference to a small count, and a two-valued collapse of a cohort, `X and known X` (`n_true` beside `n_false` + `n_unknown`) or `X or unknown X` (`n_true` + `n_unknown` beside `n_false`), is another cohort whose counts, shown when they are large, give back what `X`'s pass suppressed; no pass over one cohort's count can see either, and §8.4's Limits disclaim both. A count over an empty unit table carries `NOT_ESTIMABLE` (its size has no estimate, `no_units`). The digest is taken after the pass, and so are the golden count digests: the one golden case under a floor now hashes what `count_cohort` serves (no count was ever served with the digest before the pass), which over its unit table of 4 units under a floor of 5 is every count suppressed, the size and `lift_differs` included (the latter since review round 3; no count had been served from a release), so suppressing `lift_differs` under every *k* (review round 4) moved no golden digest | §8.4's linked sets for cohort counts; a small table's size beside two zeros is the count the pass suppressed, and it is the `n_rows` the catalogue hides; zeros shown bound suppressed counts from below, so a small total pins them unless all are hidden, and the rule for one suppressed bounds the second, so a split at its bounds pins both, which one more suppression undoes; `known X` shows `n_true` + `n_false` whenever it is large, so a small `n_true` hidden beside a shown `n_false` would be read off it, and a listed-order tie hid the count `not X` shows; a `lift_differs` shown beside a suppressed `n_unknown` it equalled gave that count back, and the small count beside it, and `known X`'s, shown beside counts all large, gave back the count of `X` that the size less it left; making the pass closed under `not`, `known` and `unknown` keeps each count a function of its cohort alone, and protects a cohort's counts from those three cohorts only, not from others of the same call, a two-valued collapse included, while suppression across other cohorts would need their overlaps, which a count does not compute, and would still not stop differencing across calls; §7.6 digests outputs as served; the model of a cohort count refuses one without `NOT_ESTIMABLE` for a not-estimable size (§8.2), so an empty table could not otherwise be counted |
| D298 | Outputs check their digests (M2) | The hashing, rounding and digests are `schema/digests.py`'s (`engine/ids.py` names them too); a `CohortCount` or a `ResultEnvelope` whose `digest` is not the digest of the digested members it carries, as it carries them, cannot be built (`digest`) | #15's open question: a wrong digest is then unbuildable rather than caught by a test; rendered text stays outside it, so readbacks and labels can change freely |
| D299 | `validate_document` (M2; refined by D317) | Takes `{"document", "format"?}`; loads the document from its JSON text as a document as written (§7.1), resolves each dataset reference (the document's `dataset` and each cohort's) as the catalogue resolves a pin, refusing where the reference is written, listing the published labels or datasets, a withdrawn release (`RELEASE_WITHDRAWN`), a discarded draft, an unknown label or manifest (`UNKNOWN_RELEASE`) and an unknown dataset (`UNKNOWN_DATASET`); pins the releases and reads them in outline; canonicalises; and returns `{valid, refusals, translation?, cohorts, views, params?}`: every refusal (§8.6), each cohort that canonicalised, by name as UTF-16 code units, with its name as data, its ids, the status the derivation log gives its id (`not_issued` until `count_cohort` issues it), its canonical form, unit, release, effective disclosure, packs, leaf map, readback and the caveats of its count that need no data (`UNCONFIRMED_SEMANTICS`, `DRAFT_RELEASE`, the caveat rules' codes, affecting `/population` as the count will carry them), the views unchecked (D284), and the parameters used and unused. The document's refusals point into the document as written; the request's own into the request | §11.1's `validate_document`, which reads no row; JSON object key order is never used (§7.4), so cohorts go by name; the log's status tells a client whether an id is already citable |
| D300 | `count_cohort` (M2; refined by D318) | Takes `{"document"}`, loads and canonicalises it as D299 does, and fails with the first refusal; compiles every cohort and runs the document's queries in one query worker (`Workers` of the catalogue, from `[queries]`), which ends `RECORD_SECONDS` (2 s) before the call's deadline (D301), a worker's refusal refusing the call (`LIMIT_EXCEEDED` naming its limit) and a fault of the compiler or the engine being `INTERNAL_ERROR`; makes each cohort's count (`count_parts`, the disclosure pass under the effective *k*, the digest after it, the readback); and records, while the releases are still pinned, one issuance per cohort, all in one transaction, each naming the call's request (the document as written and the parameters used) and the cohort's SQL as run (`{"statements", "parameters"}`, blobs as their digests), with `aibi <version>` and the packs' versions. The log stores each request and each SQL text once, under the SHA-256 of its RFC 8785 text (`log_texts`, migration 5, which moves earlier issuances' texts there), so the cohorts of a call share one request and a count made again adds its issuances alone: what a call adds grows with its document (its request, and each constant once in its cohort's SQL and once in its derivation's object), not with its cohorts, but that a cohort others reference is inlined into each of their forms and SQL (at most six cohorts). The transaction commits only while `ANSWER_SECONDS` (1 s) of the deadline are left, so that a call that will not be answered records nothing; a release withdrawn meanwhile refuses the whole call (`RELEASE_WITHDRAWN`) and records nothing, and so does an erasure of a dataset one of its issuances names (through its derivation, or in the document as written or its parameters, as D290 has it) recorded since the call took the audit trail's greatest erasure before it canonicalised (`erasure_mark`, checked in the recording transaction), as D290 requires, since an erasure passes over the log once and a release it leaves alone may still be counted; and what would take the log past `log_bytes` refuses it (`LIMIT_EXCEEDED` naming `log_bytes`). The issuances' ids are drawn in that transaction, with their times, and the counts that name them are made after it. The call's pins, and `validate_document`'s, are released without waiting on the store's lock (`Store.pin(background=True)`): what their release frees is swept, and the redactions they held off run, with the vacuum after them, in a housekeeping thread of the store's, never in the call's, so that a vacuum, which rebuilds the whole app DB under the store's lock (D223), never delays the answer of a call that recorded; a pin that thread drops a little later only keeps its blobs and holds off a redaction a little longer, and an erasure first waits for that thread, so that the pins released before it began count as released. `[log]` in the configuration holds `keep_count_issuances_days` (30 by default, 0 never, at most 36,500, which the store's clock can still count back) and `log_bytes` (8 GiB by default, at least 1 MiB). `log_bytes` bounds what pruning can free: every text, every derivation not erased with its object and its list of releases, and every issuance, each counted by the pages it can take in the app DB's pages of 4,096 bytes, which it sets on a new app DB and requires of one it opens (`aibi-server serve` exits with 1 and the store's message on one of another page size), since `PRAGMA page_size` changes only a database not yet written (`appdb.LOG_USAGE`, which triggers keep in `log_usage`): a text's or a derivation's row, appended in rowid order, is charged its cell twice, since the next row opens a page when it does not fit and so leaves less than its own cell of the page before empty, however rows of different sizes alternate, and its overflow pages whole (`stored_sql`); a text adds 192 bytes for its digest's entry in the index of random digests, which leaves its pages half full or more, a derivation 256 for its id's and 4 for each byte of its list of releases, for their rows and entries, and an issuance is its packs' text and 1,024 bytes, for its row and its six indexes' entries; so the pages the log adds as it grows are at most what it counts, and about half of it or more; an erased derivation, which only erasure makes and pruning never removes, is not counted. `count_cohort`'s issuances that expire are pruned in a thread of the log's own, never in a tool call's: up to 10 batches as the store opens, the rest at once, and then hourly; and an operator prunes before a given time or the configured period (`POST /operator/log/prune`, `aibi prune [--before]`; a time outside UTC's years 1 to 9999 is `INVALID_VALUE`). Each transaction of a pruning removes at most 1,000 issuances, the newest first, and the derivations and texts they leave unnamed (amending D289, whose derivations were kept for good; migration 5 removes those that no issuance named before it), so pruning before now frees all that `count_cohort` recorded; redaction removes the texts no issuance names. It returns `{counts, views, params}`, each count beside its cohort's name (as data) and its leaf map, by name, and is not read-only, since every call records. A document that holds a token's or a handle's shape, or the curator token, is refused before it is loaded, as other text the server keeps is (D265). Counts are not cached in M2: each is its own issuance, `values_from` itself. A server on a system without `/proc`, where no worker's memory can be watched (D293), runs no query workers, and `count_cohort` refuses as `NOT_SUPPORTED` while the rest is served. A draft's effective *k* is at least its dataset's latest published release's (as D275 has it for the catalogue), passed to canonicalisation as a floor of the draft's own (`floors`) | §8.1's cohort counts and §12.2's issuances; one worker run bounds a document's queries together (§14); unauthenticated clients add to a log that only erasure and pruning shrink, so what one call adds is bounded by its document and all of it by `log_bytes`, a request copied into each cohort's issuance multiplied it by the cohorts; counting what pruning cannot free let a log filled with distinct cohorts refuse counts for good, and counting no issuance's row let counts made again grow it without bound; counting a row's bytes let texts of just over half a page, one to a page, take 1.7 times what was counted, and a longer period than a century overflowed the store's clock as it opened; pruning 100,000 issuances in one transaction held the app DB for seconds, in the call that happened to record; a call answered `tool_seconds` while its thread went on would otherwise leave issuances no one received, and a due vacuum run by the release of its pins, after its issuances committed, did so, and a withdrawal between two cohorts' issuances would leave half a call; a session must not lower *k* before it publishes; a result cache for counts would add nothing M2 needs |
| D301 | Where the query tools run (M2; amends D285) | A query tool loads, resolves and canonicalises its document, pack compilers, summaries, caveat rules and translators included, in its tool call's thread, under the call's `tool_seconds` (the call is answered `LIMIT_EXCEEDED` past it, its thread finishing in the background with its place, a count recording nothing then, D300); only its queries run in a killable worker (D293), whose wait and run end by the call's deadline less `RECORD_SECONDS` (`Workers.run(…, ends=)`), its child killed; the worker then raises `CallerDeadline`, naming no limit of its own, and the call is answered naming `tool_seconds`, the thread learning the deadline from `mcp.calls` (`catalog.service.DEADLINE`). The core's own work outside the worker is bounded by a document's limits (§7.1, §14: 2 MiB of JSON, 200,000 values, six cohorts, 64 leaves each, 256 pack leaves each and their step budget), about 5 s on a development machine for a document at its value limit, well within the default 30 s; pack code is trusted and has no budget of its own. This reverses D285's plan to run canonicalisation in §14's workers in M2 | §14 requires killable workers for what data drives, the queries, whose cost depends on the rows; canonicalisation's cost depends on the document alone, which its limits bound, and running it in a process of its own would load the core and the packs in every worker and send it the release's descriptors, for no bound it lacks; a worker given a full `query_seconds` of its own could outlive a call already answered |
| D302 | `explain` (M2) | Takes `{"id"}`, a `drv:` or an `iss:` id, and gives `{id, status, derivation?, issuance?}`: the status `Store.explain` gives (D289), the derivation's kind, the object its id hashes (absent once erased) and its releases, never when it was recorded; for an issuance its derivation, tool, SQL as run (absent for a cache hit), `values_from`, engine, packs and time, never its request (the document as written and the parameters), which the operator router alone serves (`POST /operator/log/issuance`, the id in the body, so that no URL or access log holds it; `aibi issuance`); every record is data. Issuance ids hold 80 fresh random bits each, so one cannot be reached from another, and increase by their time alone, the millisecond after the last one's when the clock has not moved on, drawn as the issuance is recorded and seeded, when the log opens, with the greatest it holds, so that they increase across restarts and a derivation's issuances are listed by id (amends D289's monotonic generation). Whether a cohort was counted within the retention period stays visible to whoever can write it, through its id's status, but not when (§8.4 Limits): `not_issued` means not in the log, never counted or pruned since. It reads the log alone, so it answers after any cache is cleared or the server restarts | §11.1's `explain` and §13.4's provenance test; what the log holds is what a client may cite, withdrawn and erased ids included; a request holds the whole document (other cohorts, notes, `drafted_by`, parameters), which a client citing one cohort need not see; with no access control (§1.2) an unguessable id is the only capability, and monotonic ids made several at a time by one call were each the last plus one; the status is what makes an id citable, while the time it was first counted serves no client and tells when someone asked |
| D303 | Document translators (M2) | `validate_document` with a `format` finds its pack's translator (`NOT_SUPPORTED`, listing the installed formats, when none), calls it on a deep copy of the document, and refuses `PACK_FAILED`, with a message of the core's, whatever it raises but a `MemoryError` (logging the pack and the exception's type only), and a result that is not a JSON document and at most 1,000 notes, each a pointer into the document given and at most 64 segments; the core adds a note, pointing into the translation, at every negation of its cohorts, each `not` clause, `negate: true`, `op: "!="` and `every` quantifier (in a list too, and among a pack leaf's members), sorted by pointer, since whether its meaning changes under three-valued logic depends on the source's semantics, which the core cannot know. The translation, its notes and the core's are returned, and the refusals point into the translation. `count_cohort` takes aibi documents only | §7.1 and §10.1: translation goes through `validate_document`, which flags every negation; each form keeps a unit whose answer is unknown out of both a question and its negation (`every` asks that no child fail, §6.4), so flagging each is the only rule that misses none; a pack's failure refuses the translation, not the call, and its message, which may quote the document, reaches no one (as D285) |
| D304 | Documents in tool requests (M2) | A tool's `document` member is any JSON object in the request, whose body is read under a request's rules (so a document nests at most 63 deep inside it); it is then written back as JSON text and loaded under a document's rules and limits. The request's schema, as MCP and the HTTP API give it, puts the schema of a document as written in `count_cohort`'s `document` (its definitions joining the request's); `validate_document`'s is any object, since a `format` may name another | A client writes the document and the tool reads it as written (§11.1); loading it from its text applies every rule of §7.1 in one place; the schema tells a client what a document is before it writes one |
| D305 | Named connections as import sources (M1) | An import or re-import names a source `{"connection": "<name>"}`, an identifier of `[databases.<name>]` and nothing else (`INVALID_VALUE`, listing the configured names, for another); a connection is a SQLite or DuckDB `path` inside an import directory, or for Postgres and MySQL `url_env`, and for Postgres and DuckDB a `schema` (`public` and `main` by default; the configuration refuses one on SQLite and MySQL, whose MySQL schema is the URL's database, and one holding a control character or a `/`, which would make the provenance's `<host>/<database>/<schema>` ambiguous, D310); a file is confined when the import runs, as any import's path is (D232), must be a regular file, and must still be that file (its device and inode) when the snapshot opens it and when it ends (`PATH_NOT_CONFINED`); a URL is read from the environment when the import runs, and read by D310's grammar (`INVALID_VALUE`, naming the connection and the variable, never the value); the dataset's `source` is `{kind: "database", location}`, the location `<host>/<database>/<schema>` for a server, the file's location for SQLite and `<location>/<schema>` for DuckDB, never a port, user, password or query string; the dataset's name defaults to the connection's; a named connection is read by the core's importer (`aibi.databases`), never a pack's (`CONFLICTING_MEMBERS`), and pack validators' `validate_source` gets a `DatabaseSource` (connection, kind, location); the CLI names one with `aibi import <dataset> <name> --connection` | §14 has a request name a connection and provenance hold host, database and schema only; reading the URL when the import runs means a rotated credential needs no restart and the configuration never holds it; the location says which data it was without saying how to get in |
| D306 | Reading a snapshot (M1; deviates from issue #13 and §12.4's stack row, which read SQLite sources through DuckDB) | A snapshot is read in the import's worker process (D225's `Reader`, under `reader_memory`, `reader_seconds`, `decoded_bytes` and `reader_workers`), one per import, which the server's process only names: it never loads DuckDB. SQLite files are read with Python's `sqlite3`, not DuckDB's scanner: a `file:` URI with `mode=ro`, `SQLITE_DBCONFIG_DEFENSIVE` on, views, triggers, extension loading and `trusted_schema` off, `mmap_size` 0, `cell_size_check` on, temporary data in memory and `query_only`, and the tables those of type `table`, not virtual or shadow tables (D309), not named `sqlite_*` (one that is not a table SQLite keeps of its own, such as `sqlite_sequence`, is noted as skipped); a cell is an integer, a real, a text or null, and a BLOB is `UNSUPPORTED_FORMAT`, and so is a `VIRTUAL` generated column (D309); reading a file in WAL mode that was closed cleanly makes its `-wal` and `-shm` beside it, which SQLite needs to read it, so a read-only directory cannot hold one. A DuckDB file is attached read-only to a session with extensions neither installed nor loaded, unsigned and community extensions refused, persistent secrets off, external access disabled but for the file and its `.wal`, no temporary directory, UTC, one thread, half `reader_memory` as its memory limit, and its configuration locked before it is attached; a generated column is `UNSUPPORTED_FORMAT`, naming it (its catalogue gives its expression as a default, which `DESCRIBE` does not). A SQLite or DuckDB file's `-wal`, `-shm` and `-journal` (DuckDB's `.wal`) must be regular files where they exist, and, where `/proc/self/fd` lists them, the regular files the reader opened must be the file and those beside it (`PATH_NOT_CONFINED`). A Postgres or MySQL session loads the scanner of its kind from its pinned wheel (`duckdb-extension-postgres-scanner` and `-mysql-scanner`, the version of DuckDB's own), whose signature DuckDB checks, attaches the connection string built from the URL (D310) read-only, then disables external access and locks its configuration, a MySQL session with D308's settings first; persistent secrets keep DuckDB's default there, since the scanners' `ATTACH` uses the secret manager, whose settings cannot change once it has been used, and it runs no statement that makes a secret, but every snapshot's session is given a `secret_directory` that cannot exist (`/dev/null/secrets`) before it loads an extension, so that no stored secret supplies a port, a password or anything else the URL leaves out (D310); Postgres base tables are ordinary and partitioned tables (not partitions) of the schema, MySQL's `BASE TABLE`s (and MariaDB's system-versioned tables, D308) of the URL's database, and the catalogue is read from their own catalogues (Postgres's by `postgres_query`, filtered on the server by the schema's name, D309; MySQL's `information_schema` through the attached catalogue, D308). Every read happens in one transaction (SQLite's deferred read transaction; DuckDB's, which the Postgres scanner runs as `REPEATABLE READ` and read-only, and the MySQL scanner as `START TRANSACTION READ ONLY` at a level that reads a snapshot, D308); each table is counted before any row is read (`import_tables`, `table_columns`, `import_cells`), then read by a SQLGlot `SELECT` of its quoted columns ordered by its declared primary key, or else by all its columns, text by its bytes (D309); a Postgres table is counted and read on the server, by `postgres_query` in the snapshot's transaction, its statement generated by SQLGlot in Postgres's dialect and bound as a parameter, each column the catalogue names, in its order, selected as `c0`, `c1`… and ordered by DuckDB, an ordinary table read `ONLY` (without the rows of the tables that inherit from it), and every `numeric` (through its domains too) cast to text there and a `money` to `numeric` and then text (D309), whose rows are its exact text as a DuckDB file's and a Parquet file's decimals are; a MySQL table is counted and read through the attached catalogue (D308); DuckDB's rows are read as Arrow, whose types the Parquet reader reads (`parquet.from_arrow`, D225: another type is `UNSUPPORTED_FORMAT`, naming the table, the column and its type); a cell over 131,072 characters is `UNPARSEABLE_SOURCE`, naming its row by its place in the snapshot's order, from 1, as every refusal of a snapshot's rows does, a snapshot having no header row; whatever else the database or its driver raises is `UNPARSEABLE_SOURCE` naming its class alone, and the table being counted or read (D309), and DuckDB running out of memory is `reader_memory`. A Postgres snapshot reads its columns and keys after it has locked its tables (D309); the list of relations, and what skips one, are read before, in the snapshot's transaction, so a table's DDL committed between that read and the lock is seen by the lock and the reads after it, as `pg_dump`'s is between its catalogue read and its locks, and a lock another session holds past half of `reader_seconds` (`lock_timeout`, set in the snapshot's transaction as it starts, with `client_connection_check_interval` where the server has it) is `LIMIT_EXCEEDED` naming `reader_seconds` and the table, so that a session the worker leaves behind when it is stopped waits no longer than that | DuckDB's SQLite scanner gives no foreign keys and binds a file's views as DuckDB SQL, while SQLite can refuse views and triggers itself and gives declared keys in the same read transaction; hardened DuckDB cannot attach a server (D172), so the server's session is locked once it has; the scanners must be bundled, not downloaded, and DuckDB's signature check makes a repackaged wheel safe to load; an order by key makes the same data the same raw snapshot, so a re-import of an unchanged database is `NO_CHANGE` whatever the server's storage order; DuckDB's and the drivers' messages can hold the URL; the scanners read a decimal wider than DuckDB's 38 digits as a double, silently losing digits, DuckDB renames Postgres's columns that differ only in case, and a Postgres parent table's rows include its children's unless read `ONLY`; a generated column would run the file's expression in the snapshot's session, where it could read its settings; neither SQLite nor DuckDB opens a path without following a link that replaced it after the check, and a link beside the file could have the reader open or write a file elsewhere; MySQL's `TINYINT(1)` holds integers; the servers' tests run only with `AIBI_TEST_POSTGRES_URL` and `AIBI_TEST_MYSQL_URL`, which CI does not set, and have run against PostgreSQL 16 and MariaDB 10.11 |
| D307 | What a snapshot declares (M1) | A declared primary key is the table's key, `imported`, and inference looks for no other; a declared foreign key to a table of the snapshot is a relationship whose tables and columns are `imported` and whose cardinality is proposed from the rows (one-to-one when its present values are distinct), SQLite's without parent columns naming the parent's primary key, and no relationship is proposed by containment from a column it holds; names are matched as written, or else ignoring the case of ASCII letters alone, as SQLite does, when exactly one matches (D309); one whose parent is not read (in another schema or database too), whose columns do not match or repeat one, in the child or the parent, whose parent columns are not the parent's declared primary key in some order, or whose child columns another of the table's has, in any order, is not kept, with a note; a table's and a column's comment is its `definition`, `imported` (at most 10,000 characters, `LIMIT_EXCEEDED`, and Unicode text without a control character but tab, line feed and carriage return, `UNPARSEABLE_SOURCE`, naming the table and the column); a column type DuckDB reads is its datatype, `imported` (a naive timestamp `imported_default`, as a Parquet file's, D227), and SQLite declares none; a declared key or foreign key the rows break stops the import (§13.2's declared keys), never dropped, the rows it names counted from 1 in the snapshot's order (D306) | §13.1 imports declared keys and comments as `imported`; a key the database enforces is no guess, and one it only declares (SQLite does not enforce foreign keys unless asked) is still the source's statement, which the gate checks rather than drops; a relationship leads to its parent's key (§5.6), so a foreign key to a unique column that is not the key is noted rather than refused by the release, and one that repeats a column, which SQLite accepts, would be a relationship no descriptor holds, while two on the same columns in different orders would be relationships from the same child columns, which need roles an import cannot give; a comment becomes a definition that agents and terminals read (A6), so a control character in it is refused, as text that is not Unicode is, rather than changed |
| D308 | MySQL snapshots in one transaction (M1; amends D306) | A MySQL snapshot reads its catalogue (`information_schema`'s `TABLES`, `ENGINES`, `COLUMNS` and `KEY_COLUMN_USAGE`, filtered by the URL's database as a bound parameter) and its rows through the attached catalogue alone, which the scanner reads between one `START TRANSACTION READ ONLY` and its `COMMIT` on the one pooled connection DuckDB's transaction holds (`mysql_enable_transactions` on, `mysql_pool_size` 1, `mysql_pool_acquire_mode` `try`, so that a read needing another connection fails), and never by `mysql_query`, which runs each statement in a transaction of its own; `mysql_query` reads only the session's isolation level (`@@transaction_isolation`, or else MariaDB's `@@tx_isolation`), before that transaction starts, and a level other than `REPEATABLE READ` or `SERIALIZABLE` is `UNSUPPORTED_FORMAT`, listing those. Its base tables are `BASE TABLE`s and MariaDB's `SYSTEM VERSIONED` tables, read as they are now; views, system views and sequences are skipped and noted as what they are. A base table whose engine keeps no transactions (`ENGINES.TRANSACTIONS` other than `YES`: MyISAM, Aria, MEMORY…), a `DECIMAL` of more than 38 digits (38 is read exactly), and two relations of the database, one a base table, or the URL's database and another of the server, whose names are equal under the server's collation (their `WEIGHT_STRING`s in `information_schema.SCHEMATA` and `TABLES`, which `mysql_query` reads before the transaction starts, the database named by the hexadecimal digits of its UTF-8) or ignoring the case of ASCII letters, as DuckDB's catalogue matches names, are `UNSUPPORTED_FORMAT`, naming them; a failure to read the isolation level is `UNPARSEABLE_SOURCE`, naming the exception's class. The session reads `TINYINT(1)` and `BIT(1)` as the types they declare, `TIME` as its text, orders nothing on the server, pushes counts to it (`mysql_aggregate_pushdown_enabled` on), and has UTC as its time zone (`mysql_session_time_zone` `+00:00`), so that a `TIMESTAMP` is its UTC time without a zone; a date with a zero month or day is `UNPARSEABLE_SOURCE` (the scanner's conversion error, naming the table), and so is a zero date (`0000-00-00`), which the scanner reads as null, naming its column: the column's count of present values on the server, where a zero date is one, is more than the present values read; every name the scanner writes into what it sends the server must be one it quotes safely (`urls.mysql_quotable`, an allow-list): printable ASCII but the backtick and the backslash, and characters beyond ASCII of no control, format, surrogate, private-use or unassigned category, and the URL's database, which it also writes as a string constant, no `'` either. The URL's database is refused as `INVALID_VALUE` before connecting (D310); every base table of the database, and every column of one, is checked when the catalogue is read, before any row is, and refused as `UNSUPPORTED_FORMAT`, naming the table, or the table and the column, while a view's or a sequence's name, which the scanner never writes, is not; and the URL's database must be one of `SCHEMATA`'s names exactly (`INVALID_VALUE`, listing the server's spellings of it in other cases), since a server with `lower_case_table_names` 1 or 2 opens a database whose name differs in case | Issue #13 and §13.1 promise one consistent snapshot, and `mysql_query` sends `SET autocommit=1` before each statement, so that a child table read after its parent saw commits made after the parent's read (round 2's B1: `DANGLING_REFERENCE` under a concurrent writer, and tables without a declared foreign key silently inconsistent); reads through the catalogue in DuckDB's transaction are sent inside the scanner's own transaction, and `mysql_execute` cannot open one on a read-only attachment; a `READ COMMITTED` transaction reads each statement's own snapshot, and a table of an engine without transactions none; MySQL's column names are case-insensitive, so reading through the catalogue loses only table and database names that differ in case, which DuckDB's catalogue resolves to one for both (a database `Db` beside `db` made `src.db` read `Db`'s tables) and which are refused rather than read wrongly, and so are names MariaDB's accent-insensitive `utf8mb3_general_ci` equates (`cafe` and `café`), whose tables its catalogue joins merge; the server's own collation says which, where a fold of ours over-refused `strasse` beside `straße` and missed accents; the scanner reads a decimal wider than 38 digits as a double and a zero date as null, both silent changes of a value; a `TIMESTAMP` would otherwise be read in whatever time zone the server is configured with; DuckDB's MySQL scanner escapes a backtick in an identifier with a backslash, which MySQL does not read as an escape there, so that a table named `` cover` UNION SELECT amount FROM other.salaries -- z `` beside a view named `` cover\ `` read the other database's rows under the provenance of this one (round 6's blocker), and it leaves a backslash unescaped in a string constant (a database `d\b` was read as empty) and escapes a `'` with a backslash in its own catalogue query, which a server whose `sql_mode` has `NO_BACKSLASH_ESCAPES` reads as the literal's end (checked); an allow-list of what was checked to be quoted safely leaves no next character to find; with `lower_case_table_names` 2 the server opens a database named in another case, whose `DATABASE()` matches the URL while the catalogue's filters match none of its tables, so the snapshot would be silently empty |
| D309 | Values and names a snapshot keeps (M1; amends D306 and D307) | A Postgres `numeric` of any precision and scale, through its domains too, is cast to text on the server, and a `money` to `numeric` and then text; Postgres's catalogue is filtered on the server by the schema's name, written as `convert_from(decode('<hex>', 'hex'), 'UTF8')` from its UTF-8. An array of `numeric` or `money` is cast to an array of text, read as a list of text; a partition whose partition tree's root (`pg_partition_root`) is a table of another schema is skipped and noted, and so is a partitioned table whose tree holds a foreign table (`pg_partition_tree`), with the foreign table noted as one. Each Postgres column of a type outside `pg_catalog` (an enum, a domain, a composite, an array of such, a range or a multirange) is read through its type's output function, called by `pg_catalog.format('%s', …)` with its null kept, so that no cast is resolved on the read: cast lookup ignores the transaction's snapshot and resolves against the current catalogue, so a schema owner could add or change a function cast to `text` or `varchar` after the snapshot has begun and have it run, as the import's role, on a later read in the snapshot (the scanner casts a type it does not map to `varchar` in its `COPY`); the output functions `format` reaches (`enum_out`, `record_out`, `array_out`, `range_out`, a multirange's, and a domain's base type's) are `pg_catalog`'s, or an installed extension's for an extension's type (`citext`, `hstore`, `ltree`…), which is read the same way. Only a superuser can make a base type or set its I/O functions, and an extension was installed by a superuser or through the trusted-extension mechanism, so under the threat model of a non-superuser schema owner an extension's code is trusted, and its type is read rather than skipped. The rule: no read path may resolve a cast, operator or function by catalogue lookup for a type outside `pg_catalog`. A Postgres table with row security (`relrowsecurity` or `relforcerowsecurity`) or a virtual generated column (`attgenerated` `v`, from PostgreSQL 18) is skipped and noted, since reading it would run its owner's code as the import's user and a policy would choose its rows, and so is a table with a column that reaches a base type whose own output or send function is neither `pg_catalog`'s nor an installed extension's (`pg_depend` on `pg_extension`), through the column's type, its domains, arrays, ranges, multiranges and composite fields, whose function `format` would run and which a superuser made outside any extension; every catalogue query names each function, type and operator it uses by its schema, `pg_catalog`, and the session fixes at connect time every setting that shapes a value's text (`DateStyle`, `IntervalStyle`, `TimeZone`, `client_encoding`, `extra_float_digits`, `bytea_output`, `lc_numeric`, `lc_monetary`, `quote_all_identifiers`), so that a role or database cannot steer the recorded bytes and an unchanged database re-imports the same (D306). Each Postgres base table read is locked `ACCESS SHARE` in the snapshot's transaction before its columns are read, as it is read (an ordinary table `ONLY`, so that its inheritance children are not locked; a partitioned one with every partition), so a `RENAME` cannot commit between the catalogue read and the row read and mislabel a column; a table another session holds locked is waited for at most half of `reader_seconds`, then refused naming it (D306). A `time with time zone`, or an array of one, is read as its exact text (Postgres) or refused as `UNSUPPORTED_FORMAT` (a DuckDB file), since Arrow's `time` has no zone and would drop the offset, collapsing distinct values; a value out of a type's range (an `infinity` or a BC date) that the reader cannot represent is `UNPARSEABLE_SOURCE`, naming the table and the column. A partition that an interrupted `DETACH PARTITION … CONCURRENTLY` left pending (`pg_inherits.inhdetachpending`, read through `to_jsonb` so that a server without the column reads none), or a partition beneath one, is skipped and noted as a partition being detached. SQLite's `pragma_table_list` runs when a table has no root page (`rootpage` 0 in `sqlite_schema`) or its `sql` holds `virtual` in any case, since SQLite reads a table as virtual by that keyword of its `sql`, whatever its `rootpage` says. DuckDB running out of memory as it attaches the database or reads the MySQL session's isolation level is `reader_memory`, as it is elsewhere. Catalogue text that is not ASCII fails in a database whose encoding is not UTF-8 (the scanner's binary copy does not convert it), as `UNPARSEABLE_SOURCE`. A table is read in the order of its key, or of all its columns, text compared by its bytes whatever its collation: SQLite's `COLLATE BINARY`, then each cell's `typeof`, since SQLite's order ties an integer and the real that equals it, then a zero real's sign (a function of the session's, since SQLite's order ties `-0.0` and `0.0` and none of its own functions tells them apart), and DuckDB's `COLLATE C` for a `VARCHAR`; a server's rows are sorted by DuckDB without a collation. A SQLite `VIRTUAL` generated column is `UNSUPPORTED_FORMAT`, naming it, as a DuckDB file's generated column is; a `STORED` one is read. What the database or its driver raises while a table is counted or read is `UNPARSEABLE_SOURCE` naming the table and the exception's class, never its message; opening the database still names the class alone. A name from a catalogue that a refusal or a note gives has each lone surrogate and noncharacter written as its `\u` escape, so that a skipped relation's or a schema's name is never refused (a table's or a column's still is, D307); at most `import_tables` skipped relations are named, by the worker, which counts the rest in the snapshot it sends, and one more note counts them; SQLite's relations are listed from `sqlite_schema`, and `pragma_table_list`, which tells shadow tables apart, runs only when a virtual table may be there; the server running out of memory while it describes a snapshot is `LIMIT_EXCEEDED` naming `decoded_bytes`, which bounds its cells; a skipped relation's note says what it is (`a view, not a base table`), and a base table without columns says so | A Postgres `numeric` can hold `NaN` at any typmod (since PostgreSQL 14), which the scanner read as `0.00`, and a negative scale or a scale over the precision (since 15) made DuckDB fail without naming the column; `money` is text in the server's locale's format; a database of many schemas was fetched whole to filter it in Python, and hexadecimal digits cannot be quoted wrongly whatever `standard_conforming_strings` says; rows a collation or SQLite's comparison ties came in storage order, so an unchanged database could re-import as other rows and not `NO_CHANGE` (D306); a `VIRTUAL` column runs the file's expression as it is read, which D306 refuses in a DuckDB file (`trusted_schema` off limits what SQLite's can call, but the rule is the same); a table's name comes from the catalogue, not the driver's message, which can hold the URL; a noncharacter in a view's or a schema's name made the import fail with `INTERNAL_ERROR`, and 50,000 views made 50,000 notes and took 105 s; capping the notes alone still sent every name through the pipe, and `pragma_table_list` computes every view's columns in time that grows faster than their number (32,000 views, 37 s); a partition whose parent is not read vanished without a note, the immediate parent's schema misjudged a sub-partition, and a foreign partition's rows were read through the foreign server, outside the snapshot's transaction and from a database the provenance does not name; a comment in a virtual table's `sql` hid it from a match of its text, and a `rootpage` forged non-zero, which `writable_schema` can write as it can the `sql`, had a virtual table and its shadow tables read through the module; a partition pending detach, whose rows its table no longer holds, vanished without a note; a table's row security policies are its owner's expressions, run as the reader and choosing its rows (round 7's second blocker), which skipping the table avoids rather than trusting whatever the policy returns; a schema owner's `CAST` to `varchar` with a function ran as the import's role when the scanner serialised a user type, another way owner code ran on a read (round 8's first major); skipping such a table missed a cast added mid-snapshot, since cast lookup ignores the snapshot (round 9's follow-up), so every non-`pg_catalog` column is now read through its output function and resolves no cast, and only a user base type, which that function would still run, is skipped; a role's `DateStyle`, `TimeZone` or `IntervalStyle` changed the text of a `daterange` or `interval` so an unchanged database re-imported as changed (round 8's second major); a column renamed mid-snapshot mislabelled the rows read after it; a `timetz` read through Arrow lost its offset; `numeric[]` was refused as a list of doubles; a role's `quote_all_identifiers` changed a `regclass`'s text, a multirange's range was not followed, a `timetz[]` refused the import, an ordinary table's lock took its inheritance children's too, and a worker stopped while its session waited for a lock left the session waiting, holding the locks it had (round 9) |
| D310 | Connection files and URLs (M1; amends D305) | A SQLite or DuckDB connection's file must be the server's user's or root's, and neither its group nor others can write it, in a directory that the server's user or root owns and that neither its group nor others can write, unless it has the sticky bit (`PATH_NOT_CONFINED`, naming the file or the directory by its location in its import directory, never by its absolute path); the links on the way to it and the directories above it are not walked as D253 walks the configuration's, since the file's device and inode are checked when the snapshot opens it and when it ends and, where `/proc/self/fd` lists them, the files the reader opened are checked too (D306), which leaves a window only where there is no `/proc`. A Postgres or MySQL connection's URL is never given to a driver. The server reads it by one grammar of its own (`importers.urls`), `<scheme>://<user>[:<password>]@<host>[:<port>]/<database>[?<name>=<value>&…]`: the scheme `postgresql` or `postgres`, or `mysql`, in lower case; printable ASCII without a space or a `#`; exactly one `@` in the authority, and a user; one host, a DNS name, an IPv4 address in dotted decimal (a host whose last label is all digits or begins with `0x` must be one, since `getaddrinfo` reads `0177.0.0.1` in octal, `0x7f.0.0.1` in hexadecimal and `127.1` or `2130706433` in short), or a bracketed IPv6 address, without a `,` or a percent-escape, recorded in lower case or as its address's canonical text; a port from 1 to 65,535 if any; one path segment, the database, without a `/` or a control character (C0, DEL or C1, as the configuration's `schema`) once decoded, and no longer than the server keeps a name (Postgres: 63 bytes of UTF-8, and the user too; MySQL: 64 characters), a MySQL database holding only what DuckDB's scanner quotes safely (D308); the user, the password, the database and each value percent-decoded as UTF-8 without a NUL, a `+` being itself; and each parameter's name one the kind allows, once (Postgres: `application_name`, `connect_timeout`, `sslmode`, `sslrootcert`, `sslcert`, `sslkey`, `sslcrl`, `channel_binding`, `gssencmode`, `keepalives`, `keepalives_idle`, `keepalives_interval`, `keepalives_count`, `tcp_user_timeout`; MySQL: `socket`, `ssl_mode`, `ssl_ca`, `ssl_capath`, `ssl_cert`, `ssl_cipher`, `ssl_crl`, `ssl_crlpath`, `ssl_key`). Anything else is `INVALID_VALUE`, naming the connection, the variable and why, a parameter by its name when it is plainly one, never a value. The driver is given the `keyword=value` connection string built from exactly what was read (`host`, `port`, the database, `user`, `password`, the parameters), a Postgres one with `options` too (`-c search_path=pg_catalog,pg_temp -c row_security=off` and a `-c` for each output setting D309 fixes, `quote_all_identifiers=off` among them), which a startup packet sets over whatever the role or the database sets, so that the session resolves names in `pg_catalog` alone and a read that a row security policy would filter fails, each value quoted and escaped as it reads quotes (libpq's `'…'`, the MySQL scanner's `"…"`), and the provenance records the same host and database. The snapshot then reads back the database its session opened, Postgres's `current_database()` in the snapshot's transaction and MySQL's `DATABASE()` on the one pooled connection before it starts (D308), and refuses one other than the URL's as `INVALID_VALUE`, naming the server's name for it; the provenance records the database as the server named it. A MySQL URL whose host is `localhost` must give `socket`, since the scanner's client otherwise connects to its compiled-in default socket (`/tmp/mysql.sock`), which any local user can plant, and a `?` in any MySQL value (the user, the password, the database, a parameter's value) is refused, since the scanner splits the whole connection string at its first `?`, whatever the quoting; both are `INVALID_VALUE`. MySQL's `socket` names a local socket, which the client uses in place of TCP when the host is `localhost`, and which the provenance does not record, so that with it the provenance's host is `localhost` whatever server listens on that socket (a deviation from recording the server read, accepted since the URL is the operator's); Postgres's `sslkey` can name an OpenSSL engine's key (`engine:key-id`), which loads that engine; both are the operator's, as the URL is. The worker's environment holds no variable a driver reads as a default (`PGHOSTADDR`, `PGSERVICE`, `MYSQL_HOST`…, D225's worker is given `PATH`, `PYTHONPATH` and a locale alone) | Three rounds found a URL that the server's parser and libpq's read differently (query parameters that override the host, a `#` that hid them, a second `@` that made the password's text a host list or a socket path), each time with provenance naming a host other than the one read; checking for differences one at a time leaves the next, while a connection built from what the server read reaches the host it names (but for a `socket` the operator gives, which `localhost` now requires, since without it the MySQL client read a planted default socket, round 7's major); a Postgres session resolved the names its catalogue statements called through the reading role's and the database's `search_path`, which whoever could write the imported schema could use to change what they returned (round 7's first blocker), and a startup option is the one setting neither can override; Postgres cuts a database's or a user's name longer than 63 bytes short and opens the database the shorter name names, and a control character reached `source.location`, so the grammar bounds names as the servers keep them and the snapshot checks the database back rather than assume it; `getaddrinfo` read numeric shorthand as an address other than the one the provenance showed; a persistent secret beneath the worker's working directory, which is DuckDB's default with no `HOME`, gave a URL without a port or a password the ones it left out; a file others can write can be rewritten in place, which its device and inode do not show; a directory or a file another user owns lets that user swap the file; an absolute path tells a client how the server is laid out |
| D311 | The read-only catalogue page (M1; refines D255 and D265; deviates from §12.4's Frontend row, which it amends) | The server renders the catalogue page itself, as HTML without a script, a form or a remote resource, at `GET` alone: `GET /` lists the latest published release of every dataset, 50 at a time from `?offset=`, with its label, name, description, domain tags, data-use terms, packs, concepts and pack facets and its tables with their grains, roles and row counts; `GET /datasets/<id>` describes the latest published release of one dataset, 500 columns at a time from `?columns_offset=` in at most 50 tables (the next page starts at the columns of the first table not shown), with its release, effective disclosure setting, standing caveats and dataset descriptor on every page, and, for the tables the page shows, their part of the table graph (the tables, and each relationship whose child they are, with its child and parent columns and cardinality, as a table), each table's descriptor, row count and brief columns, and the relationship and coverage descriptors whose child they are and the endpoint descriptors on them (those naming no table, and coverage of an unknown relationship, on the first page), every member with its curation status and `by` (A5), so that a page holds at most 50 tables, 500 columns and what starts from them (about 200 KB, rendered in about 30 ms here, for a dataset of 1,000 tables); a Previous link never starts past the last page. Each page runs its tool (`search_catalog`, `describe_dataset`) through the server's tool calls (`Calls.tool`), as the client that asked: the same service function, disclosure settings, places, `tool_seconds` and refusals as MCP and `POST /api/tools/<name>`, the tool's refusals shown without their paths (which point into a request the browser never sent), and each request at a page path counts against the client's `page` rate (D314), never its `api` rate; the page is rendered in the call's worker thread while it holds its place, never on the event loop, so a page view holds one of the client's two tool places (D278) for as long as its tool and the rendering take (tens of milliseconds for a small dataset; for a dataset of 1,000 tables, the half-second `describe_dataset` takes to describe it, as over MCP, and the page's 30 ms), and a burst of views within the `page` rate can make an agent's call of the same client wait for its place, for seconds at the largest datasets, but never refuse it sooner than `tool_seconds`; a hit's name and description are cut as a descriptor member's value is (D312); every count is shown with its `stat:` reference, a suppressed one as suppressed and one not estimable for another reason with its reasons. Every answer at a page path (`/`, `/datasets` and below, and `/favicon.ico`, which a browser asks for) is a page, refusals included: the page's own and its tool's, request protection's (`HOST_NOT_ALLOWED`, `ORIGIN_NOT_ALLOWED`, a rate), routing's (`NOT_FOUND`, and `METHOD_NOT_ALLOWED` for `HEAD` or `POST`, with `Allow: GET`) and `INTERNAL_ERROR`, each with D265's status, refusals and blanking, as HTML (refining D265's one error shape), `Retry-After` with a 429 and a 503; every other path keeps D265's JSON. A query parameter the page does not take, one given twice, or an offset that is not at most 16 digits and at most 2^53 − 1 is `INVALID_VALUE`, naming the parameter and never echoing what was given, and so is an address whose dataset is not a dataset id, in plain words. Drafts, other labels, column descriptors (`describe_column`), the curation queue and search filters are not shown; a mount at or below `/datasets` is refused as one under `/api` is (refining D255). The React UI replaces the page in M5 | §15 ships a thin read-only page with M1 so there is something to show without an agent; rendering tool outputs on the server keeps one set of functions behind every surface (§1.1 goal 7, §11.1), so the page can show nothing an agent is not shown, suppressed counts included; a page without a script runs nothing that injected markup could reach, and needs neither a build toolchain nor generated types before M5; a person who follows a link, or mistypes an address at the page's paths, should get a page rather than JSON (an address elsewhere, such as `/index.html`, keeps D265's JSON) |
| D312 | Text on the catalogue page (M1) | Every text of the page is written as an HTML text node or attribute value, never as markup: its unsafe characters, the control characters (C0 but tab and line feed, DEL, C1), the format characters (Unicode's `Cf`, the bidi formatting, zero-width spaces, the byte order mark, soft hyphens and tag characters among them, but the zero-width non-joiner and joiner that scripts and emoji need), lone surrogates (`Cs`, which UTF-8 cannot carry) and the line and paragraph separators, become visible `\xNN`, `\uNNNN` or `\UNNNNNNNN`, each run of them in an element's text in one `<span class="escape">`, styled apart, so that it is told from the same characters in the data (a carriage return before a line feed is dropped); then `&`, `<`, `>`, `"` and `'` become character references; text from data (labels, definitions, descriptor members, pointers, statistic references, data tokens) is each in a `<bdi>` of its own. Elements and attributes come from an allow-list without scripts, embeds, forms, event handlers, `style` or `src`; a link is `/`, or segments after each `/` of letters, digits, `.`, `_`, `~`, `-` and percent-escapes, with an optional trailing `/` and a query of those characters, `=` and `&`, or a fragment of one such segment (so no tab, line feed, backslash or second `/` can turn it into a link off the server), built from the server's root path, percent-encoded, a dataset id the tool validated, a table id as a fragment, and numbers alone; a root path that is not plain segments (with `.`, `..`, an empty segment or a trailing `/`) is refused loudly, the request failing with `ValueError`, rather than dropped, since links built without it would leave the application; the stylesheet is a literal of the server's code. A descriptor member's value is its text, or its compact JSON, shown up to 2,000 characters, the rest cut and marked, and so is every other text from data (labels, names, descriptions, grains, tags and terms); a list from data (tags, facet values) shows its first 64 members and counts the rest. The operator CLI's `escaped` keeps its own narrower set (control and bidi characters) | A6 and §14: labels, definitions and cell values are rendered as plain text; escaping at the one place text is written, and building only from allowed names, makes that structural, so no rendering path can forget it; an override or an isolate in a label could otherwise reorder the text around it, invisible characters make two values that look the same differ, and an escape sequence could reach a terminal a page is copied into; a marked escape cannot pass for data that spells one, and a span per run, not per character, keeps a text of invisible characters from growing the page more than a few times; `aibi-server` sets no root path, so only an embedder can meet the refusal, and a wrong link is worse than a failure it sees |
| D313 | The catalogue page's policy (M1) | Every response at a page path (D311), refusals included, carries `Content-Security-Policy: default-src 'none'; style-src 'sha256-<its stylesheet>'; img-src 'self'; connect-src 'self'; form-action 'none'; base-uri 'none'; frame-ancestors 'none'`, `Cache-Control: no-store` and `Cross-Origin-Opener-Policy: same-origin` (D314), besides request protection's headers (D255); the page's one stylesheet is inline, allowed by its hash, and loads nothing | §14's policy for the UI allows images and connections to the server only; with no script source nothing runs, and a hash admits the page's own stylesheet alone, so injected markup could neither style nor load anything; `no-store`, as the operator router's (D255), keeps a withdrawn or erased release's descriptors out of the browser's cache and history, where `no-cache` would keep them, and the page has no validators that `no-cache` could save a transfer with |
| D314 | Links from other sites to the catalogue page (M1; refines D257 and D259) | Request protection lets a top-level navigation to a page with the person's activation through its fetch-metadata check: `GET` of `/` or `/datasets/<id>` (one path segment, any query) with `Sec-Fetch-Site` `same-site` or `cross-site`, `Sec-Fetch-Mode: navigate`, `Sec-Fetch-Dest: document` and `Sec-Fetch-User: ?1`, each given exactly once and exactly as a browser writes it (a header given twice, contradicting or not, or in other case, lets nothing through); an `Origin`, if given, is checked as D257 checks it. Every request at a page path, a navigation and a CORS preflight included, is counted against the client's own `page` rate (`[server.rates] page`, 120 a minute, bursts of 30; `LIMIT_EXCEEDED` naming `page_requests`, 429), never against its `api` rate. Its Host is checked first, and any other request from another site (a navigation without the person's activation, which has no `Sec-Fetch-User`; a page's subresources, frames and `fetch`; other methods, `HEAD` included; other paths, `/favicon.ico` and `/datasets/<id>/…` included; a WebSocket) is still `ORIGIN_NOT_ALLOWED` before any rate counts it. Every page response carries `Cross-Origin-Opener-Policy: same-origin` (D313) | A lab wiki, a chat or a README links to the catalogue, and a browser that follows the link sends `Sec-Fetch-Site: cross-site`, which D257 refused, so only typed addresses and bookmarks opened it. Letting such a navigation through is safe because the page is read-only: it serves only what the public tools already show anyone who can reach the server, `GET` changes no state and runs no operator operation, the page carries no token, handle or CSRF key and reads no cookie, and the answer goes to the person's tab, which the linking site cannot read (no CORS, `frame-ancestors 'none'`). `Sec-Fetch-User: ?1` does not make a hostile page harmless: a browser sends it for a navigation the person started, and also for one a script starts while the page still has the transient activation of the person's last click or key (about 5 s in Chromium, 1 s in Firefox; assigning a window's location does not use it up), so a page the person clicks on can send a burst of navigations, to a window it opened or to its own before the first answer arrives; `Cross-Origin-Opener-Policy: same-origin` cuts the opener's handle only once the first answer commits, and is defence in depth (`127.0.0.1` and `localhost` are secure contexts, where browsers apply it over plain HTTP; any other address needs TLS, §14). What `?1` does is refuse, before any rate, the navigations a page starts without the person having clicked on it lately. What bounds the rest is the `page` bucket: a burst of at most 30 and then two requests a second (one page view a second with the favicon a browser asks for beside it, which `no-store` lets it ask for each time), which can spend only the person's own page views and the load D311 states, and never the `api` bucket the same loopback client's agents use, so page views never make an agent's request `LIMIT_EXCEEDED` on a rate, and the tool places they hold can only make an agent's call wait (D311). Subresources stay refused before the rate is counted, as D255 has it; a browser without fetch metadata could always spend a client's rates, as on any path |
| D315 | M3 in slices (M3) | M3 is delivered in five slices, each a PR with its tests: M3.1 the registry and applicability, phase 2 of canonicalisation, `list_analyses` and `run_analysis`, `compare.existence` with its methods held to R, the disclosure of the counts a result adds, and its chart; each slice is a sub-issue of #18 (#40 to #44); M3.2 aggregates (§9.2) in the reference evaluator and the compiler, the materialised inputs a `requires` names, `summary.distribution`, `summary.members`, `compare.columns` with its bootstrap, and packs' analyses run; M3.3 `survival.km` and `survival.cox`; M3.4 the thread-count determinism tests over a fixture of a million rows (§9.3); M3.5 the result cache (§8.1). §15's exit criteria hold for M3 as a whole | `compare.existence` needs no aggregate and no endpoint, only each unit's truth value, which M2's queries already give, so it exercises the registry, phase 2, `run_analysis`, estimability and result disclosure end to end before the aggregates they will carry |
| D316 | The registry and applicability (M3.1; refines D275 and D279) | `analyses.registry.Analyses` holds the core's entries (each an analysis descriptor whose `params` and `returns` are generated from its models) and each installed pack's, by id; a view reaches an analysis only through it. `applicable(descriptors, dataset, manifest, unit?)` matches each entry's `requires`: a requirement with a `kind` is met by at least `min` (1 by default) descriptors of that kind (endpoints; columns of its `datatype`; tables other than the unit), on the unit table with `"on": "unit"`; a `predicate` requirement also needs its pack's predicate to hold of the release, one not installed, not registered or raising meeting nothing; requirements without a `kind` are the view's. `unavailable` names the roles not met (`missing`), `available_with_caveats` those met, but not by `min` descriptors without a field `imported_default`, `proposed` or `undeclared` (`unconfirmed`); without a unit, each keyed table is tried and the best status kept, each requirement predicate run once, since it reads the release and not the unit. `describe_dataset` gives it for the release it describes; `list_analyses` (`{"dataset"?, "release"?, "unit"?}`, `release` and `unit` needing `dataset`) gives every entry and, for a dataset, its applicability (`UNKNOWN_TABLE` at `/unit`); `aibi://analysis/<id>@<version>` resolves, and the resource list names every entry. A pack's analysis is registered, listed and matched from M3.1 but run from M3.2 (`NOT_SUPPORTED` at `/views/<i>/analysis`) | §9.1 and §9.4; running a pack's analysis needs the columns, aggregates and endpoint rows its `requires` names materialised (§10.1), which is M3.2's; trying each keyed table lets `describe_dataset` answer without a unit |
| D317 | Phase 2 (M3.1; revises D284, refines D299) | A view is checked in two steps around phase 1. Before it: its `analysis` must be in the registry (`UNKNOWN_ANALYSIS` at `/views/<i>/analysis`, listing its ids); its `params`, after substitution, are validated by the analysis's model, each refusal where it is written (or at the `"$name"` that supplied it); an analysis with `uses_reference` needs `cohorts` listed (`MISSING_MEMBER`); and a predicate of its parameters may hold no `ids` or `cohort` leaf at any depth (`LEAF_NOT_ALLOWED`) and at most a cohort's pack leaves. Each predicate is handed to canonicalisation as a `ViewPredicate` and resolved with the cohorts, in the view's cohorts' release, on the unit table, as a cohort of one clause (its pack leaves expanded, its caveat rules run, its refusals below `/views/<i>/params/predicates/<j>`). After it, a view whose cohorts and predicates all canonicalised gets its canonical form: the analysis id and version, cohort ids in view order (by computation id when it lists none, D284), `reference` as a position (the first by default) for `uses_reference`, `overlap` for `assumes_independent_groups`, and its canonical parameters, every default written and each predicate its canonical clause tree; its effective *k* is the largest of its cohorts', its predicates' and the floor; its packs are those of its analysis, cohorts and predicates. `validate_document` and `count_cohort` return each view that checked (`position`, `analysis`, ids, the log's status of its result id, form, readback, the caveats that need no data), and a view refused refuses `count_cohort` too, but for one of a core analysis of §9.5 that a later slice implements (`summary.distribution`, `summary.members` and `compare.columns` in M3.2, `survival.km` and `survival.cox` in M3.3): that is `NOT_SUPPORTED` at `/views/<i>/analysis`, naming the slice, which `validate_document` lists among its refusals and `run_analysis` refuses, while `count_cohort` counts the cohorts beside it. A view whose cohorts are of more than one release is `MIXED_RELEASES` at `/views/<i>/cohorts`, which resolution refuses first. A view's readback names its cohorts by position and states each predicate from the release's descriptors | §7.4 and §7.6; a predicate asked of every unit is not a set of units, so `ids` and `cohort` leaves belong in a cohort; resolving predicates as cohorts of one clause reuses phase 1 whole, so a predicate's leaf keys, caveats and SQL are a cohort's |
| D318 | `run_analysis` (M3.1; refines D300) | Takes `{"document"}`, loads and checks it as `count_cohort` does, and fails with the first refusal; a document without views is `MISSING_MEMBER` at `/views`. Each cohort the views name is counted once by computation id, and each view's crossing of its cohorts with its predicates (each predicate that holds a `lift` under the other lift rule too) once, views asking the same crossing sharing it, in one query worker (`run_crossed`), which ends `RECORD_SECONDS` before the deadline. A crossing is integer `COUNT(*)`s in DuckDB, one statement per cohort that joins its units once with every predicate and groups them by `GROUPING SETS`: per cohort and predicate, the units by truth code and reasons, with those the other lift rule changes and the flags' `BIT_OR`; per cohort, its units for which some predicate is known, and those for which none is by reason; and the units each pair of cohorts shares. No query gives a row per unit, so an answer's size depends on the number of cohorts and predicates, never on the units, and the answer cap bounds no view (an answer over it would name the widest view, `/views/<i>`); the work does grow with the units: over a million units and 16 predicates, 3 cohorts took 10 s on one thread and 5 s on four, and 6 cohorts 22 s and 8 s, within the default `query_memory` of 2 GiB (a branch per predicate and a join of all of them, as first written, took 11 s, 5 s, 23 s and ran out of memory on four threads), so 6 cohorts at that scale on one thread come near the default `query_seconds` of 25 and want more threads or seconds (M3.4 measures it). The SQL compiler must give the reference evaluator's result, digest included, and a crossing what `cross` counts from each unit's truth value. A view of an analysis with `assumes_independent_groups` whose cohorts share units, without `overlap: "allow"`, refuses the call (`COHORTS_OVERLAP` at `/views/<i>`), its `counts` the count of each pair's shared units (the canonical intersection of the two cohorts), counted in a second worker run, disclosed and issued as `count_cohort`'s; a shared part whose form would pass a cohort's cap on leaves (§7.1), which two cohorts within it can (its depth is its deeper cohort's), is not counted, and refuses the call (`LIMIT_EXCEEDED` at `/views/<i>`, naming the cap). Each view's result is recorded as one issuance, naming the call's request and `{"queries": [{"statements", "parameters"}, …]}` of its cohorts' counts and its crossing as run, and each cohort the views name as one of its own, so `explain` resolves every id a result names, for as long as the result is kept (a count issuance recorded with a kept result's request, over its release, is kept with it, and results are pruned before counts); all in one transaction under D300's rules (deadline, withdrawal, erasure mark, `log_bytes`). It returns `{results, params}`, one envelope per view in document order; each envelope's derivation `document` holds the canonical view and each of its cohorts' hashed objects, and its `source` the leaves of the view's cohorts and predicates as written. It is not read-only. Results are not cached until M3.5: every issuance is `values_from` itself. An issuance of a count, whichever tool issued it (its cohorts', and a refused call's shared parts), expires with `count_cohort`'s after `keep_count_issuances_days`; an issuance of a result after `[log] keep_result_issuances_days` (365 by default, 0 keeping them until an operator prunes, at most 36,500); `aibi prune --before` prunes every issuance before the time, and without it each kind before its period, as the log's thread does, refused only when both are kept (`Pruned` names both times, each absent when that kind is kept); migration 6 lets pruning's permit remove any issuance recorded before its time that no kept issuance recorded later names (an issuance names as `values_from` only one of its own derivation, so of its own kind) | §8.1, §11.1 and §12.2; one worker run bounds a document's queries together (§14); an overlap refused with the size of what the cohorts share tells the client how to fix it (§7.4); issuances that never expire filled `log_bytes` for good after a few thousand calls of large documents, refused ones included, with no operator remedy; a result is what a finding cites, so it is kept longer than a count, and v1 promises no availability of an id (§7.6), whose document run again gives it and its digest again; each unit's truth value for every query filled the 64 MiB answer cap at about a dozen queries over a million units, where §9.3 promises that scale |
| D319 | `compare.existence` (M3.1) | Version 1.0.0; parameters `predicates` (1 to 16 clauses) and `level` (0.95; at most 1 − 10⁻⁹, `INVALID_VALUE` above, since the normal quantile of a level within 2⁻⁵³ of 1 is infinite). Per position and predicate: the proportion TRUE over units for which the predicate is known (Wilson), and `lift_differs`; `analysed` counts the units for which some predicate is known, `excluded` every reason any predicate gave the rest, and with two or more predicates `variables` gives each predicate's. Per predicate: the test over the cohorts with known units (Fisher's for two, Pearson's chi-squared without continuity correction for more, `SMALL_N` when an expected count is below 5), the risk difference (Newcombe hybrid score) and ratio (Katz log) of every other position versus the reference; Benjamini–Hochberg over the tests computed (`family`: `tests`, `not_computed`, `suppressed`). Estimability, first reason first: shared units under `overlap: "allow"` (`overlapping_cohorts`, no between-cohort value, `COHORTS_OVERLAP`); then, under *k*, suppression; `no_units`; `degenerate_table`; `no_events` for a zero numerator of a ratio. A test not computed for overlap or suppression names no positions and the method its cohorts' number gives; one not estimable names the positions it would use | §9.5; a reason that overlap or suppression sets first says nothing of the counts behind it |
| D320 | Disclosure of results (M3.1; refines §8.4) | A result's population is disclosed position by position as a cohort count is (D297), so a cohort's counts are the same in a result as in `count_cohort`'s answer. At each position, each predicate's split of the cohort's units (TRUE, FALSE and UNKNOWN) is disclosed as a cohort count is, by `engine.suppression.suppressed`, with the cohort's `n_true` as its size (`analyses.disclosure`): a proportion shows its numerator where the TRUE units are shown, its denominator and the predicate's `excluded_units` where the UNKNOWN units are; a position whose `n_true` is suppressed shows none of its predicates' counts; `analysed` of one predicate is its split's, and of more shows nothing that combines predicates (`n`, `excluded_units` and `excluded` `null`; each predicate's `variables` entry shown as its split is): the predicates' shown counts bound a joint count from both sides (at least Σu − (m − 1)·N units with no predicate known, at most the fewest u) and a union of reasons bounds a predicate's own map, so a combined figure may be shown only where the counts shown give it alone. Every `lift_differs` is suppressed; an `excluded` map is `null` with its total, when any count is small, and wherever a map of the same units is; an estimate and interval go with their counts (a numerator of 0 shown beside a suppressed denominator withholds its estimate of 0 with them), an effect with both proportions', a test with every position's, leaving the family; a chart has a row only for an estimate and interval shown. A value whose counts are suppressed is `suppressed`, never `no_units` or `zero_denominator`. Under *k*, `UNKNOWN_EXCLUDED` is carried wherever an UNKNOWN count is suppressed; caveat messages quote no count but `LIFT_DIFFERS`', none under *k*. `test_disclosure` checks, over every split of cohorts of up to 14 units and *k* from 3 to 5 (and 2 for a predicate alone), that what a result shows of a predicate at one position, alone or beside its `not`, `known` and `unknown`, leaves every count it does not show two values or more (positions are disclosed apart, so positions that are unions of others, allowed with `overlap: "allow"`, give each other by difference, which §8.4's Limits disclaim), and over `compare.existence`'s whole output for a predicate alone and beside its `not`, and for two and three independent predicates across two positions (every split of 6 units by two predicates answered TRUE, FALSE or UNKNOWN for either of two reasons, and by three answered TRUE, FALSE or UNKNOWN, as marginals, which independent predicates take in every combination, beside the other position's 12 units: no count from 1 to *k* − 1 or combining predicates shown, and each predicate's split and reasons left two values or more unless the counts shown give them), with the second review's cases of 20 units | §8.4; the first rule linked a numerator only to its denominator, so a numerator of 9 shown beside a suppressed denominator in a cohort of 10 gave away its one unit not TRUE, and that it was UNKNOWN; a split disclosed as a cohort count has D297's symmetry, so `not P` is disclosed as `P` with TRUE and FALSE swapped and the same count under two names is suppressed in both; `no_units` beside a suppressed denominator would say it is 0; a collapse of a predicate is another predicate, as §8.4's Limits have it for cohorts; `analysed` disclosed as a split of its own was bounded from below by the predicates' own counts, so that two predicates each UNKNOWN for 11 units of 20 gave its suppressed 2 units with neither known, and its union of reasons pinned a predicate's suppressed map (1 unit not assessed beside 10) |
| D321 | Statistical methods and the R reference (M3.1) | `analyses.stats` computes each method from counts in doubles, in a fixed order, sums by `math.fsum`, no DuckDB aggregate: the normal quantile as `statistics.NormalDist` (AS 241), Wilson as `prop.test(correct = FALSE)`, Newcombe's method 10, Katz, Fisher's two-sided test as `fisher.test` (tables no likelier than observed to a relative 1e-7, by the ratio of consecutive hypergeometric probabilities from the mode), Pearson's chi-squared as `chisq.test(correct = FALSE)` with the regularised upper gamma (series, or a Lentz continued fraction), BH as `p.adjust`. Fisher's probabilities, relative to the mode's, are held as a mantissa and a binary exponent (`math.frexp`), which scaling by powers of two leaves exact, so they give a double's bits wherever it is normal and a p-value is 0 only below the least double (`[[1887, 1414], [2946, 152]]` gives R's 4.81e-306, where a double tail cut at 1e-300 gave 0). The counts `run_analysis` gives these methods are DuckDB's integer `COUNT`s and `BIT_OR`s (D318), exact and the same under any thread count. `tests/core/analyses/reference/existence.R` (R 4.3.3) wrote `existence.json`, which the tests read: every value agrees to 1e-10 relative | §9.3 and §13.4; basic operations in a fixed order give every platform the same bits, and R is the reference §13.4 names |
| D322 | Charts (M3.1) | `analyses.charts` writes each chart from the result's values after the disclosure pass: Vega-Lite with inline `data.values`, one row per value shown (none for a `null`), the cohort's label as data, no transform, expression, URL or `$schema`, and only fields it writes. `compare.existence`'s: a bar per cohort and predicate with its interval as a rule, faceted by predicate, cohorts in view order | §8.5; `$schema` is a URL a loader might fetch, and a chart must never draw what the result does not show |
| D323 | Lift in a view's predicates (M3.1; refines D200) | A predicate that holds a `lift` is also run with every lift flipped, and `lift_differs` counts, per position and predicate, the cohort's units whose truth value changes; `LIFT_DIFFERS` is carried when any cohort's or predicate's count is above zero or suppressed, its message giving the counts shown, and the envelope checks that it is | §6.5: a predicate's answer depends on the lift rule as a cohort's does |
| D324 | M3.2 in four parts (M3.2; refines D315) | M3.2 is delivered in four parts, each a PR with its tests and a sub-issue of #18: M3.2a (#41) a view's variables and aggregates (§9.2) in the reference evaluator and the SQL compiler, materialised over a view's cohorts by `run_analysis`, and `summary.distribution`; M3.2b (#45) `summary.members`; M3.2c (#46) `compare.columns` with its bootstrap, per-category denominators for multi-valued categorical columns and `count: "rows"`; M3.2d (#47) running packs' analyses on materialised inputs, and pack leaves in a variable's `where`. Until its part lands, a view of `summary.members` or `compare.columns` is `NOT_SUPPORTED` at `/views/<i>/analysis` naming it (`registry.LATER`), as are a pack's analysis and a pack leaf in a variable's `where`, naming M3.2d, and a multi-valued categorical column without an `aggregate` and a variable's `count: "rows"`, naming M3.2c (at `/views/<i>/params/columns/<j>/count`) | D315's M3.2 held four analyses' methods, materialisation and the disclosure of values; `summary.distribution` alone exercises variables, aggregates, materialisation and the disclosure of what values show end to end, so the rest builds on a reviewed base |
| D325 | Variables (M3.2a; refines §9.2 and D290) | A view's column is a variable, `{column, via?, aggregate?, values?, where?, lift?, empty?, bins?}` (`schema.analyses.Variable`): `values` goes with `some` and `every` and they need it, `where` with `count`, `max`, `min` and `mean`, `empty` with `max`, `min` and `mean` (else `CONFLICTING_MEMBERS`); `bins` are 2 to 65 increasing finite edges (`INVALID_VALUE`, `bins`); `count` takes `"rows"` alone, refused `NOT_SUPPORTED` until M3.2c. Phase 2 refuses `ids` and `cohort` leaves in its `where` (`LEAF_NOT_ALLOWED`) and pack leaves (`NOT_SUPPORTED`, M3.2d), and hands it to canonicalisation as a `ViewVariable`, resolved on the unit table in the release of the view's cohorts with the cohorts, its refusals below `/views/<i>/params/columns/<j>`: a concept reference is `NOT_SUPPORTED` (M6); an unknown column `UNKNOWN_COLUMN`; an undeclared datatype `UNDECLARED_DATATYPE` and an identifier column where row ids are not allowed `ROW_IDS_NOT_ALLOWED`, but for `count`, which reads no value; a multi-valued column without an aggregate `AGGREGATE_REQUIRED`, or `NOT_SUPPORTED` (M3.2c) for a category, boolean or list; an aggregate the column does not take `AGGREGATE_NOT_ALLOWED` (`mean` of a non-number, `max` or `min` of what is neither a number nor an ordered category, a numeric aggregate of a list, any of a single-valued column); an `empty` its column does not take `INVALID_CONSTANT`; scope columns the last step's `where` does not restrict to listed values, or any at an earlier step, `OPEN_SCOPE`; and the path and conditions a cohort's caps. It resolves to one of three kinds (`ResolvedVariable`): a column of the unit or of the row its up steps look up; a question (`some`, `every`), an existence question on the unit table; an aggregate, a chain of single-step `exists` (each but the last with the next as its one `where` and the variable's `lift`, the last with its conditions and no lift) and the lookups after the last down step. Its canonical form (`canonical.variable_form`) is `{"column", "via"?}`, `{"aggregate", "question"}` with the canonical clause tree, or `{"aggregate", "column", "rows", "lookup"?, "empty"}` (`empty` `"exclude"` by default, none for `count`); a view's canonical parameters add a number's `bins` (`null` for none). Erasure (D290) reads the members that hold values of its column, `values`, `empty` (but `"exclude"`) and, but for `count`, whose bins bound numbers of rows, each edge of `bins`, as constants on that column, as a `value` leaf's `values` and range bounds are, in a view as written and in a result's canonical parameters alike; its other members are structure or clauses. Its readback states the column and lookups, the function, the rows at its last down step with their conditions and each earlier step pooled through, and what a unit without a value takes | §9.2 left the variable's shape and its refusals to M3; one resolver and one canonical form for all three kinds give variables the same ids, leaf keys and caps as cohorts and predicates, and `count` reads rows, not values, so it needs no datatype and discloses no identifier |
| D326 | Aggregates in the evaluator and the compiler (M3.2a; refines §9.2, §13.3) | `engine.variables` gives each unit a value or its exclusion reasons and flags. A column: its cell's value when PRESENT, else `NOT_APPLICABLE`, `NOT_ASSESSED` or `NO_INFORMATION` by state, `NO_PARENT` where a step meets a null or dangling key. A question: TRUE or FALSE, UNKNOWN excluded under its reasons. An aggregate pools (`Evaluator.pool`): at each step the ups are followed (`NO_PARENT`), a parent outside the parent scope is `OUT_OF_SCOPE`, and one not closed for the values the last step's conditions admit on the scope columns adds its closedness reasons (`COVERAGE_PROPOSED` for a parent in scope over proposed coverage, `SCOPE_PARTIAL` for one closed for listed tuples); at an earlier step each child is pooled through and dropped when unknown only for reasons its lift drops, none kept adding `NOT_COVERED` and a kept unknown child its reasons; at the last step a row whose conditions and record filter are TRUE is pooled, UNKNOWN adds their reasons. A unit with a reason is excluded under all of them; otherwise `count` counts its rows, and `max`, `min` and `mean` read each row's value through the lookups, skip NOT_APPLICABLE, and are excluded `NO_INFORMATION`, `NOT_ASSESSED` or `NO_PARENT` for the rest, an ordered category compared by listed position (a value outside the list `NO_INFORMATION`), `empty` or `NO_ROWS` with no value; `mean` pools, exact for integers and `math.fsum` over doubles, divided once, the doubles scaled by the power of two that brings the largest magnitude into [0.5, 1) and back, which is exact and cannot overflow; doubles are normalised (`-0.0` is `0.0`). Variables compute no `lift_differs`. The compiler (`sql.compile_materialised`) gives one `UNION ALL` statement per cohort and variable, whose rows are tagged: units per value, units excluded per set of reasons (their bits in `ExclusionReason` order) with the flags' `BIT_OR`, units taking `empty`, and for `max` and `min` the units of each unit's greatest or least value, which DuckDB picks (`MAX`, `MIN`: exact, so allowed over doubles, §9.3), and for `mean` each unit's pooled rows grouped by value, which the server aggregates; with two variables or more, one statement per cohort counting its units by whether some variable has a value and, for none, the bits of all their reasons. Every count is an integer `COUNT` and values are grouped, never summed or averaged, by DuckDB (§9.3). A Hypothesis property holds the compiler to the evaluator on every variable kind over random cities, and a second one on multi-valued categories alone, ordered ones under `max` and `min` among them, over rows NOT_APPLICABLE, NOT_ASSESSED, outside the list or null (an ordered category's NOT_APPLICABLE row is skipped in SQL as in the evaluator, not NO_INFORMATION) | §9.2's pooling, made executable as §13.3 asks, with every rule the evaluator applies checked against SQL; averaging in DuckDB would put a DOUBLE aggregate into a digest, which D294 forbids |
| D327 | Materialised answers (M3.2a; refines D293, D318) | A query's value columns (`Query.values`) may hold doubles (`d`) or UTF-8 text (`s`: the total length, each value's length, then the bytes), which the worker packs by what DuckDB returns and the reader checks (text decoded strictly); every other column stays integers, and `Rows.columns` gives only integer columns. `run_analysis` counts each cohort once, crosses each view's predicates once and materialises each view's variables over its cohorts once (views with the same cohorts' computation ids and variables' canonical forms sharing it), all in one worker run (`queries.run_views`), a variable a view reads twice (one canonical form) materialised once; each result's issuance records its cohorts' counts and then its crossing's or its materialisation's statements. A materialisation's answer grows with the distinct values its units hold and, for `mean`, with the units, so the answer cap bounds it: an answer over the cap, or a run over the worker's `query_seconds` or `query_memory`, names the view whose materialisation is widest (cohorts × variables), else the widest crossing. The server reads a materialisation's answer, for `mean` a list per unit, outside `query_memory`, bounded by the answer cap, and stops at the call's deadline (`CallerDeadline`, looked at every 65,536 rows, and every 65,536 units while a `mean`'s units are averaged), and `summarise` looks at it before each column, so a call overruns it by one column's statistics at most (about 4 s over a million distinct values); M3.4 (#43) measures it at a million units (at 1,000,000 units with 1,500,000 rows, a `mean` of doubles gives 1.5 million rows, read in about 7 s; `max` and `min` give a row per distinct value) | D318's answers were integer counts alone; a variable's values are doubles and text, and grouping by value in DuckDB then aggregating in Python keeps D294's rule while the answer carries only distinct values |
| D328 | `summary.distribution` (M3.2a) | Version 1.0.0; `requires` 1 to 6 cohorts and at least one column; parameters `columns`, 1 to 8 variables (`MAX_VARIABLES`, `variables`). A variable is categorical (category and boolean columns, `max` and `min` of an ordered category, `some` and `every`, whose categories are `false` and `true`) or numeric (number, integer and time-offset columns, `count`, `mean`, `max` and `min` of numbers); any other column is `NOT_SUPPORTED` at `/views/<i>/params/columns/<j>/column`, listing the datatypes it takes, `bins` on categories `INVALID_VALUE`, and under *k* a number without `bins` whose column declares no range `MISSING_MEMBER` at `…/bins` (§8.4). Per position and variable: categories are listed as declared, zeros included, then the other values found in canonical order (UTF-16), at most 150 (`MAX_CATEGORIES`; more refuse the call, `LIMIT_EXCEEDED` at `…/columns/<j>`, `categories`), each a proportion of the units with a value (`counts: "known"`); numbers give `n`, the mean (exact sum of integers, or `math.fsum`, divided once), the sample standard deviation (`zero_variance` for fewer than two values or all equal), median and quartiles (R's type 7), minimum, maximum and a histogram over the view's `bins`, else the column's declared range in *B* = 10 equal bins (none for `count`), else without *k* the data's (10 bins between the least and greatest values, one bin for one value), bins [eᵢ, eᵢ₊₁), the last closed, with open bins below and above, `edges_from` saying which; with no value every statistic and a data histogram is `no_units`. `analysed` gives, per position, one variable's units with a value (`n`), excluded (`excluded_units`) and by reason, or with several, `variables` and the units some variable has a value for. `values.view` is `{}`. A numeric variable's values and standard deviation lie within ±(2^53 − 1), as an output's numbers do (§8.2), or the call is refused (`NOT_SUPPORTED` at `…/columns/<j>`), which arises only without *k*, the one case that computes them; sums of doubles are scaled so that none overflows (`stats`). Under *k*, a call that reads a column's values with other edges than it has already, in the same view or an earlier one, by any variable (the column, or `max`, `min` or `mean` of it, whatever rows), or a `count` with other edges than a `count` of the same rows has (its path, table and `where`, whatever column it names), is `CONFLICTING_MEMBERS` at the later one's `bins`, after the column's kind is checked (so `bins` on a category are `INVALID_VALUE` first); the edges compared are those the histogram takes (its `bins`, else its declared range's), so a view that gives none and one that writes the range's own edges agree; without *k* every count is shown and the rule does not apply (D329). A declared range too narrow for *B* distinct edges (a few ulps wide) has its repeated edges collapsed. The key holds the release, whose columns views over one dataset share; views over several (M6) may need another. Caveats: the cohorts' (§8.3), `UNKNOWN_EXCLUDED` naming the values where a unit is excluded for another reason than NOT_APPLICABLE (always under *k*), the flags of the values' truth values, `UNCONFIRMED_SEMANTICS` for fields the variables read, `SUPPRESSED` and `NOT_ESTIMABLE`. The readback names cohorts by count and states each column (D325). Methods held to R by the reference script (`mean`, `sd`, `quantile(type = 7)`, `cut(right = FALSE, include.lowest = TRUE)`); the mean is the exact mean correctly rounded, which R's two-pass `mean` matches but under catastrophic cancellation | §9.5's descriptive analysis over one variable kind at a time; the range divides the histogram so that its edges do not depend on the data, which §8.4 requires under *k*; a view refused for a column's categories is safer than a result that cuts them |
| D329 | Disclosure of distributions (M3.2a; refines §8.4 and D320, revises §8.4's and D271's merge rule) | At each position, whose population is disclosed as a cohort count is, a variable's split of the cohort's units into those with a value and those excluded is shown whole or not at all (either part 1 to *k* − 1 hides both), its `excluded` map only with it and with no small count, and nothing of a position whose `n_true` is suppressed; with two variables or more nothing that combines them. Categories, in their listed order, and histogram bins are merged: repeatedly, the leftmost with 1 to *k* − 1 units merges with the nearest non-empty one on its right (on its left when none is), empty ones between included, until none has 1 to *k* − 1 units or one non-empty one is left; each merged category row lists the values it merges. A category column's values that it does not declare are one row after the declared ones, `other_values`, that names none of them, listed whatever the units hold (zero included), so the rows, and whether they pass `MAX_CATEGORIES`, depend on the declaration alone. No statistic computed from the values themselves is reported (mean, standard deviation, median, quartiles, minimum and maximum, all `suppressed`, as a value whose counts are): values are not counts, so the count rule cannot protect them. Each quartile is given as the index of the merged bin that holds it, read off the bins' counts, `null` (`suppressed`) where the two order statistics it lies between are in two bins. Under *k*, a call reads a column's values with one set of edges, across its views and whichever variable reads them, and the rows a `count` counts with one (`CONFLICTING_MEMBERS` otherwise), since two histograms of one quantity give by difference the counts merging hides; across calls, and between variables whose units or rows differ over the same bins (another `empty`, `where`, `lift` or aggregate), §8.4's Limits name it, as they name cohorts that differ by few units. The catalogue's histograms merge by the same rule. `test_distribution_disclosure` checks, over every way a cohort of up to 12 units can split over three categories and units excluded for either of two reasons or both, and over three bins, the open bins and exclusion, each bin at one value, under *k* from 3 to 5, that no count shown is 1 to *k* − 1 and that outputs that read the same leave every category's or bin's count, `n`, `excluded_units` and each reason's count two values or more unless what they show gives it; that a numeric output is the same whether each bin's units hold one value or two, so nothing it shows depends on values within a bin beyond the bins' counts; and, over two declared categories and two undeclared values, that no output names an undeclared value or pins its count to one from 1 to *k* − 1; under *k* = 2, where 1 to *k* − 1 is one value, two merged units of 1 each are read off, as §8.4 allows for cohorts | Brute force found that the old rule, the smallest first toward the neighbour with fewer units, gives its order away (3, 2, 2, 2 and 1 units in five bins under *k* = 3 are the only counts it shows as 3, 4 and 3), and that a pooled row of categories is read off whenever its count is 1 or *k* − 1 per category or it pools one category, and suppressing those cases leaves them the only ones suppressed; merging left to right leaves every merged count two values or more over every case tried (2 to 7 bins of up to 20 to 12 units, *k* from 3 to 6); a quantile between two bins would be placed by the values, which merged bins' edges bound, a mean or standard deviation gives what counts hide whatever merged (the review's nine counts of 1 and one of 2, hidden by bins [0, 1, 2, 3], are the only ones with the mean 1.1 and the standard deviation 0.316 that bins [0, 10] would show, and six units aged 47 would show 47 and a standard deviation of zero), and a value held by one unit, named in a merged row, published what that unit holds |
| D330 | Charts of distributions (M3.2a; refines D322) | `summary.distribution` has one chart per column, in parameter order: for categories a bar per row with its proportion, labelled by its values each quoted as a JSON string (merged ones joined by `, `) and the row of undeclared values by `other values` unquoted, so that no label is another's; for numbers a bar per histogram bin as merged, labelled by its interval (`[0, 30)`, `(-∞, 0)`, `[60, 100]`) in order of its edges, so that a bin merged at one position is not drawn as another's, with its edges in the tooltip; one facet row per cohort, labels as data; a suppressed table or histogram has no row | D322's rules, one chart per variable because a variable's kind fixes its encoding; Vega-Lite draws equal labels as one value, so a category named `a, b` or `other values`, or bins indexed after merging, would have been drawn as another (review round 2) |
| D331 | `summary.members` (M3.2b; refines §9.5 and D324) | Version 1.0.0; `requires` exactly one cohort (`{"role": "cohorts", "min": 1, "max": 1}`), which the view lists in `cohorts` (`INVALID_VALUE` at `/views/<i>/cohorts` for more) or which is the document's only one (`MISSING_MEMBER` there otherwise); parameters `offset` (0 to 2^53 − 1, 0 by default) and `limit` (1 to `MAX_MEMBERS`, 1,000; 100 by default), canonical as `{"limit", "offset"}`, which erasure reads as settings, never constants (D290). Its value at its one position: `columns`, the unit table's key columns by descriptor id, as data; `keys`, the members' keys (its units for which it is TRUE) from the `offset`-th, at most `limit` of them, each its values in key order as stored, written as the canonical form writes constants (a boolean; an integer, beyond ±(2^53 − 1) a decimal string; a double, an integral one as an integer; text, a date as `YYYY-MM-DD` and a datetime in UTC, as data); `offset`; and `more`, whether members follow. Keys are ordered by their RFC 8785 serialisation compared as UTF-16 code units (`engine.members.ordered`), as step 8 orders an `ids` leaf's members, never by DuckDB's collation. `analysed` counts the members, none excluded, since a key column's cells are all PRESENT (§13.2); `values.view` is `{}`; the caveats are the cohort's, and `UNCONFIRMED_SEMANTICS` for the unit's `primary_key` and each key column's `datatype` where they are not confirmed (a datatype nobody declared, read as text, is `undeclared`); the readback states the key columns, the order, the offset and the limit, as data. It computes no statistic, so it has no R reference, its order being held to an independent sort of the serialisations' UTF-16 bytes, and no chart (D330): a list of keys holds no number to draw | §9.5 left the parameters, the shape and the order open; the canonical form's order is the one order the server already defines for keys, the same on every platform whatever their types, and a page's keys can be written into an `ids` leaf as they are; one cohort per view keeps an offset a position in one list |
| D332 | Members and disclosure (M3.2b; revises §8.4's rule for cohorts smaller than *k*; refines D59, D297 and D329) | A view of `summary.members` is refused (`ROW_IDS_NOT_ALLOWED` at `/views/<i>/analysis`, its alternatives `count_cohort` and the core's other analyses) where its dataset allows no row ids, and under any effective *k*, a deployment floor included, whatever its cohort's size; in phase 2, so that `validate_document` lists it, `count_cohort` refuses it as it does any refused view (D317) and no query runs. Under *k*, keys may be written into a document (`ids` leaves, and predicates on identifier columns where row ids are allowed) and are never read out of an output, as a distribution of an identifier column names none of its values (D329's `other_values`); `members.list_members` refuses a *k* itself. `test_members_disclosure` checks, under every *k* from 2 to 5 set by the dataset or by the floor, both, and without row ids, over cohorts of 0 to 8 units, that a view is refused and that neither the refusal nor a cohort's form names a key (and, over a store, that `validate_document`, `count_cohort` and `run_analysis` name none), and without either, over every split of up to 6 units into TRUE, FALSE and UNKNOWN units and pages of 1, 3, all and 1,000 keys, that the whole output names each member's key once, in order, and no other unit's | §8.4 listed keys for cohorts of *k* units or more whose size the pass shows, but a key is a value one unit holds, which no rule over counts protects: lists of cohorts whose sizes are each shown give by difference the keys of a set of any size. Over 24 units under *k* = 4, `X` of 8, 13 and 3 units shows only its 13 (D297), and the lists of every unit (24 keys), of `not X` (13) and of `X and known X` (8, a collapse, §8.4's Limits), each of whose sizes is shown, would name the 3 units whose `X` is unknown, so the differencing the Limits disclaim for counts would name the units behind every count the pass hides, in one call; D329 names no value that few units hold for the same reason |
| D333 | Keys in the evaluator and the compiler (M3.2b; refines D318 and D327) | `engine.members.keys` gives a cohort's members' keys by the reference evaluator, and `sql.compile_members` by SQL: one statement per cohort that joins its truth values to the unit table's key columns, a row per member by `rid`, each key column as stored (text and doubles as value columns, D327), a date as its days since 1970-01-01 (`date_diff`) and a datetime as its microseconds since the epoch (`epoch_us`), which the server reads back into dates and datetimes in UTC; a `NULL` or a value of another type is a fault (`QueryError`). `run_analysis` lists the keys of each cohort a view of it names once, by computation id, in its one worker run (`queries.run_views`, `members`), and the server orders them and takes the page, so a page reads every member's key and the answer grows with the cohort (8 bytes a unit for an integer key), which the answer cap bounds: an answer over it, or a run over its seconds or memory, names the first view that lists keys, which reads a row per member, before the widest materialisation. The server reads and orders the keys outside `query_memory`, bounded by the answer cap, and stops at the call's deadline (`CallerDeadline`, looked at every 65,536 keys). Each result's issuance records its cohort's count and then the listing's statement and parameters, which hold no key, so the derivation log keeps no key a result listed (D318) and erasure has none to find beyond the constants a document wrote (D290); the result cache (M3.5) caches no `summary.members` result, whose values are keys. A Hypothesis property holds the compiler to the evaluator over keys of every stored type (integers beyond ±(2^53 − 1), doubles, booleans, text outside the Basic Multilingual Plane, dates and datetimes), of one column and of two | D318's queries gave no row per unit, and a list of keys is one by nature, so the answer cap bounds it as it bounds D327's `mean`; ordering on the server keeps §7.6's order, which no SQL collation gives; a key kept in the log or a cache would outlive the erasure of its unit, whose redaction reads the constants documents write |

---

## Appendix B. Change log

- **v0.8.16** — `summary.members`, M3.2's second part (D331–D333): a view of it names one cohort
  and pages its members' unit keys by `offset` and `limit`, each key as stored, in the canonical
  form's order (§9.5); it is refused under any disclosure setting, not only for cohorts smaller
  than *k*, since lists of cohorts shown whole give by difference the keys of a set of any size
  (§8.4, §11.1); the reference evaluator and the SQL compiler list keys, read and ordered on the
  server within the answer cap and the call's deadline, and no key reaches the derivation log
  (§14).
- **v0.8.15** — Variables, aggregates and `summary.distribution`, M3.2's first part (D324–D330):
  M3.2 comes in four parts (§15, §10.1); a view reads columns as variables, resolved, canonicalised
  and read back like predicates, with `AGGREGATE_REQUIRED`, `AGGREGATE_NOT_ALLOWED` and `OPEN_SCOPE`
  (§9.2); aggregates pool as §9.2 says in the reference evaluator and the SQL compiler, which groups
  values, picks `max` and `min` and leaves means to the server (§9.3); a query's answer carries
  doubles and text in its value columns, and `run_analysis` materialises variables in its one worker
  run; `summary.distribution`'s parameters, values and estimability (§9.5), its methods held to R;
  categories and histogram bins merge from the left, in results and in the catalogue, and quartiles
  are placed by the merged bins' counts, since brute force found the old rules pinned counts (§8.4);
  and one chart per column (§8.5). After the first review, within v0.8.15: under *k* no statistic of
  the values themselves is reported, only merged bins' counts and quartiles' bins, since values are
  not counts; a category column's undeclared values are one unnamed row under *k*, and the category
  limit depends on the declaration alone; a variable's `values` and `empty` are erased as constants
  on its column; a view reads one variable with one set of bins, and a variable read twice runs
  once; values and standard deviations beyond ±(2^53 − 1) are refused and sums of doubles scaled; a
  run over its seconds or memory names the view; `count: "rows"` is `NOT_SUPPORTED`; and §8.4's
  Limits name distributions' differencing and the catalogue's pooled categories (#49) (D324–D329).
  After the second: a variable's `bins` are erased as constants on its column too, but a `count`'s
  (D325); a call reads a column's values with one set of bins across its views and variables, and
  §8.4's Limits name calls (D328, D329); `max` and `min` are picked in SQL, which §9.3 allows over
  doubles since they compute nothing, and the server reads a `mean`'s rows by the call's deadline
  (D326, D327); chart labels quote values and name bins by their intervals (D330). After the third:
  an ordered category's NOT_APPLICABLE row is skipped by `max` and `min` in SQL as in the evaluator,
  and a property draws multi-valued categories (D326); the server's averaging and `summarise` look
  at the deadline too (D327); the one-set-of-bins rule holds under *k* only, compares the edges a
  histogram takes and keys a `count` by the rows it counts, and a narrow range's repeated edges
  collapse (D328, D329); and §8.4's Limits name variables whose units differ over the same bins.
- **v0.8.14** — The analysis registry, phase 2, `run_analysis` and `compare.existence`, M3's
  first slice (D315–D323): M3 comes in five slices (§15); the registry holds the core's entries
  and the packs', matched for applicability (§9.4, `describe_dataset`, `list_analyses`) and served
  as resources, a pack's analysis run from M3.2 (§10.1); phase 2 checks each view against its
  entry and resolves its predicates as cohorts of one clause (§7.6, revising D284);
  `run_analysis` runs a document's views in one worker, refuses cohorts that share units with the
  count of what they share, and issues each result and cohort (§11.1); `compare.existence`'s
  parameters, values and estimability (§9.5), its methods held to R, the disclosure pass for the
  counts an analysis adds and a suppressed value's reason (§8.4), charts without `$schema` (§8.5),
  and `LIFT_DIFFERS` for a view's predicates (§8.3). After the first review, within v0.8.14: each
  predicate's split of a cohort's units is disclosed as a cohort count is, and nothing shown of a
  predicate or its `not` pins a suppressed count (D320, §8.4); issuances of results expire after
  `keep_result_issuances_days` and `run_analysis`'s counts with `count_cohort`'s, and an operator
  prunes them all (D318, §12.2, refining D300); a query two views ask runs once, the units two
  cohorts share are held to a cohort's caps, `level` is at most 1 − 10⁻⁹ (D319), and
  `available_with_caveats` needs `min` confirmed descriptors (D316). After the second review,
  within v0.8.14: with two predicates or more a result shows nothing that combines them (D320,
  §8.4), and whether cohorts share units is named among the Limits; `run_analysis` crosses cohorts
  with predicates in integer counts, whose answer never grows with the units, and prunes results
  alone while counts are kept (D318); the core's analyses of later slices are `NOT_SUPPORTED`,
  and `count_cohort` counts beside them (D317); and Fisher's test no longer rounds a p-value far
  in the tail to 0 (D321). After the third review, within v0.8.14: §8.4 promises no pinning at
  one position, its Limits naming positions that are unions of others and whether a view's
  result was issued; a crossing is one pass per cohort by `GROUPING SETS`, and `query_memory`
  should grow with `query_threads` (D318, §14); a kept result keeps its cohorts' counts (D318,
  §12.2); and D320's brute force runs over marginals at sizes that show counts.
- **v0.8.13** — The read-only catalogue page (D311–D314): `GET /` and `GET /datasets/<id>`,
  rendered by the server from `search_catalog` and `describe_dataset` through the same tool calls,
  as HTML without a script; every text escaped, its control and format characters shown as marked
  escapes, and data in `<bdi>`s; a content security policy that allows its one stylesheet by
  hash, and images and connections to the server only; every answer at the page's paths a page
  (refining D255 and D265); and a link from another site let through request protection
  (refining D257). §11.1 names the page, §12.4 says it is the server's until the React UI of M5,
  §12.5 puts it in `api/`, and §14 says how it renders text and which navigations pass. After the
  first review, the page's refusals are all pages, its responses `no-store`, a link from another
  site opens it (D314), format characters are escaped, and the elements, attributes and links it
  can write are allow-listed, within v0.8.13. After the second, a navigation from another site
  must be the person's (`Sec-Fetch-User: ?1`), page responses carry
  `Cross-Origin-Opener-Policy: same-origin`, and page views have a rate of their own (`page`,
  refining D259) that never spends the agents' `api` rate; links are built from an allow-listed
  character set and an unusable root path is refused; escapes are marked a run at a time. After
  the third, pages are rendered in the call's worker thread, a dataset's page shows at most 50
  tables and what starts from them, preflights at the page's paths spend the `page` rate, texts
  and lists from data are cut, and D311 and D314 state the page's cost and what bounds a hostile
  page (the `page` bucket, not `Sec-Fetch-User`).
- **v0.8.12** — Database snapshots after their second review (D308–D310, amending D305–D307): a
  MySQL snapshot reads its catalogue and rows through the attached catalogue in the scanner's one
  read-only transaction, at an isolation level that reads a snapshot, in UTC, refusing tables of
  engines without transactions, decimals over 38 digits, zero dates and table names that differ
  only in case; every Postgres `numeric` and `money` is read as exact text and its catalogue
  filtered on the server; rows are ordered by their bytes whatever the collation; SQLite's
  `VIRTUAL` generated columns are refused; a driver's failure names its table; names that are not
  Unicode text are escaped in notes and refusals, and skipped relations noted up to
  `import_tables`; foreign keys match names ignoring ASCII case alone and are not kept when they
  repeat a column; a connection's file must be in a directory no one else can write, and a
  Postgres URL's query cannot name another server or database. D306 says what reading a SQLite
  file in WAL mode makes beside it. After the third review, within v0.8.12: MySQL database names
  that differ only in case and Unicode-case collisions are refused; a URL holding `#` is refused
  and a Postgres query read as libpq reads it; foreign keys on the same columns in another order
  are noted; a partition of another schema's table is noted; arrays of `numeric` and `money` are
  read as lists of text; SQLite's `-0.0` and `0.0` are ordered by sign; the worker names at most
  `import_tables` skipped relations and lists SQLite's from `sqlite_schema`; a connection's file
  must be in a directory the server's user or root owns, and refusals name it by its location.
  After the fourth review, within v0.8.12: a server's URL is read by the server's own grammar and
  the driver given a connection string built from what it names (D310); MySQL name collisions
  follow the server's collation; partitioned tables with a foreign partition are skipped and
  noted, partitions judged by their root; SQLite's virtual tables are found by their root page;
  a connection's file must be the server's user's or root's. After the fifth review, within
  v0.8.12: a server's database and user are bounded as the server keeps names, and the snapshot
  reads back the database its session opened, refusing another, from which the provenance is
  recorded (D310); numeric hosts must be IPv4 addresses in dotted decimal, and hosts are recorded
  in canonical form; no persistent secret is read; a connection's file others can write is
  refused; a schema holding `/` is refused; SQLite virtual tables are found by their `sql` too,
  and Postgres partitions pending detach are noted (D309). After the sixth review, within
  v0.8.12: MySQL names DuckDB's scanner cannot quote safely (a backtick, a backslash, a control
  character, and a `'` in the database) are refused before any row is read, since a crafted
  table name read another database's rows (D308). After the seventh review, within v0.8.12: a
  Postgres session resolves names in `pg_catalog` alone with `row_security` off, its catalogue
  queries name `pg_catalog` throughout, and tables with row security or virtual generated
  columns are noted and not read (D309, D310); a MySQL `localhost` needs `socket`, a `?` in any
  MySQL value is refused, the URL's database must be a `SCHEMATA` name exactly, and a view's
  name is not held to the scanner's quoting (D308, D310). After the eighth review, within
  v0.8.12: a Postgres table whose column's read path would run a function outside `pg_catalog`
  is skipped, the session fixes every output setting, each base table is locked `ACCESS SHARE`
  before its columns are read, and a zoned time is read as text or refused (D306, D309, D310).
  After the ninth review, within v0.8.12: `quote_all_identifiers` is fixed too, a multirange is
  followed to its range, a `timetz[]` is read as text, an ordinary table is locked `ONLY`, and a
  lock waited for past half of `reader_seconds` is refused naming its table (D306, D309, D310).
  Round 9's follow-up, within v0.8.12: since a cast lookup ignores the transaction's snapshot, a
  cast a schema owner adds mid-snapshot could still run; every Postgres column of a type outside
  `pg_catalog` is now read through the type's output function, resolving no cast, and only a user
  base type is skipped (D309). After the tenth review, within v0.8.12: an installed extension's
  type (`citext`, `hstore`…) is read through its output function too, and only a base type whose
  I/O is neither `pg_catalog`'s nor an extension's is skipped (D309).
- **v0.8.11** — Database snapshots (D305–D307): named connections as import sources, resolved
  when the import runs, with provenance of host, database and schema only; snapshots read in the
  import's worker process in one transaction, SQLite with SQLite's own defensive reader rather
  than DuckDB's scanner (a deviation from §13.1's "through DuckDB"), DuckDB files hardened, and
  Postgres and MySQL through DuckDB's signed scanners, bundled as wheels, in a session locked
  once it has attached them; declared keys, foreign keys, types and comments `imported`. §5.2
  says what a database's location is, §10.1 what a validator's source is, §12.4 the stack, §13.1
  what a snapshot imports, and §14 how SQLite files and snapshots are read. After the first
  review, a server's rows are read on the server by the catalogue's names (`ONLY` for a Postgres
  parent), decimals wider than 38 digits as their exact text, MySQL's `TINYINT(1)` as an integer
  through one connection; foreign keys to another schema or to a column that is not the
  parent's key are noted; DuckDB's generated columns and control characters in comments are
  refused; and the files beside a SQLite or DuckDB file, and those the reader opens, confined.
- **v0.8.10** — Choices the query tools settle (D296–D304): readbacks from templates, stating every
  step, condition, quantifier, lift rule, unit, coverage, record filter, parent scope and negation
  in the canonical order; the disclosure pass over cohort counts, with `NOT_ESTIMABLE` for an empty
  unit table and the digest after the pass (refines D288); outputs checking their own digests;
  `validate_document`, `count_cohort` and `explain`, with their requests, outputs, refusal paths
  and issuances; where the query tools run (amends D285's last sentence); document translators;
  and documents in tool requests. §7.7 says what a readback is a function of, §8.1 what
  `count_cohort` returns and that digests are checked, §8.4 the linked sets of a cohort count,
  §8.6 where a tool's document refusals point, §10.1 the translator's contract, §11.1 the three
  tools, and §14 where canonicalisation runs. Review round 1 amended D296, D297 and D300–D303,
  D285's last sentence and D289's issuance ids within v0.8.10, adding no numbers: a unit table of
  1 to *k* − 1 units suppresses its size and, under max(*k*, 4) units, every count, and one more
  count is suppressed wherever what is shown would pin one (§8.4); a call's issuances are recorded
  in one transaction only while it can be answered, its request stored once in the log's texts
  (migration 5), the log pruned by `[log]`'s period and by `aibi prune`, and bounded by
  `log_bytes` (§12.2, §14); the queries end by the call's deadline, and the work around them is
  bounded by the document's limits (D301); `explain` gives neither an issuance's request nor
  when a derivation was recorded, the operator router serving the request, and issuance ids are
  not guessable (§7.3, §8.4 Limits); pack summaries go by leaf key, and keys' parts and the
  release's label are data tokens (§7.7); every negation of a translation is flagged (§7.1,
  §10.1); `count_cohort` is not read-only. Review round 2 amended D297, D300, D301 and D302,
  D200's check of two suppressed counts and D289's rules on removing derivations and on ids
  within v0.8.10, adding no numbers: the pass treats `n_true` and `n_false` alike, suppresses
  any count beside a small complement and a suppressed count's partner with it, so that a
  cohort's `not`, `known` and `unknown`, counted with it, pin none of its counts under *k* of 3
  or more, though other cohorts of the call can (§8.4); `log_bytes` counts
  every row pruning can free, issuances' included, and pruning removes a derivation no
  issuance names, a batch at a time, in a thread of the log's own (§12.2, §14); issuance ids
  are drawn as they are recorded and increase across restarts; a run the call's deadline ends
  names `tool_seconds`; an issuance's request is asked for in a request's body; a server without
  `/proc` serves without query workers. Review round 3 amended D297, D300 and D302, and D200's
  check of `lift_differs`, within v0.8.10, adding no numbers: `lift_differs` is suppressed with
  any count of its cohort, which bound it, and a two-valued collapse of a cohort is named among
  what the pass does not stop (§8.4 Limits); `log_bytes` counts the pages rows can take, and
  `keep_count_issuances_days` is at most 36,500 (§12.2); `not_issued` means not in the log, and
  a cohort's status tells whether it was counted within the retention period; `aibi-server check`
  and `serve` say when `count_cohort` is disabled for want of `/proc`; and `count_cohort` records
  nothing once an erasure of a dataset it names is recorded after it began canonicalising (D300,
  as D290 requires). Restacked onto #35's sign-off and #34's round 6, D300 is amended within
  v0.8.10: the datasets a call names are those its issuances name as D290 has it, through their
  derivations or in the document as written and its parameters. Review round 4 amended D297,
  D300 and D200's check of `lift_differs` within v0.8.10, adding no numbers: `lift_differs` is
  suppressed under every *k*, a 0 included, since the units it counts lie on one side of the
  cohort, and `known X`, counted with `X` in one call, shows that side beside counts all large
  (§8.4); the app DB refuses to open with pages of another size than `log_bytes` charges; and a
  test holds that `count_cohort` takes its erasure mark before it pins and canonicalises.
  Restacked onto #34's sign-off, whose vacuum after every redaction leaves no copy of a redacted
  text of the log in the app DB. Review round 5 signed it off, amending D297 and D300 within
  v0.8.10, adding no numbers: complements within a breakdown are not linked, which is
  differencing (§8.4 Limits); a tool call's pins are released without waiting on the store, and
  the sweep, the redactions and the vacuum their release allows run in the store's housekeeping
  thread, never in the call's; `aibi-server serve` exits with the store's message on an app DB
  of another page size; and a test holds the charge for a derivation's releases over
  derivations of 64 releases.
...
- **v0.8.8** — Choices the canonical form, ids, digests and the derivation log settle (D281–D290):
  ...
  Review round 7 amended D223 and D290 and §12.2 within v0.8.8, adding no numbers: erasure's
  matching follows one principle, that a constant goes exactly when it could name one of the
  person's rows, set out as a table keyed by where it sits: a key or foreign key column matches
  the keys of the person's rows in the table it names and nothing else, a unit key as a whole,
  a part of a composite key only through it, an identifier column the values that identify the
  person from any table, and a number in other units as evaluation converts it; text is
  compared in normalisation form C without format characters; a number in free text is parsed
  only when its digits could be a term's; a clause or unit key given as a parameter is matched
  as the cohorts that refer to it are. Review round 8 signed #34 off, with no blocker and no
  major, and amended D223 and D290 and §12.2 within v0.8.8, adding no numbers: a constant goes
  when it could name one of the person's rows or quotes one of their own identifying texts, so a
  key or foreign key column also matches the texts that identify the person, by their text
  alone, while a number there still matches only the keys of the table it names; a number given
  through a parameter is read only as the places that refer to it read it; and a vacuum that
  fails leaves its redaction pending, recorded in the redaction's own transaction and retried
  when the store opens and when a pin is released.
...
- **v0.8.4** — Choices the file importers settle (D224–D235): detecting parse settings; sources
  ...
  most (D225, D233, §14). A worker's peak memory is its `VmPeak`, sampled while an answer is
  awaited and sent with a panic, never its peak resident size on Linux, which holds the
  server's (D225).
