...
**Status:** Draft v0.8.14 · 2026-09-26
...
cohorts, on the unit table, as a cohort of one clause. A view whose cohorts and predicates all
canonicalised gets its canonical form (every default written, each predicate its canonical
clause tree) and its ids; a view that names no cohorts orders them by their computation ids, so
...
- **Limits.** These settings reduce casual disclosure. They do not prevent inference across
  ...
  gives no issuance's document as written (D302).
...
- The extension points are delivered in M1–M3 alongside the core features that call them, each
  ...
  applicability from M3.1, and run from M3.2, which materialises the inputs their `requires`
  name; until then a view of a pack's analysis is `NOT_SUPPORTED` (D316).
