...
**Status:** Draft v0.8.3 · 2026-09-24
...
| D213 | Release manifests (M1) | A manifest (format 1) holds the dataset, the descriptors blob, the sources (name, `text` or `rows`, hash) sorted by name, and the tables (id, hash, source, the source columns' ids and names in source order) sorted by id, with optional statistics and tombstones blobs; it is stored in RFC 8785 form as a blob named by its hash, and read back only in that form | One manifest, one hash: a release's id is a function of its content |
