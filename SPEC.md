...
**Status:** Draft v0.8.11 · 2026-09-25
...
- With *k* set, a disclosure pass runs on every output after it is computed. A suppressed value
  ...
  - **Linked counts.** In each linked set, a count from 1 to *k* − 1 is suppressed, and if exactly
    ...
    and `lift_differs` alone, which is also suppressed, a 0 included, whenever one of `n_true`,
    `n_false` and `n_unknown` beside it is, since it is bounded by them. A count of 0 is otherwise
    shown. The pass runs after categories are pooled and
  ...
  - **Cohort counts** (D297): the unit table's size is the total of the linked set `n_true`,
    ...
    with the two swapped. `lift_differs` is a set alone, and is suppressed, a 0 included, with any
    of the three counts: a strict cohort of `some` steps turns only unknown units definite when
    its lifts flip, so its `lift_differs` is at most, and may be, its `n_unknown`, and other
    cohorts bound it by `n_true` + `n_false`. The breakdowns are `null` with
    ...
    and `unknown` in one call, but under *k* = 2, where 1 to *k* − 1 is itself one value. The
...
| D305 | Named connections as import sources (M1) | An import or re-import names a source `{"connection": "<name>"}`, an identifier of `[databases.<name>]` and nothing else (`INVALID_VALUE`, listing the configured names, for another); a connection is a SQLite or DuckDB `path` inside an import directory, or for Postgres and MySQL `url_env`, and for Postgres and DuckDB a `schema` (`public` and `main` by default; the configuration refuses one on SQLite and MySQL, whose MySQL schema is the URL's database); a file is confined when the import runs, as any import's path is (D232), must be a regular file, and must still be that file (its device and inode) when the snapshot opens it and when it ends (`PATH_NOT_CONFINED`); a URL is read from the environment when the import runs, and must be `postgresql://` or `postgres://` (Postgres) or `mysql://` (MySQL) with a host and a database (`INVALID_VALUE`, naming the connection and the variable, never the value); the dataset's `source` is `{kind: "database", location}`, the location `<host>/<database>/<schema>` for a server, the file's location for SQLite and `<location>/<schema>` for DuckDB, never a port, user, password or query string; the dataset's name defaults to the connection's; a named connection is read by the core's importer (`aibi.databases`), never a pack's (`CONFLICTING_MEMBERS`), and pack validators' `validate_source` gets a `DatabaseSource` (connection, kind, location); the CLI names one with `aibi import <dataset> <name> --connection` | §14 has a request name a connection and provenance hold host, database and schema only; reading the URL when the import runs means a rotated credential needs no restart and the configuration never holds it; the location says which data it was without saying how to get in |
| D306 | Reading a snapshot (M1; deviates from issue #13 and §12.4's stack row, which read SQLite sources through DuckDB) | A snapshot is read in the import's worker process (D225's `Reader`, under `reader_memory`, `reader_seconds`, `decoded_bytes` and `reader_workers`), one per import, which the server's process only names: it never loads DuckDB. SQLite files are read with Python's `sqlite3`, not DuckDB's scanner: a `file:` URI with `mode=ro`, `SQLITE_DBCONFIG_DEFENSIVE` on, views, triggers, extension loading and `trusted_schema` off, `mmap_size` 0, `cell_size_check` on, temporary data in memory and `query_only`, and the tables those of `pragma_table_list` of type `table`, not named `sqlite_*` (one that is not a table SQLite keeps of its own, such as `sqlite_sequence`, is noted as skipped); a cell is an integer, a real, a text or null, and a BLOB is `UNSUPPORTED_FORMAT`. A DuckDB file is attached read-only to a session with extensions neither installed nor loaded, unsigned and community extensions refused, persistent secrets off, external access disabled but for the file and its `.wal`, no temporary directory, UTC, one thread, half `reader_memory` as its memory limit, and its configuration locked before it is attached; a generated column is `UNSUPPORTED_FORMAT`, naming it (its catalogue gives its expression as a default, which `DESCRIBE` does not). A SQLite or DuckDB file's `-wal`, `-shm` and `-journal` (DuckDB's `.wal`) must be regular files where they exist, and, where `/proc/self/fd` lists them, the regular files the reader opened must be the file and those beside it (`PATH_NOT_CONFINED`). A Postgres or MySQL session loads the scanner of its kind from its pinned wheel (`duckdb-extension-postgres-scanner` and `-mysql-scanner`, the version of DuckDB's own), whose signature DuckDB checks, attaches the URL read-only, then disables external access and locks its configuration; a MySQL session first turns off `mysql_tinyint1_as_boolean`, `mysql_bit1_as_boolean` and `mysql_order_pushdown_enabled` and pools one connection, taken without waiting (`mysql_pool_size` 1, `mysql_pool_acquire_mode` `try`); persistent secrets keep DuckDB's default there, since the scanners' `ATTACH` uses the secret manager, whose settings cannot change once it has been used, and it runs no statement that makes a secret; Postgres base tables are ordinary and partitioned tables (not partitions) of the schema, MySQL's `BASE TABLE`s of the URL's database, and the catalogue is read from their own catalogues (`postgres_query`, `mysql_query`), constants whose filters are bound parameters. Every read happens in one transaction (SQLite's deferred read transaction; DuckDB's, which the Postgres scanner runs as `REPEATABLE READ` and read-only); each table is counted before any row is read (`import_tables`, `table_columns`, `import_cells`), then read by a SQLGlot `SELECT` of its quoted columns ordered by its declared primary key, or else by all its columns; a server's tables are counted and read on the server, by `postgres_query` or `mysql_query` in the snapshot's transaction, its statement generated by SQLGlot in the server's dialect and bound as a parameter, each column the catalogue names, in its order, selected as `c0`, `c1`… and ordered by DuckDB, a Postgres ordinary table read `ONLY` (without the rows of the tables that inherit from it), and a decimal of more than 38 digits or of any (Postgres's `numeric` without a precision, through its domains too) cast to text there, whose rows are its exact text as a DuckDB file's and a Parquet file's decimals are; DuckDB's rows are read as Arrow, whose types the Parquet reader reads (`parquet.from_arrow`, D225: another type is `UNSUPPORTED_FORMAT`, naming the table, the column and its type); a cell over 131,072 characters is `UNPARSEABLE_SOURCE`, naming its row by its place in the snapshot's order, from 1, as every refusal of a snapshot's rows does, a snapshot having no header row; whatever else the database or its driver raises is `UNPARSEABLE_SOURCE` naming its class alone, and DuckDB running out of memory is `reader_memory` | DuckDB's SQLite scanner gives no foreign keys and binds a file's views as DuckDB SQL, while SQLite can refuse views and triggers itself and gives declared keys in the same read transaction; hardened DuckDB cannot attach a server (D172), so the server's session is locked once it has; the scanners must be bundled, not downloaded, and DuckDB's signature check makes a repackaged wheel safe to load; an order by key makes the same data the same raw snapshot, so a re-import of an unchanged database is `NO_CHANGE` whatever the server's storage order; DuckDB's and the drivers' messages can hold the URL; the scanners read a decimal wider than DuckDB's 38 digits as a double, silently losing digits, DuckDB renames Postgres's columns that differ only in case, and a Postgres parent table's rows include its children's unless read `ONLY`; a generated column would run the file's expression in the snapshot's session, where it could read its settings; neither SQLite nor DuckDB opens a path without following a link that replaced it after the check, and a link beside the file could have the reader open or write a file elsewhere; MySQL's `TINYINT(1)` holds integers, and one pooled connection keeps every read in the transaction; MySQL snapshots are untested here, and their tests run only with `AIBI_TEST_MYSQL_URL` |
| D307 | What a snapshot declares (M1) | A declared primary key is the table's key, `imported`, and inference looks for no other; a declared foreign key to a table of the snapshot is a relationship whose tables and columns are `imported` and whose cardinality is proposed from the rows (one-to-one when its present values are distinct), SQLite's without parent columns naming the parent's primary key, and no relationship is proposed by containment from a column it holds; names are matched as written, or else case-insensitively when exactly one matches; one whose parent is not read (in another schema or database too), whose columns do not match, whose parent columns are not the parent's declared primary key in some order, or whose child columns another of the table's has, is not kept, with a note; a table's and a column's comment is its `definition`, `imported` (at most 10,000 characters, `LIMIT_EXCEEDED`, and Unicode text without a control character but tab, line feed and carriage return, `UNPARSEABLE_SOURCE`, naming the table and the column); a column type DuckDB reads is its datatype, `imported` (a naive timestamp `imported_default`, as a Parquet file's, D227), and SQLite declares none; a declared key or foreign key the rows break stops the import (§13.2's declared keys), never dropped, the rows it names counted from 1 in the snapshot's order (D306) | §13.1 imports declared keys and comments as `imported`; a key the database enforces is no guess, and one it only declares (SQLite does not enforce foreign keys unless asked) is still the source's statement, which the gate checks rather than drops; a relationship leads to its parent's key (§5.6), so a foreign key to a unique column that is not the key is noted rather than refused by the release; a comment becomes a definition that agents and terminals read (A6), so a control character in it is refused, as text that is not Unicode is, rather than changed |
...
- **v0.8.10** — Choices the query tools settle (D296–D304): readbacks from templates, stating every
  ...
  derivations or in the document as written and its parameters.
