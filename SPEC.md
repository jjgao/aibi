...
**Status:** Draft v0.8.8 · 2026-09-25
...
| Data | DuckDB (CSV, Parquet, and Postgres, MySQL, SQLite and DuckDB sources), python-calamine (XLSX, XLS, ODS; `.xls` is refused in v1, D225), Parquet, SQLite |
...
- **Resource limits.** Request bodies, strings, lists (at most 10,000 members in a `values` or
  ...
  a DuckDB memory limit. The catalogue's tools (M1) read no rows, so a release built before its
