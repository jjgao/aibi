...
**Status:** Draft v0.8.13 · 2026-09-26
...
- **Cohort counts** (`count_cohort`, §11.1) are `{id, digest, population, size, disclosure,
  ...
  the views unchecked and the parameters used (D300).
...
- With *k* set, a disclosure pass runs on every output after it is computed. A suppressed value
  ...
  - **Linked counts.** In each linked set, a count from 1 to *k* − 1 is suppressed, and if exactly
    ...
    `analysed.n` and `excluded_units` (with
...
- **Limits.** These settings reduce casual disclosure. They do not prevent inference across
  ...
  units, which is differencing again. Restricted data needs access control, which is outside v1
  (§1.2). Without it, whether a cohort was counted on the server within the retention period of
  `count_cohort`'s issuances (§12.2) is visible to anyone who can write the cohort (its id's
  status in the derivation log), though not when; issuance ids are not guessable, and `explain`
...
- The MCP transport is stateless streamable HTTP at `/mcp`, behind request protection (§14); the
  ...
  `propose_descriptor`; M2 adds `validate_document`, `count_cohort` and `explain`; `list_analyses`
  and `run_analysis` come with the registry (M3).
- `validate_document` and `count_cohort` take `{"document": <the document as written>}`, and
  ...
  issues them), its readback and the caveats of its count that need no data, and the views
  unchecked (D299). `count_cohort` counts every cohort of the document in one query worker, which
...
- **Derivation log.** Two tables. *Derivations*, keyed by derivation id, hold the canonical
  ...
  *Issuances*, keyed by issuance id, record each time `run_analysis` or `count_cohort` produced
  ...
  Issuances of `count_cohort` are pruned after a configured period (`[log]
  keep_count_issuances_days`, 30 days by default) when the server starts and hourly after, in a
  thread of their own, a batch at a time, and when an operator asks (`aibi prune`), except one
  that a kept issuance names as the source of its values; a derivation no issuance names then goes
  with them. What pruning can free (the texts, and the derivations and issuances with their rows),
...
- **Resource limits.** Request bodies, strings, lists (at most 10,000 members in a `values` or
  ...
  of rows (`query_answer_bytes`, D293). A query tool resolves and canonicalises its document in
